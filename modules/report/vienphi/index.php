   ...
   $('#btn_luu').click(function(){	   
    $('#dialog_sotien').html('Số tiền thu là '+$('#tienthu').val()+'. Bạn có muốn tiếp tục')	;
	   ...
	   $.cookie("in_status", "print_preview"); 
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=hoan_ung&header=top&lien=2&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
			 $('#dialog_print').dialog('close');
	   
	   })
	      $('#btn_lien2').click(function(){
	   $.cookie("in_status", "print_preview"); 
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=hoan_ung&header=top&lien=3&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
			 $('#dialog_print').dialog('close');
