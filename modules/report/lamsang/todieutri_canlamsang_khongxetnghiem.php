...
<body>
	<?php
		...
		$params1 = array($_GET["id_todieutrichitiet"]);//tao param cho store 
        $store_name1="{call GD2_SelectChiDinhCanLamSangByIdToDieuTriChiTiet(?)}";//tao bien khai bao store
		...
		$params2 = array($_GET["id_todieutrichitiet"]);//tao param cho store 
        $store_name2="{call GD2_SelectThongTinLuotKhamByID_ToDieuTriChiTiet(?)}";//tao bien khai bao store
 ...
 <table cellpadding="0" cellspacing="0" border="0" style="width:100%;font-family:Arial, Helvetica, sans-serif;">
	...
	<h2 style="text-align:center; font-family:Arial, Helvetica, sans-serif;">PHIẾU CHỈ ĐỊNH</h2>
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;font-family:Arial, Helvetica, sans-serif; font-size:12px;">
		...
		if($ttcd["XetNghiem"]==1 && $flag==0 || $ttcd["XetNghiem"]!=1){
			if($ttcd["XetNghiem"]==1){
				...
				?>
                <tr>
                    <td <?php if($ttcd["ID_TrangThai"]=='Xong') {?>bgcolor="#F1F1F1" <?php }?>  style=" width:70%; text-align:left; padding-left:10px; padding-top:2px; padding-bottom:2px;">
                   <?=$i.' '.$ttcd["TenLoaiKham"]; ?>
                    </td>
