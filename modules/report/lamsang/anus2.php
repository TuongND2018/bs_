...
<body>
	...
	<?php if($_GET["header"]=="left"){ ?>
		<div style="font-size:14px;font-family:Arial, Geneva, sans-serif;-webkit-transform: rotate(270deg);-moz-transform: rotate(270deg);-o-transform: rotate(270deg);writing-mode: rl-tb;position:absolute;top:505px;left:-385px">  <span style="letter-spacing: 0.5px;font-weight:bold;color:#00b3a0">FAMILY</span> <img src="images/logo_mau.png" />  <span style="letter-spacing: 0.5px;text-transform:uppercase"><?php echo $_SESSION["com"]["TenBenhVien"]?></span> - <?php echo $_SESSION["com"]["DiaChi"]?> * T: <?php echo $_SESSION["com"]["SoDT"]?></div>
	<?php } ?>
     ...
     <table cellpadding="0" cellspacing="0" border="0" style="font-size:13px; width:100%;font-family:Arial, Geneva, sans-serif;padding:5px 0px">         
         <tr>
            <td valign="top" style="width:65%;padding-top:10px">
                ...
                <div>
                <?php if($thongso[0]["ChiSo21"]){ ?><i>+ Bị bệnh đã <?=$thongso[0]["ChiSo21"] ?> năm<?php } if($thongso[0]["ChiSo23"]!='' && $thongso[0]["ChiSo21"]!=''){ ?>, đã điều trị <?=$thongso[0]["ChiSo22"] ?> lần bằng phương pháp: <?=$thongso[0]["ChiSo23"] ?></i> <br /><?php }elseif($thongso[0]["ChiSo21"]){ ?>.<br /> <?php }?>
                ...
                <i><br />+ <?php
				 ...
				 if($thongso[0]["ChiSo31"]=='True'){
					  echo ", và hiện có đau khi đi cầu.";
				 }else {
					 echo ", và hiện không đau khi đi cầu.";
				 }
				?>
                </i>
                <i><br />+ Tiền sử:<?php 
