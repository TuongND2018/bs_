  ...
  top:-70px;
	...
	//background="images/toatam.jpg">
		...
		if($thongtinluotkham[0]["NgayKeDon"]!=''){
			$thongtinluotkham[0]["NgayKeDon"]=$thongtinluotkham[0]["NgayKeDon"]->format('d-m-Y');}
	  ...
	  if($tiemtruyen==0){
		 echo('		 
		  <tfoot  cellpadding="0" cellspacing="0" border="0" width="100%" >
			 ...
			 <tr><td colspan="2" align="center"><strong><i> Vì sự an toàn dược phẩm, vui lòng không trả lại thuốc đã mua</i></strong></td></tr>
		  </tfoot>
		 ');
	 }else{
		  echo('		 
  	...
  	for($i=0;$i<=count($thongtinluotkham)-1;$i++){
		...
		if($thongtinluotkham[$i]['ExtField2']==1){
			if($z==0){
				...
				echo('<td width="50%" style=""><div  style="display:table-cell;width:220px"><strong>' .($y) .' . '.$thongtinluotkham[$i][$_GET['tenin']].' </strong></div><div  style="display:table-cell">
						<strong>x   '.round($thongtinluotkham[$i]["SoThuocThucXuat"]).'   '.$thongtinluotkham[$i]["TenDonViTinh"].' </strong></div>
					 </div>
					 <div style="font-size:11px!important">
					'.$thongtinluotkham[$i]["CachDung"].' 
