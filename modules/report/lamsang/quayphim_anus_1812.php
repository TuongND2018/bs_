...
<body>
	<?php
        ...
        $store_name2="{call GD2_Anus_SelectKQByID_Kham_New(?)}";//tao bien khai bao store
	...
	<?php if($_GET["header"]=="left"){ ?> 
		<div style="font-size:14px;font-family:Arial, Geneva, sans-serif;-webkit-transform: rotate(270deg);-moz-transform: rotate(270deg);-o-transform: rotate(270deg);writing-mode: rl-tb;position:absolute;top:490px;left:-365px">  <span style="letter-spacing: 0.5px;font-weight:bold;color:#00b3a0">FAMILY</span> <img src="images/logo_mau.png" />  <span style="letter-spacing: 0.5px;text-transform:uppercase"><?php echo $_SESSION["com"]["TenBenhVien"]?></span> - <?php echo $_SESSION["com"]["DiaChi"]?> * T: <?php echo $_SESSION["com"]["SoDT"]?></div>
	<?php } ?>
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="width:100%;font-family:Arial, Geneva, sans-serif;">
     ...
     <table cellpadding="0" cellspacing="0" border="0" style="color:#036;font-size:15px; width:100%;font-family:Arial, Geneva, sans-serif; border-top:1px solid #000; border-bottom:1px solid #000; padding:5px 0px">         
         ...
         <tr>
            <td  style="width:47%">Địa chỉ: <?=$thongtinbenhnhan[0]["DiaChi"];?></td> 
     ...
     <table cellpadding="0" cellspacing="0" border="0" style="font-size:13px; width:100%;font-family:Arial, Geneva, sans-serif;padding:5px 0px">         
         <tr>
            <td valign="top" style="width:65%;padding-top:10px">
				...
				?>
                	<i><pre><?=$thongso[0]["BenhSu"]?></pre></i>
                <?php
				}
				?>
                </div>
                <b style="font-size:15px;color:#00b38b">KHÁM HẬU MÔN TRỰC TRÀNG</b>:
                <div>
                <?php
				if($thongso[0]["DacDiemHauMon"]==""){
				?>
                    <i><b>+ Đặc điểm hậu môn:</b> <?php 
				...
				?>	<i><pre><?=$thongso[0]["DacDiemHauMon"]?></pre>
                    </i>
                <?php
				}
				?>
                 ...
                 <b style="font-size:15px;color:#00b38b">KẾT QUẢ QUAY PHIM</b>:
                 <div>
                	<i><pre><?php if($thongso[0]["ThongSoKyThuat"]){?><?php }?><?=$thongso[0]["ThongSoKyThuat"]?></pre></i>
                </div>
                ...
                <b style="font-size:15px;color:#00b38b">KẾT LUẬN</b><br />
                 <i>
                <div><b>+ Chẩn đoán: <?=$thongtinluotkham[0]["ChanDoan"] ?><br />
                + Phương pháp điều trị: <?=$thongtinluotkham[0]["KetLuan"] ?><br />              
                </b>
                </i>
                </div>
			</td> 
             <td style="width:35%;padding-top:10px; text-align:center" valign="top">
                 <div id="images_container"></div>
             	 <br /><br />
                 <i>
                 	In từ dữ liệu gốc <br/> Ngày <?php echo $thongtinluotkham[0]["NgayGioChanDoan"]->format("d")." tháng " . $thongtinluotkham[0]["NgayGioChanDoan"]->format("m")." năm " .  $thongtinluotkham[0]["NgayGioChanDoan"]->format("Y"); ?>
                 </i>
                 <br />
                 <b>
    ...
    <script type="application/javascript">
		$(document).ready(function() {
			...
			for(i=0;i<=_split_link.length-2;i++){	
				if(i==0){		 
					$("#images_container").append(' <img width="200px" height="180px" src="'+_split_link[i]+'"  /><div style="height:7px;margin-top:4px;margin-bottom: 10px"><b>THÌ NGHĨ</b></div> ');
					
				}
				if(i==1){		 
					$("#images_container").append(' <img width="200px" height="180px" src="'+_split_link[i]+'"  /><div style="height:7px; margin-top:4px; margin-bottom: 10px"><b>THÌ RẶN</b></div> ');
