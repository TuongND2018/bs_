...
<body>
<?php //echo $_COOKIE["username_window"]; ?>
   <div id="printresults"></div>  
	<div style="display:none" id="grid_phong_ban"</div>  
...
$(document).ready(function() {
	<?php
		...
		echo "var elem1=[];\n";
		echo "elem1[0]='$_GET[from]';\n";
		echo "elem1[1]='$_GET[to]';\n";
		echo "idphong='$_GET[idphong]';\n";
		echo "loailich='$_GET[loailich]';\n";	
		//echo "mang_ngay='$_REQUEST[mang_ngay]';\n";
	?>
	creat_grid(elem1);
...
function creat_grid(elem1){	 
	...
	for (i = 1; i < dateArray.length+1; i ++ ) {
		mang_ngay1[i-1]=$.datepicker.formatDate("yy-mm-dd",new Date(dateArray[i-1]));
...
function build_print_view(){                
        ...
        /*var gridtable = jQuery("#rowed3");
        //var lista = gridtable.jqGrid('getGridParam','data');
		var lista = $("#rowed3").jqGrid('getGridParam','data');
        var tablestr = "";   
		alert(lista.length)   */   
		
				 var tablestr = "<table style='font-size:11px;' border=1>";   
				ids = $('#rowed3').jqGrid('getDataIDs');
                cell1 = $('#rowed3').jqGrid('getCell');
				tablestr += "<tr>";
				tablestr += "<td align='center'>Nickname</td>";
				for (ii = 0; ii < mang_ngay1.length; ii++) {
							tablestr += "<td align='center' class=\"wide cycle\" style=\"width:80px;\" id=\"sku\">"+mang_ngay1[ii]+"</td>";
				}
				tablestr += "</tr>";  
                for (i = 0; i < ids.length; i++) {
					...
					//alertObject(colvalues)                   
						tablestr += "<tr>"
						tablestr += "<td class=\"wide cycle\" style=\"width:50px;\" id=\"sku\">"+rowData["name"]+"</td>";
						for (ii = 0; ii < mang_ngay1.length; ii++) {
							tablestr += "<td align='center' class=\"wide cycle\" style=\"font-size:10.5px;width:160px;\" id=\"sku\">"+rowData[mang_ngay1[ii]]+"</td>";
						}
						//tablestr += "<td class=\"wide cycle\" style=\"width:300px;\" id=\"skudesc\">"+rowData[1]+"</td>";             
						tablestr += "</tr>";  
					}
					tablestr += "</table>";  
					 $('#printresults').html(tablestr);
       ...
       /* for(var i=0;i<lista.length;i++){
            ...
            var rowData = lista[i];
            tablestr += "<tr>"
            tablestr += "<td class=\"wide cycle\" style=\"width:50px;\" id=\"sku\">"+rowData[0]+"</td>";
            tablestr += "<td class=\"wide cycle\" style=\"width:300px;\" id=\"skudesc\">"+rowData[1]+"</td>";             
            tablestr += "</tr>";  
        }*/

  //  jQuery.get('resource.php?module=web_services&action=get_print_view', function(data) {            
