...
#ui-id-11.ui-autocomplete-loading {
    background: white url('images/ui-anim_basic_16x16.gif') right center no-repeat;
}
#ui-id-11.ui-button { margin-left: -1px; }     .ui-button-icon-only .ui-button-text { padding: 0.35em; }      .ui-autocomplete-input { margin: 0; padding: 0.48em 0 0.47em 0.45em; }

#ui-id-11.ui-menu {
 width: 500px!important; display:none; position:absolute;
 box-shadow:0px 0px 3px #333;
}
#ui-id-11.ui-state-highlight{
    color:#F60!important;
}
#ui-id-11.ui-menu-item a{
        ...
        <div class="ui-dialog-content ui-widget-content top_main"style="display: block; width: auto; min-height: 0px; max-height: none; height: 100%">
            <div id="calendar_option">
                ...
                <div style="float:left;margin-left:30px;margin-top:22px; ">
                    ...
                    <a style="margin-top:2px;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" id="xLog" href="#">Xem log<span class="ui-icon ui-icon-circle-check"></span></a>
                    <a style="margin-top:2px;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" id="xChamCong" href="#">Xem chấm công<span class="ui-icon ui-icon-person"></span></a><br />

                    <a style="margin-top:2px;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" id="xDonChamCong" href="#">Đơn chấm công<span class="ui-icon ui-icon-person"></span></a>
                    <a style="margin-top:2px;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" id="xTinhLaiCong" href="#">Tính lại công<span class="ui-icon ui-icon-person"></span></a>
                    <a style="margin-top:2px;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" id="xTongHop" href="#">Tổng hợp<span class="ui-icon ui-icon-person"></span></a>
                    <a style="margin-top:2px;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" id="xExcel" href="#">Xuất Excel<span class="ui-icon ui-icon-person"></span></a>
                </div>
            </div>
        </div>
   ...
   $("#calendar_option").find('input[type=radio]').each(function(idx, val, f) {
    if ($(this).is(':checked')) {
        if ($(this).attr("id") == "date_range_radio") {
         ...
         window.open("resource.php?module=report&view=lich_lam_viec&action=xuat_excel_tonghopcong&type=excel&fromdate="+elem1[0]+"&todate="+elem1[1]);
        //showGetResult();
...
window.abc=	 $.ajax({
    ...
    async:false
                  ...
                  window.loailich = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=1&tables=DM_LoaiLich&id=ID_LoaiLich&name=TenLoaiLich', async: false, success: function(data, result) {
                    if (!result)
                        alert('Không load được danh mục phòng ban');
                }}).responseText;
    ...
    dialog_sub_grid_xem_log("Xem log chấm công " + tam + " từ ngày " + dateFrom + " đến ngày " + dateTo, 570, 500, '', '');
            // alert('ds');
        })
...
//alert(window.tonggiocongNhanvien);





            dialog_sub_grid_xem_chamcong("Xem chấm công", "1240px", "600px");
        })
...

{
   ...
   $("#calendar_option").find('input[type=radio]').each(function(idx, val, f) {
    if ($(this).is(':checked')) {
        ...
        //alert( elem1[0])
        dialog_main("Xem đơn chấm công " + ' nhân viên', 98, 98, 5674365743657, "resource.php?module=<?=$modules?>&view=<?=$view?>&action=formduyetdon&type=test&id_form=139&fromdate="+elem1[0]+"&todate="+elem1[1]);
          /////////////////////
      })
...

{
   ...
   $("#calendar_option").find('input[type=radio]').each(function(idx, val, f) {
    if ($(this).is(':checked')) {
        ...
        //alert( elem1[0])
        dialog_main("Xem tổng hợp " + ' ', 98, 98, 5674365743657, "resource.php?module=<?=$modules?>&view=<?=$view?>&action=formTongHop&type=test&id_form=139&fromdate="+elem1[0]+"&todate="+elem1[1]);
...
function  right_menu(rowid, iCol, e) {
    ...
    function creat_sub_grid(row, colName, rowID) {
        ...
        $("#rowed4").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_lich1&q=2&idphong=' + row + '&from=' + colName + '' + '&loailich=' + $("#loailich option:selected").val() + '',
            datatype: "json",
            ...
            data :["onclose"],
            grid_save_option: "",
            editurl: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller1',
            rowNum: -1,
            pager: '#prowed4',
            gridview: true,
...
$("#rowed4").jqGrid('bindKeys', {"onEnter":function( rowid ) {
 ...
 $("#prowed4 .ui-icon-pencil").click();
        ...
        {}, // edit options
        {}, // add options
        {reloadAfterSubmit: true, url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller1&oper=del'}, // del options
...
aftersavefunc: function(rowId, response) {
 ...
 tooltip_message("Sửa dữ liệu thành công");
    ...
    function creat_sub_grid_XemLog(tungay, denngay) {
        ...
        $("#rowed_xLog").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_xem_log_chamcong&from=' + elem1[0] + '&to=' + elem1[1] + '&idphong=' + $("#phongban option:selected").val(),
            datatype: "json",
            colNames: ['cID', 'Tuần', 'Ngày', 'Tên', 'Giờ', 'Tên ngón', 'NgayThu','Chấm tại'],
            colModel: [
                {name: 'cID', index: 'cID', key: true, search: false, width: "80%", editable: false, align: 'right', hidden: false},
                {name: 'TuanThu', index: 'TuanThu', width: "80%", align: 'center', edittype: "select", hidden: false, formoptions: {}},
                {name: 'Ngay', index: 'Ngay', width: "80%", align: 'center', formatter: laytenngay, edittype: "select", hidden: false, formoptions: {}},
                {name: 'TenNhanVien', index: 'TenNhanVien', width: "80%", align: 'center', edittype: "select", hidden: false, formoptions: {}},
                {name: 'Gio', index: 'Gio', width: "80%", align: 'center', editable: true, editrules: {required: true, time: true}},
                {name: 'TenNgon', index: 'TenNgon', width: "80%", align: 'center', edittype: "select", hidden: false, formoptions: {}},
                {name: 'dayweek', index: 'dayweek', width: "0%", align: 'center', edittype: "select", hidden: true, formoptions: {}},
                {name: 'cDevive', index: 'cDevive', width: "80%", align: 'center', edittype: "select", hidden: false, formoptions: {}},
            ],
            rowNum: -1,
            pager: '#prowed4',
            loadonce:true,
...
/*function creat_sub_grid_XemLog(tungay, denngay) {
    ...
    $("#rowed_xLog").jqGrid({
        ...
        colNames: ['cID', 'Tuần', 'Ngày', 'Tên', 'Giờ', 'Tên ngón', 'NgayThu','Chấm tại'],
        colModel: [
        {name: 'cID', index: 'cID', key: true, search: false, width: "30%", editable: false, align: 'right', hidden: false},
        {name: 'TuanThu', index: 'TuanThu', width: "30%", align: 'center', edittype: "select", hidden: false, formoptions: {}},
        {name: 'Ngay', index: 'Ngay', width: "30%", align: 'center', formatter: laytenngay, edittype: "select", hidden: false, formoptions: {}},
        {name: 'TenNhanVien', index: 'TenNhanVien', width: "50%", align: 'center', edittype: "select", hidden: false, formoptions: {}},
        {name: 'Gio', index: 'Gio', width: "30%", align: 'center', editable: true, editrules: {required: true, time: true}},
        {name: 'TenNgon', index: 'TenNgon', width: "30%", align: 'center', edittype: "select", hidden: false, formoptions: {}},
        {name: 'dayweek', index: 'dayweek', width: "0%", align: 'center', edittype: "select", hidden: true, formoptions: {}},
        {name: 'cDevive', index: 'cDevive', width: "30%", align: 'center', edittype: "select", hidden: false, formoptions: {}},
        ...
        groupOrder: ['desc', 'des', 'asc'],
            ...
            caption: "Các log chi tiết " + $("#phongban1").val()
        });
...
searchOperators: true
    ...
    /*
     * 25/10/2013
     * khatm sửa lại xem chấm công theo kiểu mới
     */
     function creat_sub_grid_XemChamCong_new() {
  ...
  var curr_date = d.getDate();
    ...
    $("#rowed_xChamCong").jqGrid({
        url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_xem_chamcong_new&from=' + elem1[0] + '&to=' + elem1[1] + '&ID_NhanVien=' + $("#text_nhanvien").val(),
        datatype: "json",
        colNames: ['ID_ChamCong', 'ID_NhanVien', 'Tháng', 'Tuần', 'Tên', 'Ngày','s','GCông',
        'Giờ công', 'Chi tiết công - Logs','Ng. viện', 'Ko hợp lệ','Trễ sớm','Log Trễ sớm',
        'TG CB', 'dayweek','Ghi chú','Năm', 'Id_Donchamcong', 'Sự cố','Modify','LastUp'],
        colModel: [
        ...
        {name: 'Ngay', index: 'Ngay', width: "35%", formatter: tomauNgayChamCong, align: 'center', edittype: "select", hidden: false,},
        ...
        {name: 'TC', index: 'TC', summaryType: 'sum', width: "15%", align: 'center', hidden: false, },
        {name: 'ChiTietCong', index: 'ChiTietCong', width: "90%", align: 'left', edittype: "text", hidden: false, },

        {name: 'SoLanDiNgoaiVien', index: 'SoLanDiNgoaiVien', width: "12%", summaryType: 'sum', align: 'center', edittype: "select", hidden: false, },
        {name: 'LogKoHle', index: 'LogKoHle', width: "35%", align: 'center', edittype: "select", hidden: false, },

        {name: 'TreSom', index: 'TreSom', width: "18%", summaryType: 'sum', align: 'center', edittype: "select", hidden: false, },

        {name: 'TreSomChiTiet', index: 'TreSomChiTiet', width: "20%", summaryType: 'sum', align: 'center', edittype: "select", hidden: false, },
        {name: 'TGCB', index: 'TGCB', width: "12%", sorttype: 'number', formatter: 'number',  summaryType: 'avg', align: 'center', edittype: "select", hidden: false},
        {name: 'dayweek', index: 'dayweek', width: "0%", align: 'center', edittype: "select", hidden: true, },

        {name: 'GhiChu', index: 'GhiChu', width: "80%",formatter:tomaughichuchamcong, align: 'left', edittype: "select", hidden: false, },
        {name: 'Nam', index: 'Nam', width: "10%", formatter: 'number', summaryType: 'count', align: 'center', edittype: "select", hidden: true, },

        {name: 'Id_Donchamcong', index: 'Id_Donchamcong', width: "40%", formatter: 'number', summaryType: 'count', align: 'center', edittype: "select", hidden: true},

        {name: 'action', index: 'action', width: "25%", align: 'center', edittype: "button", hidden: false, },
        {name: 'NgayGioSua', index: 'NgayGioSua', width: "35%",  align: 'center', edittype: "text", hidden: false,},
        {name: 'NName', index: 'NName', width: "20%",  align: 'center', edittype: "text", hidden: false,},
        ...
        sortname: 'ID_ChamCong',
            ...
            groupText: ['<b class="total" style="color: blue"> Tháng {0}. Tổng giờ công: <span style="color:red" id="kha">{TongCong}</span> h. Đi trễ và ra sớm: <span style="color:red">{TreSom}</span> lần. Đi Ngoại viện: {SoLanDiNgoaiVien} lần. Trung bình thời gian chuẩn bị làm việc :;{TGCB};{Nam}',
            '<b style="color: red"> Tuần  {0} - Trung bình thời gian chuẩn bị : <span class="subTotal">{TGCB};{Nam}</span>'],
            groupCollapse: false,
            showSummaryOnHide: false,
        },
        ...
        , caption: 'Công chi tiết của ' + $("#nhanvien").val() + ' từ ngày '+dateFrom+' đến ngày '+dateTo,
        loadComplete: function(data) {
            ...
            for (i = 0; i < ids.length; i++) {
                ...
                var rowData = jQuery('#rowed_xChamCong').getRowData(ids[i]);
                   ...
                   // alert (rowDataNgay);
                    //var ngaysuco=parseDate(rowData['Ngay']);alert(ngaysuco);
                    var rowDataId_Donchamcong = rowData['Id_Donchamcong'];
                    //alert(rowDataId_Donchamcong);
                    ...
                    if (rowDataId_Donchamcong != 0) {
                      se = "<input class='quantitative' style='height:22px;width:65px;color: blue;' type='button'  value='Xem đơn' onclick=\"lamdon('" + rowDataNickName + "','" + rowDataNgay  + "','" +  rowData['ID_NhanVien']  +"');\" />";
                  }
                  else
                  {
                   se = "<input class='quantitative' style='height:22px;width:65px;color: blue;' type='button'  value='Làm đơn' onclick=\"lamdon('" + rowDataNickName + "','" + rowDataNgay  + "','" +  rowData['ID_NhanVien']  +"');\" />";
               }
...
function tomauNgayChamCong(cellvalue, options, rowObject) {
    ...
    if (rowObject[15] == 6)
    {
        return  '<span style="color:blue">' + newdate1 + "</span>";
    }
 ...
 function creat_grid() {
    ...
    var dateArray = getDates(new Date(elem1[0]), new Date(elem1[1]));
     ...
     window.url = "resource.php?module=report&view=lich_lam_viec&action=in_lich_lam_viec&idphong=" + $("#phongban option:selected").val() + '&loailich=' + $("#loailich option:selected").val() + '&from=' + elem1[0] + '&to=' + elem1[1] + "&mang_ngay=" + mang_ngay;

     $("#rowed3").jqGrid({
        url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_lich&q=2&idphong=' + $("#phongban option:selected").val() + '&loailich=' + $("#loailich option:selected").val() + '&from=' + elem1[0] + '&to=' + elem1[1] + "&mang_ngay=" + mang_ngay,
        datatype: "json",
        colNames: colnames,
        colModel: colvalues,
        loadonce: false,
...
var getColumnIndexByName = function(grid, columnName) {
 ...
 $("#rowed3").jqGrid('navButtonAdd', '#prowed3', {caption: "Ds ẩn", buttonicon: 'ui-icon-document',
        ...
        dialog_main("Xem Ds ẩn " + ' ', 100, 100, 5674234365743657, "resource.php?module=<?=$modules?>&view=<?=$view?>&action=formNhanvien_An&type=test&id_form=111&id_pban=19");
...
function render_col(cellValue, opts, rowObject) {
    ...
    function render_name(cellValue, opts, rowObject) {
        //cellValue
        if (kiemtra == false) {
            window.nickname = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_NhanVien&id=NickName&name=ID_NhanVien', async: false, success: function(data, result) {
                if (!result)
...
function col_span_week(date1, date2) {
    ...
    function  loadphongban() {
      window.ischamconglich = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_LoaiLich&id=ID_LoaiLich&name=IsChamCong', async: false}).responseText;
      window.chamconglich = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_LoaiLich&id=ID_LoaiLich&name=KieuLich', async: false}).responseText;
      window.phongban2 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=DM_PhongBan&id=ID_PhongBan&name=TenPhongBan', async: false, success: function(data, result) {
        if (!result)
            alert('Không load được danh mục phòng ban');
    }}).responseText;
      window.tenloailich1 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_loailich&id=ID_loailich&name=Tenloailich', async: false, success: function(data, result) {
        if (!result)
            alert('Không load được danh mục tên loại lịch');
    }}).responseText;
      window.nickname2 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option_byid&action=index&term=ID_PhongBan=' + $("#phongban option:selected").val() + '&status=2&tables=DM_NhanVien&id=ID_NhanVien&name=NickName', async: false, success: function(data, result) {
        if (!result)
            alert('Không load được danh mục phòng ban');
    }}).responseText;
...
;
 ...
 function customRange(input) {
    ...
    function tomauBackGround(cellvalue, rowId, mangthamso) {
        ...
        {
            $("#rowed_xChamCong").setCell(rowId, mangthamso, '', {background: 'orange'});
        }
    ...
    function lamdon(rowDataNickName, ngay, id_nhanvien) {
      ...
      dialog_main("Xem đơn của " + rowDataNickName, 100, 100, 5674365743657, "resource.php?module=<?=$modules?>&view=<?=$view?>&action=formlamdon&type=test&id_form=111&Ngaysuco="+ngay+"&nhanvien="+rowDataNickName+"&idnhanvien="+id_nhanvien);
...
function TimeCheck(id) {
        ...
        if (hieu >= 0) {
          ...
          /*  $.ajax({
                type: "POST",
                url: "resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_checktime&oper=" + oper,
                dataType: "json",
                data: dataToSend,
                async: false,
...
function pastelichtuan(){
   ...
   $('.danlichtuan').bind('click',function(e){
      ...
      if(window.lichtuan){
          ...
          $.post('resource.php?module=lich&view=lich_lam_viec&action=controller_Copy&loailich=' + window.id_lich_copy + '&phongbancopy=' + window.id_phongban_copy + '&ngay_copy=' + ngay_copy + '&ngay_paste=' + ngaypaste  + '&kiemtra='+window.colich).done(function(data){
    ...
    buttons: {
        Yes: function () {
         ...
         option_tinhcong=$('#optiontinhlaicong').val();
                                      ...
                                      $.post('resource.php?module=lich&view=lich_lam_viec&action=controller_duyetdon&oper=tinhlaicong&fromdate='+ elem1[0]+'&todate='+ elem1[1]+'&id_phongbanORnhanvien='+id_phongbanORnhanvien+'&option='+option_tinhcong+'&Ghide='+x+'&hienmaloi=3').done(function(data){
...
window.dataToSendCopy += ']}';
        ...
        function dan() {
            ...
            $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller1&oper=copy&user_copy=' + window.rowid + '&ngay_copy=' + window.from,
                window.dataToSendCopy
                ).done(function(data) {
            ...
            function del() {
             ...
             $('.xoa').click(function(e){
                ...
                $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller1&oper=del_cell&user_copy=' + window.rowid + '&ngay_copy=' + window.from
                    ).done(function(data) {
                        if ($.trim(data)== '') {
	...
	//	alert(ngay_del);
    $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller1&hienmaloi=2&oper=del_tuan&id_phongban=' +window.id_phongbancopy + '&ngay_del=' + ngay_del +'&loailich=' + window.id_loailichcopy
        ).done(function(data) {
            if ($.trim(data)== '') {
                tooltip_message("Xóa thành công");
                window.scrollPositiontop = jQuery("#rowed3").closest(".ui-jqgrid-bdiv").scrollTop();
...
function callback_form_lam_don(title, width, height){
    ...
    $("#first_navigator").click(function(){
      $.post('resource.php?module=lich&view=lich_lam_viec&action=data_lamdonchamcong&iddonsuco=1&id_nhanvien=' + $("#text_nhanvien").val()).done(function(data) {alert(data)});
  });
  ...
  $("#calendar_option").find('input[type=radio]').each(function(idx, val, f) {
    if ($(this).is(':checked')) {
        if ($(this).attr("id") == "date_range_radio") {
            ...
            var phongban = $.ajax({url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_totalgiocong&fromdate='+ fromdate +'&todate='+todate + '&ID_NhanVien=' + id_nvien, async: false, success: function(data, result) {if (!result) alert('Không load tổng giờ');}}).responseText;
