    ...
    <script type="application/javascript">
		...
		id_luotkham_array=id_luotkham.split(";;");
			for(var i=0;i<id_luotkham_array.length-1;i++){
				var _report_data=$.ajax({url: "resource.php?module=report&view=hanhchinh&action=ds_diachi&header=top&id_benhnhan="+id_benhnhan_array[i]+"&id_luotkham="+id_luotkham_array[i]+"&tencongty="+tencongty+"&type=&id_form=10", async: false, success: function(data, result) {if (!result) alert('Không load được danh mục NhomCLS');}}).responseText;
