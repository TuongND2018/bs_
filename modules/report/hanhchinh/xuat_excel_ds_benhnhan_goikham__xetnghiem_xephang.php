...
<div id="wrapper">
  ...
  <?php
   // $idbn=explode(";;",$_GET['id_benhnhan']);
	...
	$params = array($_GET['id_congty'],$_GET['id_goikham'],$_GET['xetnghiem'],$_GET['xephang']);//tao param cho store 
	$store_name="{call GD2_GetKSKCongTyByID_CongTyAndID_GoiKhamChoCongTyAndXetNghiemAndXepHang(?,?,?,?)}";//tao bien khai bao store
	...
	if($ttbn["NgayThangNamSinh"]!='')
		...
		if($tenbophan==$ttbn['TenNgheNghiep']){
			$tenbophan=$ttbn['TenNgheNghiep'];	
		}else{
			$tenbophan=$ttbn['TenNgheNghiep'];
			$sott_bophan=1;	
		}
