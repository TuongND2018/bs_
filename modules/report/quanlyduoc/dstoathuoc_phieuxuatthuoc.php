...
<body>
	...
	for($i=0;$i<=count($thongtinluotkham)-1;$i++){
		if($thongtinluotkham[$i]["NgayXuat"]!=''){
				$thongtinluotkham[$i]["NgayXuat"]=$thongtinluotkham[$i]["NgayXuat"]->format('H:i '.$_SESSION["config_system"]["ngaythang"]);}
  ...
  <tr >
   ...
   <table width="100%" border="0">
    <td ><label style="font-size:12px">Ngày xuất thuốc:<?=($thongtinluotkham[0]["NgayXuat"]) ?></label></td>
     <td  ><label style="font-size:12px">Phiếu xuất:<?=($thongtinluotkham[0]["MaPhieu"]) ?></label></td>
    </tr>
    </table>
    <table width="100%">
  <tr >
