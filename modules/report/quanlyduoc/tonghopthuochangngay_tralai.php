...
<?php
	...
	$store_name="{call GD2_TongHopThuocTraLai(?,?)}";//tao bien khai bao store
...
<body>
            <div style="width:100%;text-align:center">
                    <span  style="font-weight:bold">  TỔNG HỢP THUỐC TRẢ LẠI HẰNG NGÀY</span><br />
                   	<?php 
        ...
        <table id="bang_1" width="100%" cellpadding="0" cellspacing="0" border="1">
          ...
          <tbody id="tbody_1">
              <?php 
			  	$temp=$result[0]['NgayDuyet']->format($_SESSION['config_system']['ngaythang']);
				$count=1;
			  	for( $i=0;$i<count($result);$i++){
					
					$result[$i]['NgayDuyet']=$result[$i]['NgayDuyet']->format($_SESSION['config_system']['ngaythang']);
					if($i==0||$result[$i]['NgayDuyet']!=$temp)
					{
						$count=1;
						echo '<tr>';
                		echo '<td colspan="8" style="font-weight:bold">Ngày '.$result[$i]['NgayDuyet'].'</td>';
                		echo '</tr>';
						echo '<tr>';
                		echo '<td align="center">'.$count.'</td>';
						echo '<td>'.trim($result[$i]['TenGoc']).'</td>';
						echo '<td align="right">'.$result[$i]['SoLuong'].'</td>';
                		echo '</tr>';	
						$temp=$result[$i]['NgayDuyet']; $count++;
					}
					else{
						echo '<tr>';
                		echo '<td align="center">'.$count.'</td>';
