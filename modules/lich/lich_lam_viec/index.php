        ...
        <div class="ui-dialog-content ui-widget-content top_main"style="display: block; width: auto; min-height: 0px; max-height: none; height: 100%">
            <div id="calendar_option">
                ...
                <div style="float:left;margin-left:30px;margin-top:22px; ">
                    ...
                    <a style="margin-top:2px;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" id="xChamCong" href="#">Xem chấm công<span class="ui-icon ui-icon-calculator"></span></a><br />

                    <a style="margin-top:2px;" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" id="xDonChamCong" href="#">Đơn chấm công<span class="ui-icon ui-icon-document"></span></a>
...
window.phongban = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option_byid&action=index&term=IsPhongChuyenMon=0 and Active=1&status=2&tables=DM_PhongBan&id=ID_PhongBan&name=TenPhongBan', async: false, success: function(data, result) {
    if (!result)
...
function  right_menu(rowid, iCol, e) {
    ...
    function creat_sub_grid(row, colName, rowID) {
        ...
        $("#rowed4").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_lich1&q=2&idphong=' + row + '&from=' + colName + '' + '&loailich=' + $("#loailich option:selected").val() + '&idnhanvien=' + rowID ,
            datatype: "json",
...
$("#rowed_xLog").jqGrid('filterToolbar', {
  ...
  var curr_date = d.getDate();
    ...
    $("#rowed_xChamCong").jqGrid({
        url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_xem_chamcong_new&from=' + elem1[0] + '&to=' + elem1[1] + '&ID_NhanVien=' + id_nhanvien,
        datatype: "json",
        colNames: ['ID_ChamCong', 'ID_NhanVien', 'Tháng', 'Tuần', 'Tên', 'Ngày','s','GCông',
        'Giờ công','Nghỉ giữa ca', 'Chi tiết công - Logs','Ngoại viện', 'Log Ko hợp lệ','Trễ sớm','Log Trễ sớm',
        'TG CB', 'dayweek','Ghi chú - Cảnh báo từ hệ thống','Năm', 'Id_Donchamcong', 'Sự cố','Tạo lúc','Bởi','Lịch sửa lúc'],
        colModel: [
        ...
        {name: 'ThoiLuongNghiGiuaCa',sortable:false, index: 'ThoiLuongNghiGiuaCa', summaryType: 'sum', width: "10%", align: 'center', hidden: false, },


        {name: 'ChiTietCong',sortable:false, index: 'ChiTietCong', width: "85%", align: 'left', edittype: "text", hidden: false, },

        {name: 'SoLanDiNgoaiVien', sortable:false,index: 'SoLanDiNgoaiVien', width: "10%", summaryType: 'sum', align: 'center', edittype: "select", hidden: false, },
        ...
        {name: 'TGCB', index: 'TGCB',sortable:false, width: "12%", sorttype: 'number', formatter: 'number',  summaryType: 'avg', align: 'center', edittype: "select", hidden: false},
        {name: 'dayweek',sortable:false, index: 'dayweek', width: "0%", align: 'center', edittype: "select", hidden: true, },

        {name: 'GhiChu',sortable:false, index: 'GhiChu', width: "80%",formatter:tomaughichuchamcong, align: 'left', edittype: "select", hidden: false, },
        ...
        {name: 'NgayGioSua',sortable:false, index: 'NgayGioSua', width: "20%",  align: 'center', edittype: "text", hidden: false,},
        {name: 'NName', sortable:false,index: 'NName', width: "20%",  align: 'center', edittype: "text", hidden: false,},

        {name: 'NgayGioTaoLich', index: 'NgayGioTaoLich',sortable:false, width: "40%", align: 'center', edittype: "select", hidden: false, },
       ...
       // sortname: 'ID_ChamCong',
           ...
           // sortorder: "asc",
            ...
            groupText: ['<b class="total" style="color: blue"> Tháng {0}. Tổng giờ công : <span style="color:red" id="kha">{TongCong}</span> h. Đi trễ và ra sớm: <span style="color:red">{TreSom}</span> lần. Đi Ngoại viện: <span style="color:red">{SoLanDiNgoaiVien}</span> lần. Trung bình thời gian chuẩn bị làm việc :;{TGCB};{Nam}',
            ...
            for (i = 0; i < ids.length; i++) {
                ...
                var rowData = jQuery('#rowed_xChamCong').getRowData(ids[i]);
                    ...
                    if (rowDataId_Donchamcong != 0) {
                      countDonchamcong++;
                      se = "<input class='fm-button'   type='button' style='color:blue !important;padding: 0  0 !important'  value='Xem đơn' onclick=\"lamdon('" + rowDataNickName + "','" + rowDataNgay  + "','" +  rowData['ID_NhanVien']  +"');\" />";
                  }
                  else
                  {
                   se = "<input class='fm-button'  type='button' style='padding: 0  0 !important'  value='Làm đơn' onclick=\"lamdon('" + rowDataNickName + "','" + rowDataNgay  + "','" +  rowData['ID_NhanVien']  +"');\" />";
                  }
               ...
               $("#rowed_xChamCong").setCell(ids[i], 'TreSomChiTiet','', {color: '#FF0A0A'});
                $("#rowed_xChamCong").setCell(ids[i], 'LogKoHle','', {color: '#CC33FF'});
                   $("#rowed_xChamCong").setCell(ids[i], 'GhiChu','', {background: '#FFFFCC'});
                     //$("#rowed_xChamCong").setCell(ids[i], 'GhiChu','', {color: 'red'});
...
var getColumnIndexByName = function(grid, columnName) {
    ...
    for (var i = 0, l = cm.length; i < l; i++) {
        ...
        dialog_main("Xem Ds ẩn " + ' ', 100, 100, 5674234365743657, 'resource.php?module=<?=$modules?>&view=<?=$view?>&action=formNhanvien_An&id_pban='+ $("#phongban option:selected").val()+'&id_form=19&type=test');
...
function col_span_week(date1, date2) {
    ...
    function  loadphongban() {
      ...
      window.phongban2 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option_byid&action=index&term=IsPhongChuyenMon= 1 &status=0&tables=DM_PhongBan&id=ID_PhongBan&name=TenPhongBan', async: false, success: function(data, result) {
        if (!result)
            alert('Không load được danh mục phòng ban');
    }}).responseText;
