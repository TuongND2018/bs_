...
<style>
 ...
 -->
           ...
           function load_select() {
            ...
            window.nickname = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_NhanVien&id=ID_NhanVien&name=NickName', async: false, success: function(data, result) {
                if (!result)
...
function create_grid_sucobynhanvien() {
    ...
    $("#rowed3").jqGrid({
        url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&id_nhanvien=' + window.para_id_nhanvien + '&action=data_donsuco',
        datatype: "json",
        colNames: ['IndexID', 'Ngày sự cố', 'Gửi lúc','Sự cố', 'Nội dung giải trình','Chi tiết công', 'Log ko hợp lệ','Log trễ sớm',
        'Yêu cầu', 'Gửi', 'Đi trễ', 'Ra sớm', 'TBP','GSCM','GSHC','Xong','Tên','Công PM'],
        colModel: [
        ...
        {name: 'Id_Suco',editrules:{required:true}, index: 'Id_Suco', search: false, width: "20%", editable: true, align: 'center', formatter: "select", edittype: "select", hidden: false, editoptions: {value: tensuco}},
        ...
        shrinkToFit: true,
                   ...
                   caption: " DS đơn sự cố                                    <div class='hinhvuong camdo'></div><label class='diengiai'>Chờ TBP</label> <div class='hinhvuong quathoigian_min'></div><label class='diengiai'>Chờ G.sát</label> <div class='hinhvuong sansanggoi'></div><label class='diengiai'>Chờ TBP Hành chính</label> <div class='hinhvuong chuasansang'></div><label class='diengiai'>Đã giải quyết</label>"
               });
...
$("#rowed3").jqGrid('navGrid', "#prowed3", {add: permission["add"], edit: permission["edit"], del: permission["delete"], search: false, closeAfterEdit: true, clearAfterAdd: true,navkeys : [ true, 38, 40 ], closeOnEscape: true, bSubmit: "Submit",
                    bCancel: "Cancel"}, //options

                    {recreateForm: true, height: '500', width: '670', closeAfterAdd: true,
                    closeAfterEdit: true, reloadAfterSubmit: false, url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_donsuco&oper=edit', closeOnEscape: true, modal: true, recreateForm: true,
            ...
            onClose: function(formid) {
                ...
                {
                    height: '500', width: '670', reloadAfterSubmit: false, url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_donsuco&hienmaloi=3&oper=add&id_nhanvien='+para_id_nhanvien, closeOnEscape: true, closeAfterAdd: true, modal: true, addedrow: "first", recreateForm: true, savekey: [true, 121]
                ,
                ...
                afterSubmit: function(response, postdata) {
                 ...
                 // xóa options
                 {
                    ...
                    reloadAfterSubmit: false, url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_donsuco&oper=del', navkeys: [true, 38, 40], closeOnEscape: true,
                    afterSubmit: function(response, postdata) {
                ...
                {reloadAfterSubmit: true, height: '500', width: '670', sopt: ["cn"], url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_donsuco&q=2', closeOnEscape: true,
...
function getChiTietCong()
{
  ...
  /*  $.ajax({
        type: 'POST',
        async : true,
        url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_xem_chamcong_new&from='+Ngay+'&to='+Ngay+'&ID_NhanVien='+id_nvien,
...
$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_xem_chamcong_new&from='+Ngay+'&to='+Ngay+'&ID_NhanVien='+id_nvien).done(function(data){

 data=jQuery.parseJSON(data);
            var ChiTietCong=data.rows[0]['cell'][10];
            var LogkhongHopLe=data.rows[0]['cell'][11];
            var TreSomChiTiet=data.rows[0]['cell'][13];
...
function appendColumnGioPhut()
{
    ...
    $("#DenghiCongthem").before('<input type="text" name="DenghiCongthemGio" id="DenghiCongthemGio" style="border: none ; width: 20px!important" ><label id="label_gio">giờ</label><input type="text" name="DenghiCongthemPhut" id="DenghiCongthemPhut" style="border: none; width: 20px!important " ><label id="label_phut">phút</label> <label id="yeucau" name="yeucau"></label>');
