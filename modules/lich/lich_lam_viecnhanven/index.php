...
var lastsel; 	
	...
	jQuery(window).resize(function() {		 
	 ...
	 $("#rowed3").setGridHeight($(window).height()-$("#form_danh_muc_phong_ban").height()-145);
	}); 
   ...
   window.phongban = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_nhanvien&id=ID_nhanvien&name=tennhanvien', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban');}}).responseText;
...
function creat_sub_grid(rowid,colName){
	$("#rowed4").jqGrid({
...
function creat_grid(){	 
	...
	$("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_lich&q=2&idnhanvien=67',
		datatype: "json",	
		colNames:['Id','nickname','TenLoaiLich','GioBatDau','GioKetThuc'],
		colModel:[
			{name:'ID_Thuoc',index:'ID_Thuoc',search:false, width:"2%", editable:false,align:'right',hidden:true}, 
			{name:'nickname',index:'nickname', width:"100%", editable:true,align:'left',hidden:false,editrules: {required:true},edittype:"text",formoptions:{ rowpos:1, colpos:1}},
			{name:'TenLoaiLich',index:'TenLoaiLich', width:"200%", editable:true,align:'left',hidden:false,edittype:"text",formoptions:{ rowpos:2, colpos:1}},	 
			{name:'GioBatDau',index:'GioBatDau', width:"100%", editable:true,align:'left',hidden:false,edittype:"text",formoptions:{ rowpos:1, colpos:2}},	 
			{name:'GioKetThuc',index:'GioKetThuc', width:"200%", editable:true,align:'left',hidden:false,edittype:"text",formoptions:{ rowpos:2, colpos:2}},	
