...
<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"><span id="ui-id-5" class="ui-dialog-title">Thông tin hẹn khám</span></div>
    <div class="ui-dialog-content ui-widget-content"style="display: block; width: auto; min-height: 0px; max-height: none; height: 100%">
        ...
        <div class="form_thongtin_henkham" style=" height:54px">                            
                ...
                <div class="profile_container">
                     <span class="label_input" href="#" style="width:77px;">
                        <i class="fa icon" ></i><label for="bacsy"  alt="time" >Bác sỹ</label>
                    </span>
 ...
 "></span></a>     
    ...
    <div class="form_dat_lich" style="display:none;">        
          <input id="ID_HenKham"  name="ID_HenKham" type="hidden" value="">          
             ...
             <div class="profile_container">
                     <span class="label_input" href="#" style="width:77px;">
                        <i class="fa icon"></i><label for="ID_BSYeuCau1">Bác sỹ khám</label>     
                    </span>
...
$(document).ready(function() {	 
	...
	$.get( "resource.php?module=web_services&function=create_panel1&action=index&id_benhnhan=<?=$_SESSION["ThongTinBenhNhan"]["ID"]?>", function( data ) {
	...
	window.bacsy = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option_byid&action=index&term=IsDoctor=1&status=2&tables=DM_NhanVien&id=ID_NhanVien&name=NickName', async: false, success: function(data, result) {
           if (!result)
                    alert('Không load được danh mục bác sỹ');
            }}).responseText;
	...
	$(function() {
		...
		.click(function() {
			...
			$("#ngayhen").val(dates);			 
			//$("#rowed3").jqGrid().setGridParam({url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_lich&from=&to=&mangbacsy='+mangbacsy+"&date="+dates}).trigger("reloadGrid");
...
function init_lichcanhan(){	
	...
	$('#calendar').fullCalendar({
			...
			events: 'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_lich_canhan&idnhanvien='+_idnhanvien,			
...
function init_datlich(){
		...
		$.timeEntry.setDefaults({timeSteps: [1,10,1]});
...
function save_lich(){			    
				...
				if(window.oper=="add"){
					...
					/*$('.modal4732479').find(':input[type=text],select,textarea,input[type=hidden]').each(function(){							
						if((this.name!="ID_BSYeuCau1")&&(this.name!="ID_HenKham")){	
							if(i>0){
							  phancach=",";	
							}				 
							if(this.alt=="time"){								 												
					...
					$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper='+window.oper,
						dataToSend
					 )
					...
					dataToSend +='}';				 		
					dataToSend = jQuery.parseJSON(dataToSend);
					$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&hienmaloi=1&oper='+window.oper,
							dataToSend
						 )
						 .done(function( gridData ) {						  					 
							 if($.trim(gridData)==""){
...
function create_grid(date,callback){
	...
	window.mangbacsy="";
	window.mangnickname="";	
	var idbacsy;var nickname;
	window.mangbacsy=$.get("resource.php?module=web_services&function=get_doctor_list&action=index&column=ID_NhanVien&date="+date, function( data ) {
	  	window.mangbacsy=data;	
		idbacsy=window.mangbacsy.split(";");
		$.get( "resource.php?module=web_services&function=get_doctor_list&action=index&column=NickName&date="+date, function( data ) {		 
...
function create_grid_callback(date,nickname,idbacsy){	
	colnames = [];
    colnames = [""];
    colvalues = []; 
