...
foreach ($tam as $row) {//duyet toan bo mang tra ve
   // $responce->rows[$i]['id']=$row["ID_NhanVien"];
    $responce[]=array('id' => $row["ID_LichLamViec"],'title' => $row["TenLoaiLich"],'start' =>$row["Ngay"]->format("Y-m-d") ." ".$row["GioBatDau"].":00",'end' =>$row["Ngay"]->format("Y-m-d") ." ".$row["GioKetThuc"].":00",'url' => "#","allDay" => false);
    $i++; 
