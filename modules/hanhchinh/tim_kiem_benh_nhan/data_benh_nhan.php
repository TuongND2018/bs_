...
$search1 = "ChanDoan like N'%".$_GET["chuandoan"]."%'";

if($_GET["loaikham1"]!=0){
	$search1 = "ID_LoaiKham =".$_GET["loaikham1"];
}
if($_GET["loaikham1"]==0&&$_GET["chuandoan"]==''){
...
$data= new SQLServer;//tao lop ket noi SQL
$store_name="{call Gd2_timkiembn_(?,?,?,?,?,?,?,?,?)}";//tao bien khai bao store
$params = array('ID_Benhnhan',$start,$end,$sidx,$sord,'DM_BenhNhan',$search,'*',$search1);	
}
...
foreach ($tam as $row) {//duyet toan bo mang tra ve
 if($row["NgayThangNamSinh"]==""){
    ...
    $responce->rows[$i]['cell']=array($row['ID_BenhNhan'],$row['MaBenhNhan'],$row['HoLotBenhNhan'],$row['TenBenhNhan'],$row['NgayThangNamSinh'],$row['NamSinh'],$row['GioiTinh'],$row['DienThoai1'],$row['DiaChi'],$row['ID_NgheNghiep'],$row['TenNguoiLienHe'],$row['QuanHeVoiBN'],$row['DienThoaiNguoiLienHe'],$row['GhiChu']);
    $i++; 
}   
