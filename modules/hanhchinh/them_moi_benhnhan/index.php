...
<div class="thongtinchinh" style="width:100%;margin-top:5px;display:inline-block!important;overflow-y:scroll!important">
      <div id="main_top" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-resizable" style="width:99%;  box-shadow:none!important;  display: block!important;z-index: 1 !important;" >
        ...
        <div class="form_row" style="vertical-align:top"  >
          ...
          <!-- <div> <label for="nocuoi" name="nocuoi" style="width:120px;text-align:left;margin-left:5px ">Nợ cuối:  <?php /*if(isset($_GET['oper']) && $_GET['oper']=='edit'){echo $nocu[0][0];}else{echo 0;}*/?></label><label></div>-->
        </div>
...
$("#save_data").bind('click',function() {
	...
	if(window.dem==0){
		window.dem++;
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=add&hienmaloi=a',dataToSend).done(function(data) {
...
function minmax(value, min, max)
{
	...
	function create_rowed3(){
		...
		datatype: "json",
		colNames:['Thời gian đến khám', 'Thời gian kết thúc','Phân loại khám', 'Bác sỹ lâm sàng','Ngày giờ hẹn trả kết quả','Đã trả kết quả','ID_LuotKham'],
		colModel:[
	...
	$("#rowed3").jqGrid('navGrid',"#prowed3",{add: false, edit: false,addtext:'Thêm BN[F3]',edittext:"Sửa [F4]", del: false,refresh:false, search:false,closeAfterEdit: true,clearAfterAdd :true,closeOnEscape : true,
			});
		$("#rowed3").jqGrid('navButtonAdd', '#prowed3', {caption: "Tạo Lượt Khám[F12]", buttonicon: 'ui-icon-document-b',id : 'datlich_rowed3',
		 ...
		 $("#rowed3").jqGrid('navButtonAdd', '#prowed3', {caption: "Refesh[F5]", buttonicon: '',
            onClickButton: function() {
