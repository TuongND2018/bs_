...
foreach ($tam as $row) {//duyet toan bo mang tra ve SONGAYTHUCHIEN
    ...
    $responce->rows[$i]['cell']=array($row['ID_LuotKham'],$row['ID_Kham'],
$row['TenLoaiKham'],$row['NgayGioTao']->format("H:i ".$_SESSION["config_system"]["ngaythang"]),$row['TenBSChiDinh'],
$row['SoNgayThucHien'],$row['SoLanThucHienTrongNgay'],$row['DonGia'],
$row['PhiThucHien'],$row['TenTrangThaiCLSCuaBenhNhan'],$row['isDalap'],
$row['ID_LoaiKham'],$row['ID_DieuTriPhoiHop'],$row['ID_Physiotherapy'],
    	);
    $i++;
    $k++;
}
