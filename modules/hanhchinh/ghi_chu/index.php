...
jQuery(document).ready(function() {	 	
	window.nickname=	 $.ajax({                       
                        url: "resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=DM_Nhanvien&id=ID_Nhanvien&name=NickName",
                                               
                        async:false                       
                    }).responseText;
					window.luotkham=	 $.ajax({                       
                        url: "resource.php?module=web_services&function=get_custom_store&action=index&store=ThongTinLuotKham_LayLuotKhamMoiNhatCuaBenhNhan&term="+window.idbenhnhan+"&status=2&tables=ThongTinLuotKham&id=ID_LuotKham&name=ID_TrangThai",
    ...
    function create_grid() {
		var d =new Date();
        $("#rowed3").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ghichubenhnhan&idbenhnhan='+window.idbenhnhan,
            datatype: "json",
            ...
            shrinkToFit: true,
			sortorder: "desc",
			editurl: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller&idbenhnhan='+window.idbenhnhan,
			...
			{},
			{},
			{reloadAfterSubmit:true,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=del',	navkeys : [ true, 38, 40 ],closeOnEscape : true,	
    ...
    function create_grid2()
    {
		var d =new Date();
        $("#rowed4").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ghichuluotkham&idbenhnhan='+window.idbenhnhan,
            datatype: "json",
            colNames: ['Tên form','Người ghi chú','Ngày ghi chú','Nội dung'],
            colModel: [
                ...
                {name: 'ghichu', index: 'ghichu', search: false, width: "60%",editrules:{required:true}, editable: true, align: 'left', hidden: false}, 				
            ...
            rowNum: 1000,
			pginput:false,
			editurl: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller1&lk='+window.lk[0],
			pgbuttons:false,
            rowList: [],
            pager: '#prowed4',
            sortname: 'Caption',
			...
			luotkham1=	window.luotkham.split(';')
				...
				if(luotkham1[1]=='KetThucKham'||luotkham1[1]=='HuyXepHang'||$.trim(window.luotkham)==''){
					$("#rowed4_iledit").addClass('ui-state-disabled');	
...
function savepermission(){
	...
	$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller',
