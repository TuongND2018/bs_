...
var report_name=["Phiếu chỉ định điều trị"];
    jQuery(document).ready(function() {
	 ...
	 <?php 
		...
		echo 'window.loaidoituongkham="'.$tam2[0]['LoaiDoiTuongKham'].'";';
		echo 'window.idbenhnhan='.$tam2[0]['ID_BenhNhan'].';';
		echo 'window.n_idtrangthai="'.$tam2[0]['ID_TrangThai'].'";';
		echo 'window.n_dathanhtoanbill="'.$tam2[0]['DaThanhToanBill'].'";';
		echo 'window.n_id_quoctich="'.$tam2[0]['ID_QuocTich'].'";';
		echo 'window.n_hesotheoquoctich="'.$tam2[0]['HeSoVienPhi'].'";';
		echo 'window.id_phanloaikham="'.$tam2[0]['ID_PhanLoaiKham'].'";';
		echo 'window.n_ngayvaokham="'.$tam2[0]['ThoiGianVaoKham']->format($_SESSION["config_system"]["ngaythang"]).'";';
		echo 'window.n_ngayhientai="'.$tam2[0]['NgayHienTai']->format($_SESSION["config_system"]["ngaythang"]).'";';
		
		if($tam2[0]['PhanTram']=='.00'){
			echo 'window.phantram="0";';
		}else{
			echo 'window.phantram="'.$tam2[0]['PhanTram'].'";';
		}
	?>
					...
					if(window.n_dathanhtoanbill!=1 && window.n_khoachidinh!=1){
						...
						for(i=0;i<tmp.length;i++){
						   ...
						   if((window.loaidoituongkham=="BHYT") && (rowData["GiaTheoBaoHiem"]>0 )){
								...
								if(window.n_id_quoctich==138 || window.n_id_quoctich==142){
									var thanhtien_vienphi=rowData["BaoGiaChoBN"];
						  ...
						  var phithuchien=0;
							for(var i=0;i < tmp3.length;i++){ 
									...
									if(rowData["Color"]=="X"){
										$("#rowed5").jqGrid('setRowData', tmp3[i], false, { background: '#00FF99',border:'1px solid #00FF99' });
									}else if(rowData["Color"]=="V"){
										$("#rowed5").jqGrid('setRowData', tmp3[i], false, { background: '#CCFF66',border:'1px solid #CCFF66' });
									}
							}
...
var lastsel;
        ...
        $("#rowed3").jqGrid({
           ...
           loadonce: false,
            ...
            onSelectRow: function(id) {
				...
				$("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_hangmucloaikham_idnhomcls&id="+ID_NhomCLS+'&phantram='+window.phantram}).trigger('reloadGrid');
            },
    ...
    function create_grid2(){
        ...
        $("#rowed4").jqGrid({
            data: mydata,
            datatype: "local",
            colNames: ['GiaThueNgoaiThucHien', 'ThoiGianTrungBinhThucHien', 'ID_LoaiKham', 'Tên loại khám', 'Đ.Giá Viện phí','Đ.Giá BHYT','P.Thu BV','P.Thu tại nhà','Hệ số người nước ngoài','BH trả',''],
            colModel: [
				...
				{name:'GiaBaoChoBN',index:'GiaBaoChoBN', width:"33%", editable:true,align:'right',edittype:"text",hidden:false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},	
				...
				{name:'GiaPhuThu',index:'GiaPhuThu', width:"28%", editable:true,align:'right',edittype:"text",hidden:false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
			...
			ondblClickRow: function(rowId, rowIndex, columnIndex) {
			  if(window.n_dathanhtoanbill!=1 && window.n_khoachidinh!=1){
				 ...
				 if((window.loaidoituongkham=="BHYT") && (rowData["GiaTheoBaoHiem"]>0 )){
					...
					parameters =
							{
								initdata : {IDLoaiKham:rowData["ID_LoaiKham"],Xoa:"Xóa",LoaiKham:rowData["ID_LoaiKham"],ThanhTien:thanhtien_vienphi,PhiThucHien:phi_thuchien,TrangThai:"DangCho",ThucHien: "0",NguoiChiDinh:"<?=$_SESSION["user"]["id_user"] ?>",NhanVienDuocChiDinhTH:"",NguoiThucHien:"",GiaThueNgoaiThucHien:rowData["GiaThueNgoaiThucHien"],ThoiGianTrungBinhThucHien:rowData["ThoiGianTrungBinhThucHien"],IDLuotKham:$("#idluotkham").val(),IDPhongChuyenMon:$("#idphongbanchuyenmon").val(),MaBenhNhan:$("#mabenhnhan").val(),IDNhomCLS:celValue["ID_NhomCLS"],DonGiaBHYT:rowData['GiaTheoBaoHiem'],PhuThuBV:rowData["GiaPhuThu"],ThanhTienBaoHiem:bh_chitra,BNChiTra:bn_chitra,PhanTramCungChiTra:window.phantram,ThanhTienCungChiTra:bnchitrabh,Color:rowData["Color"]},
								position :"last",
								useDefValues : false,
								useFormatter : false,
								addRowParams : {extraparam:{}}
            ...
            loadComplete: function(data) {
			 ...
			 for(var i=0;i < tmp3.length;i++){ 
				var rowData = $("#rowed4").getRowData(tmp3[i]);	
			 	if(rowData["Color"]=="X"){
					$("#rowed4").jqGrid('setRowData', tmp3[i], false, { background: '#00FF99',border:'1px solid #00FF99' });
				}else if(rowData["Color"]=="V"){
					$("#rowed4").jqGrid('setRowData', tmp3[i], false, { background: '#CCFF66',border:'1px solid #CCFF66' });
				}
			 }
   ...
   function create_grid3(){
        ...
        $("#rowed5").jqGrid({
            ...
            colModel: [
				...
				{name: 'SLMoiNgay', index: 'SLMoiNgay', search: false, width: "12%", editable: true,edittype:"text",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '1'},editoptions: { dataEvents: [{ type: 'change', fn: function(e) {
								...
								setTimeout(function(){
									if((window.loaidoituongkham=="BHYT") && (parseInt(DonGiaBHYT)>0 )){
										...
										if(n_thuchien==0){
											...
											var bn_chitra= (((parseInt(thanh_tien) - parseInt(bh_chitra))*parseInt(sl_moi_ngay)) * parseInt(so_ngay));
										}else if(n_thuchien==1){
											var phi_thuc_hien= (((parseInt(thanh_tien)+parseInt(phuthu[n1]['PhuThuThucHienTaiNha'])) * parseInt(sl_moi_ngay)) * parseInt(so_ngay));
...
, align: 'center', hidden: false,classes:'slmoingay'},
				{name: 'SoNgay', index: 'SoNgay', search: false, width: "12%", editable: true,edittype:"text",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '1'},editoptions: { dataEvents: [{ type: 'change', fn: function(e) {
								var row = $(e.target).closest('tr.jqgrow');
                                var rowId = row.attr('id');
								var sl_moi_ngay=$('#'+rowId+'_SLMoiNgay').val();
								var thanh_tien=$('#rowed5').jqGrid('getCell',rowId,'ThanhTien');
								...
								if((so_ngay=="") || (so_ngay==0)){
									...
									setTimeout(function(){
										var phi_thuc_hien= ((parseInt(thanh_tien) * parseInt(sl_moi_ngay)) * parseInt(so_ngay));
										var bn_chitra= (((parseInt(thanh_tien) - parseInt(ThanhTienBaoHiem))*parseInt(sl_moi_ngay)) * parseInt(so_ngay));
										$('#rowed5').jqGrid('setCell',rowId,'PhiThucHien',phi_thuc_hien,'');
										$('#rowed5').jqGrid('setCell',rowId,'BNChiTra',bn_chitra,'');
									},200);
								}
           ...
           // pager: '#prowed5',
            ...
            loadComplete: function(data) {
				...
				jQuery("#rowed5").jqGrid('editRow',tmp1[i]);
					...
					}else if(rowData["Color"]=="X"){
						$("#rowed5").jqGrid('setRowData', tmp1[i], false, { background: '#00FF99',border:'1px solid #00FF99' });
					}else if(rowData["Color"]=="V"){
						$("#rowed5").jqGrid('setRowData', tmp1[i], false, { background: '#CCFF66',border:'1px solid #CCFF66' });
					}
				}
				$("#thanhtien").val(formatMoney(tongtien));
				$("#phithuchien").val(formatMoney(phithuchien));
       ...
       $("#rowed5").jqGrid('navGrid', "#prowed5", {add: false, edit: false, del: false, search: false, refresh: false,prmView: false, closeAfterEdit: true, clearAfterAdd: true, navkeys: [true, 38, 40], closeOnEscape: true});
		...
		if(window.loaidoituongkham=="BHYT"){
			$("#luotkhamnoingoai").html("( Bảo hiểm y tế - Số thẻ: <?=$tam2[0]['SoThe']?> )");
		}else{
	...
	function create_loaikham() {
        ...
        $("#data_loaikham").jqGrid({
            data: mydata,
            datatype: "local",
            colNames: ['Id gói khám','Tên loại khám', 'Giá báo cho Bệnh nhân','Đơn giá BHYT','Phụ thu', 'Mô tả',  'GiaThueNgoaiThucHien',  'ThoiGianTrungBinhThucHien',  'IDLuotKham',  'IDPhongChuyenMon',  'MaBenhNhan',  'ID_NhomCLS','',''],
            colModel: [
				{name: 'ID_LoaiKham', index: 'ID_LoaiKham', width: "10%", editable: false, align: 'left', hidden: true},
            ...
            loadComplete: function(data) {
				...
				var tmp3 = $("#data_loaikham").jqGrid('getDataIDs'); 
				 for(var i=0;i < tmp3.length;i++){ 
					...
					if(rowData["Color"]=="X"){
						$("#data_loaikham").jqGrid('setRowData', tmp3[i], false, { background: '#00FF99',border:'1px solid #00FF99' });
					}else if(rowData["Color"]=="V"){
						$("#data_loaikham").jqGrid('setRowData', tmp3[i], false, { background: '#CCFF66',border:'1px solid #CCFF66' });
					}
				 }
			},
            caption: "Loại khám"
...
function fun_inchidinh(){
 ...
 function resize_control() {
        ...
        $("#rowed4").setGridHeight($(".center_col").height() - 95);
