	...
	if(isset($_POST['id'])){
		...
		foreach ($_POST['id'] as $key => $value) {
			if($value['TrangThai']=="HuyBo"){
				...
				if($value['IDNhomCLS']==25){
					unset($params2);
					$store_name2="{call GD2_PHYSIOTHERAPY_Update_New (?,?,?,?,? ,?,?,?,?,? ,?,?,?)}";//tao bien khai bao store
					$params2 = array($value['Id_phy_dtph'],$value['PhiThucHien'],$value['ThanhTien'],$value['SLMoiNgay'],$value['SoNgay'],$value['ThucHien'],$value['DonGiaBHYT'],$value['PhanTramCungChiTra'],($value['ThanhTienCungChiTra']*$value['SLMoiNgay'])*$value['SoNgay'],$value['PhuThuBV'],($value['PhuThuBV']*$value['SLMoiNgay'])*$value['SoNgay'],$value['ThanhTienBaoHiem'],$_SESSION['user']['id_user'] );
					$data->query( $store_name2, $params2);
				}elseif($value['IDNhomCLS']==26){
					unset($params2);
					$store_name2="{call GD2_DieuTriPhoiHop_Update_New (?,?,?,?,? ,?,?,?,?,? ,?,?,?)}";//tao bien khai bao store
					$params2 = array($value['Id_phy_dtph'],$value['PhiThucHien'],$value['ThanhTien'],$value['SLMoiNgay'],$value['SoNgay'],$value['ThucHien'],$value['DonGiaBHYT'],$value['PhanTramCungChiTra'],($value['ThanhTienCungChiTra']*$value['SLMoiNgay'])*$value['SoNgay'],$value['PhuThuBV'],($value['PhuThuBV']*$value['SLMoiNgay'])*$value['SoNgay'],$value['ThanhTienBaoHiem'],$_SESSION['user']['id_user'] );
					$data->query( $store_name2, $params2);
				}else{
					unset($params2);
					$store_name2="{call GD2_KhamUpdate (?,?,?,?,? ?,?,?,?,?,?)}";//tao bien khai bao store
					$params2 = array($value['IDKham'],$value['PhiThucHien'],$value['ThanhTien'],$value['ThucHien'],$value['DonGiaBHYT'],$value['PhanTramCungChiTra'],$value['ThanhTienCungChiTra'],$value['PhuThuBV'],$value['PhuThuBV'],$value['ThanhTienBaoHiem'],$_SESSION['user']['id_user'] );
					$data->query( $store_name2, $params2);
				}
