    ...
    <div id='botton' >
    	<div id="bt_left" style=" min-width: 450px; ">
           ...
           <div id="bt_right">
                ...
                <input type="checkbox" id="botton_cddatra" style=" margin-left: 6%; " checked /><label for="botton_cddatra">Hiện chỉ định đã hủy bỏ</label>
                <input type="checkbox" id="botton_cdmoi" style=" margin-left: 1%; " /><label for="botton_cdmoi">Chỉ in chỉ định mới</label>
               
				<a  id="botton_inphieucd" class="fm-button ui-state-default ui-corner-all " style=" margin-left: 1%;  margin-top: -6px;"  > In phiếu CĐ</a>
				<a  id="botton_luu" class="fm-button ui-widget ui-state-default ui-corner-all fm-button-icon-left ui-button-text-only " style=" margin-left: 1%;margin-top: -3px; "  > <span class="ui-button-text">Chỉ định [F12]<span class="ui-icon ui-icon-disk nam"></span></span></a>
				<a  id="botton_chidinh"class="fm-button ui-state-default ui-corner-all" style=" display:none; margin-left: 1%;margin-top: -6px; "  >Chỉ định [F12]</a>
        	</div>
		</div>	
...
var report_name=["Phiếu chỉ định điều trị"];
	...
	$.ajax({
	  ...
	  $( ".patient_info" ).css("width", $( "#patient_info" ).width()+"css");		  
		...
		$('#botton_inphieucd, #botton_luu,#chongoikham,#hentrakq').button();
...
//f10 luu
	...
	$("#botton_inphieucd").click(function(){
	...
	for(var i=0;i<tmp1.length;i++){
		...
		//console.log(rowData['IDNhomCLS']);
		//console.log(rowData['TrangThai']);
	...
	//--
	/*if(window.vatlytrilieu==1){

		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=phieuchidinhvltl&header=top&xemtruocin="+window.xemtruocin+"&id_benhnhan="+$("#idbenhnhan").val()+"&id_luotkham="+$("#idluotkham").val()+"&type=report&id_form=10",'PhieuChiDinhDieuTri');
	
		kiemtra_dialog_modalu78787878975f(1);
		load_complete();		
		
	}else if(window.dieuduong==1){
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=phieuchidinhdieuduong&header=top&xemtruocin="+window.xemtruocin+"&id_benhnhan="+$("#idbenhnhan").val()+"&id_luotkham="+$("#idluotkham").val()+"&type=report&id_form=10",'PhieuChiDinhDieuTri');
		kiemtra_dialog_modalu78787878975f(2);
			load_complete2();
		}else if(window.phauthuatgiaithuat==1){
			dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=phieuchidinhpt_tt&header=top&xemtruocin="+window.xemtruocin+"&id_benhnhan="+$("#idbenhnhan").val()+"&id_luotkham="+$("#idluotkham").val()+"&type=report&id_form=10",'PhieuChiDinhDieuTri');
			kiemtra_dialog_modalu78787878975f(3);
			load_complete3();	
			}else if(window.family_planning==1){
			dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=phieuchidinhfamily_planning&header=top&xemtruocin="+window.xemtruocin+"&id_benhnhan="+$("#idbenhnhan").val()+"&id_luotkham="+$("#idluotkham").val()+"&type=report&id_form=10",'PhieuChiDinhDieuTri');
			kiemtra_dialog_modalu78787878975f(0);
			load_complete4();
			}
	*/
	...
	$("#botton_luu").click(function(){
		$('#botton_luu').button('disable');	
	if($("#idluotkham").val()==""){
			alert("Vui lòng chọn lượt khám để thực hiện chức năng này");	
	}else{
		$("#botton_cdmoi").removeAttr("disabled");
		
		ids = $('#rowed5').jqGrid('getDataIDs');
		for(var i=0;i<=ids.length-1;i++){
			jQuery("#rowed5").jqGrid('saveRow',ids[i]);
			//$("#rowed5").jqGrid('setRowData',ids[i],{Luu:1});
			
		}
		
		//alert(tongtien_vatlytrilieu+"-"+tongtien_tthuat_pthuat+"-"+tongtien_family_flanning+"-"+tongtien_dieuduong);
			
		$("#botton_chidinh").removeClass("ui-state-disabled");
	   ...
	   postData=$.parseJSON(postData);
		 $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller&oper=add&hienmaloi=3',postData).done(function(data){
				$("#botton_cddatra").prop( "checked", true );
				$("#rowed5").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_khamby_idluotkham&id="+$("#idluotkham").val()}).trigger('reloadGrid');
				tooltip_message("Lưu dữ liệu thành công");	
				$("#botton_luu").button('enable');
				setTimeout(function(){
					$("#botton_inphieucd").click();	
				},1000);		 
				//$('#botton_inphieucd').trigger("focusin");
				$('#gs_TenLoaiKham').focus();
		
				
		  })//$.post
	}//if idluotkham
	//$('#botton_inphieucd').focusin(function(e) {
		//$('#botton_inphieucd').focus();
       //$('#botton_inphieucd').click();
  //  });
	//$('#botton_inphieucd').on("keydown", false);
	//$('#botton_inphieucd').select();
		})//$("#botton_luu")
	
	$("#botton_cddatra").change(function() {
	...
	$("#botton_cdmoi").change(function() {
		
		if($(this).is(':checked')==true){
		$("#botton_cddatra").attr("disabled", true);
		var tmp = $("#rowed5").jqGrid('getDataIDs');
				for (var i = 0; i < tmp.length; i++) 
				{
					var rowData = jQuery('#rowed5').jqGrid ('getRowData', tmp[i]);
			...
			$("#botton_cddatra").removeAttr("disabled");
			var tmp = $("#rowed5").jqGrid('getDataIDs'); 
				for (var i = 0; i < tmp.length; i++) 
				{
					var rowData = jQuery('#rowed5').jqGrid ('getRowData', tmp[i]);
        ...
        $("#rowed3").jqGrid({
           ...
           loadonce: false,
            ...
            ondblClickRow: function(rowId, rowIndex, columnIndex) {
				...
				//alert(tmp.length);
				if(tmp.length==1){
					//var rowData = $("#rowed4").getRowData(tmp.length);
					//alert(rowData["ID_LoaiKham"]);
					//alert(tmp);
				var tmp1 = $("#rowed5").jqGrid('getDataIDs'); 
				 var tam,ln=0;
...
, align: 'center', hidden: false,classes:'slmoingay'},
				...
				{name: 'SoNgay', index: 'SoNgay', search: false, width: "14%", editable: true,edittype:"text",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '1'},editoptions: { dataEvents: [{ type: 'change', fn: function(e) {
								...
								var sl_moi_ngay=$('#'+rowId+'_SLMoiNgay').val();
								var thanh_tien=$('#rowed5').jqGrid('getCell',rowId,'ThanhTien');
								//var idnhomcls=$('#rowed5').jqGrid('getCell',rowId,'IDNhomCLS');
								var so_ngay=$('#'+rowId+'_SoNgay').val();
								...
								/*	//alert(so_ngay);
								if(parseInt(so_ngay)!=1){
									if(parseInt(idnhomcls)!=25 || parseInt(idnhomcls)!=23 || parseInt(idnhomcls)!=52 || parseInt(idnhomcls)!=26){
									
									$('#'+rowId+'_SoNgay').val("1");
									alert('Loại khám này không được phép chọn số ngày');
									}
								}
								*/
								var phi_thuc_hien= ((parseInt(thanh_tien) * parseInt(sl_moi_ngay)) * parseInt(so_ngay));
					...
					var tmp5 = $("#rowed5").jqGrid('getDataIDs'); 
							...
							if(thuchien==0){
								...
								if (typeof id_rowcls2 === 'undefined')
								{
									...
									$('#rowed5').jqGrid("setCell", rowId, 'ThanhTien', parseInt(phuthu[i]['GiaBaoChoBN'])+parseInt(phuthu[i]['PhuThuThucHienTaiNha']));
									...
									var phi_thuc_hien= ((parseInt(thanh_tien) * parseInt(sl_moi_ngay)) * parseInt(so_ngay));
									$('#rowed5').jqGrid('setCell',rowId,'PhiThucHien',phi_thuc_hien);
										//$('#rowed5').jqGrid("setCell", id_rowcls2, 'PhiThucHien', parseInt(phuthu[n]['GiaBaoChoBN'])+parseInt(phuthu[n]['PhuThuThucHienTaiNha']));
									}
									else{
			...
			{name: 'GiaThueNgoaiThucHien', index: 'GiaThueNgoaiThucHien', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				{name: 'ThoiGianTrungBinhThucHien', index: 'ThoiGianTrungBinhThucHien', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				{name: 'IDKham', index: 'IDKham', search: false, width: "30%", editable: false, align: 'left', hidden: false},
    ...
    function resize_control() {
		...
		$("#rowed3").setGridHeight($(".right_col").height() - 53);
		$("#rowed4").setGridWidth($(".center_col").width() - 11);
        $("#rowed4").setGridHeight($(".center_col").height() - 75);
        $("#rowed5").setGridWidth($(".right_col").width() - 11);
        $("#rowed5").setGridHeight($(".right_col").height() - 153);
	...
	function n_xoa(tam){
		...
		var tmp5 = $("#rowed5").jqGrid('getDataIDs'); 
		  ...
		  var phithuchien =0;
			...
			/*if((rowData2["TrangThai"]!="HuyBo") && (rowData2["TrangThai"]!="Xong") && (rowData2["TrangThai"]!="DaThucHien") ){  
				if(rowData2["IDNhomCLS"]==25 || rowData2["IDNhomCLS"]==26){
				$("#idk2").val(tam);
				$('#dialog2').dialog('open');
				}else{
			...
			}else if('<?=get_system_config_("GD2_UserNameAdmin")?>'=='<?=$_SESSION['user']['username']?>'){
					if(rowData2["IDNhomCLS"]==25 || rowData2["IDNhomCLS"]==26){
					$("#idk2").val(tam);
					$('#dialog2').dialog('open');
					}else{
				...
				tooltip_message("Bạn không có quyền thực hiện chức năng này");
				}*/
		}
		//tinhtien theo nhom loai kham
		var tongtien_vatlytrilieu=0;
...
function load_select_phy(){
	/*$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_nhomcon&ac=phy').done(function(data) {
		 window.phy=jQuery.parseJSON(data);
		});
		$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_nhomcon&ac=dtph').done(function(data) {
		 window.dtph=jQuery.parseJSON(data);
		});*/
