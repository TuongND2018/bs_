...
$("#xemin").click(function(e){
		if($('#In_CoChuKy input:checkbox').is( ":checked" )){
			dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=tieuphau&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10",'ThuThuat');
		}else{
			dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=tieuphau_khongchuky&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10",'ThuThuat');
			}
...
//===========================================
	if(id_kham2!="0"){
		$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaisieuam2&idkham="+id_kham2+"&idbenhnhan="+id_benhnhan, 
			function( data ) {
				data_luotkham=data;
			var tam1_cls="";
...
function edit_images(tam){
 ...
 dialog_add_dm('Chỉnh sửa hình ảnh',95,95,elem,'resource.php?module=images_control&view=images_edit&id_form=45&search_string='+tam[1]+"&tenthumuc="+tam[2],refresh_images);  
}
