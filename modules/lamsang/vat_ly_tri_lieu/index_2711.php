...
<body>
    <div id="tabs">
      ...
      <div id="tabs-1">
            ...
            <div>
              ...
              <div class="chuandoan">
                <div>
                  ...
                  <label style="margin-left:40px">Bs vật lý trị liệu:</label>
                  <span class="custom-combobox">
              ...
              <!-- <div class="chuandoan" style="margin-top: -170px;">
            ...
            <div class="chuandoan" style="margin-top:-140px!important">
              <div>
                    <label style="width:190px;text-align:left;font-size:14px">Triệu chứng:</label>
                    <label style="width:90px;text-align:left;font-size:14px;margin-left:300px">Lưu ý cho BN này(Lưu ý của BS):</label>
              </div>
              <div>
                    <textarea id="trieuchung" name="trieuchung" style="height:80px;width:337px" lang='luu'></textarea>  
                    <textarea id="luuy" name="luuy" style="height:80px;width:337px" lang='luu'></textarea> 
              </div>

            </div>
            <div class="chuandoan" style="margin-top:-30px!important">
              <button id="themnhatky" style="">Thêm nhật ký</button>
              <button id="hoantat" style="float:right">Hoàn tất đợt điều trị VLTL</button>
              <button id="xemin" style="float:right">Xem in</button>
              <button id="luu" style="float:right">Lưu</button>
              <button id="sua" style="float:right">Sửa</button>
            </div> -->
...
$(document).ready(function() {	
  $("#xemin").click(function(e){  
    ...
    dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=vatlytrilieu&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham3+"&id_physio="+id_physio+"&type=report&id_form=10",'MedicalReportVN');
...
function create_grid(){ 
   $("#rowed3").jqGrid({
    url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua&denngay="+denngay,
    datatype: "json",  
    colNames: ['id','id_bn','Mã BN', 'Tên bệnh nhân','Nội dung thực hiện','Ghi chú','Đã hoàn tất','','',''],
            colModel: [
                ...
                {name: 'ID_LuotKham', index: 'ID_LuotKham', hidden: true,width:90},
                {name: 'ID_Physiotherapy', index: 'ID_Physiotherapy', hidden: true,width:90},
                {name: 'NgayGioTao', index: 'NgayGioTao', hidden: true,width:90},
            ],
  //
    ...
    sortorder: "asc",
     grouping:true,
                groupingView : {
                        groupField : ['tenbn','NgayGioTao'],
                        groupColumnShow : [true,false],
                        groupCollapse : true,
                        groupText : ['<b>{0}  </b>','<b>{0}</b><input type="checkbox" onclick="hoantat_dieutri({ID_BenhNhan},{0},{1})" style="float:right;margin-top: -15px; margin-right: 40px;">']
...
function get_panel(){
  ...
  $.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua3&id_benhnhan="+id_benhnhan+"&id_kham=1", 
    function( data ) {
        data_luotkham=data;
...
$("#xem").click(function(){
  ...
  $("#rowed5").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua2&tungay="+tungay+"&denngay="+denngay}).trigger('reloadGrid');
  
});
function create_grid2(){ 
   $("#rowed4").jqGrid({
    url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua4&id_benhnhan="+id_benhnhan+"&id_kham=1",
    datatype: "local",  
   colNames: ['ID_LuotKham','ID_Kham','Loại khám', 'Bác sĩ chỉ định','Số ngày','Ngày bắt đầu','','','','',''],
            colModel: [
...
function create_grid3(){ 
   $("#rowed5").jqGrid({
    url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua2&tungay="+tungay+"&denngay="+denngay,
    datatype: "local",  
   colNames: ['id','id_bn','Mã BN', 'Tên bệnh nhân','Nội dung thực hiện','Ghi chú','Đã hoàn tất','',''],
            colModel: [
                {name: 'ID_Kham', index: 'ID_Kham', hidden: true,width:50},
                {name: 'ID_BenhNhan', index: 'ID_BenhNhan', hidden: true,width:50},
                {name: 'MaBenhNhan', index: 'MaBenhNhan', hidden: false,width:20},
                {name: 'tenbn', index: 'tenbn',width:50, hidden: false},
                {name: 'NoiDungThucHien', index: 'NoiDungThucHien', hidden: false,width:100},
                {name: 'KetLuan', index: 'KetLuan', hidden: false,width:90},
                {name: 'DaHoanTat', index: 'DaHoanTat', hidden: false,width:20,align:"center",edittype:"checkbox",editoptions: {value:"1:0"},formatter:"checkbox",},
                {name: 'ID_LuotKham', index: 'ID_LuotKham', hidden: false,width:90},
                {name: 'ID_Physiotherapy', index: 'ID_Physiotherapy', hidden: true,width:90},
            ],
  //
    ...
    sortorder: "asc",
     grouping:true,
                groupingView : {
                        groupField : ['tenbn','ID_LuotKham'],
                        groupColumnShow : [true,false],
                        groupCollapse : true,
                        groupText : ['<b>{0}  </b>','<b>{0}</b><input type="checkbox" onclick="hoantat_dieutri_2({0},{1})" style="float:right;margin-top: -15px; margin-right: 40px;">']
  ...
  //jsonReader:{repeatitems:true,subgrid:{repeatitems:false}},  
    ...
    ondblClickRow: function (rowId, rowIndex, columnIndex) {
      ...
      $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luu2&hienmaloi=3',dataToSend2)
    ...
    $("#left_col div div").click(function(e) {
        for(i=0;i<val.length;i++){
          ...
          if(tam==tam1){
            ...
            // $("#bsvatlytrilieu1").val(val[i]["cell"][8]);
            if(val[i]["cell"][8]!=""){
            ...
            // $(".chuandoan").css("height",$("#rowed4").height());
           // alert($("#rowed4").height());
             $(".chuandoan").css("width",$("#dialog-form").width()-$("#rowed4").width()-32+"px")
            // alert($("#dialog-form").width());
            /* if(id_trangthai=="Xong"){
              //alert("!");
             ...
             $('#rowed4').jqGrid('setGridParam', {url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua4&id_benhnhan='+id_benhnhan+"&id_kham="+tam,datatype:'json'}) .trigger('reloadGrid');

            $('#rowed6').jqGrid('setGridParam', {url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_nhatky&id_kham='+id_kham3,datatype:'json'}) .trigger('reloadGrid');
            if( $("#bsvatlytrilieu").val()==""){
            $("#bsvatlytrilieu").val("<?=$_SESSION["user"]["id_user"] ?>");
            }
...
function load_select () {
  ...
  window.bacsi = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_NhanVien&id=ID_NhanVien&name=NickName', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban');}}).responseText; 
  window.chuan_doan = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_Template&id=ID_Template&name=TenTemplate', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban');}}).responseText; 
}
...
$("#luu").click(function(){
  ...
  phancach = ",";
   ...
   $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luu1&hienmaloi=3',dataToSend)
                 .done(function( gridData ) { 
...
$('#dialog-form').bind('dialogclose', function(event) {
		$("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua&denngay="+denngay}).trigger('reloadGrid');
	 });
...
$("#chuaht").click(function(){
     $("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua"}).trigger('reloadGrid');
})
$("#daht").click(function(){
      $("#rowed5").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua2&tungay="+tungay+"&denngay="+denngay}).trigger('reloadGrid');
})
...
function hoantat_dieutri(total_idbn,ngaygiotao,soluong){
   ...
   dataToSend = '{"cc":[';
    phancach1 = "";
         for (i = 0; i < allRowsInGrid.length; i++) {
            ...
            if( pid2==ngaygiotao && id_benhnhan==pid3){
          ...
          dataToSend = jQuery.parseJSON(dataToSend);
           ...
           $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luu&hienmaloi=3&id_kham_update='+id_kham_update,dataToSend)
                              .done(function( gridData ) {  
                                                   tooltip_message("Đã lưu");  
                                                    $("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua&denngay="+denngay}).trigger('reloadGrid');
                                                   })
...
function hoantat_dieutri_2(id_luotkham_tong,soluong){
