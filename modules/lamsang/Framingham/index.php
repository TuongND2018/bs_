...
$(document).ready(function() {
 ...
 $('#xemin').click(function(){
   ...
   dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=Framingham&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10",'Framingham');
...
//lay table framingham điều trị tạm
  ...
  $.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_Framingham_DieuTri_All",
    function( data ) {
      //getFram_DT=data;
  var re;
      var tam1_cls="";
...
function luu(){
  ...
  if (ID_Framingham==null) 
  {
   ...
   $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=add&hienmaloi=3',datalocalToSend)
   .done(function( gridData ) {  
...
function laydulieu()
{
  $.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_Framingham_Laythongtin&idbenhnhan="+<?= $_SESSION["ThongTinBenhNhan"]["ID"] ?>,
    function( data ) {
      data_luotkham=data;
...
function getFramingHamDT(ch)
{
  ...
  $.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_Framingham_DieuTri&note="+ch,
    function( data ) {
      //getFram_DT=data;
  var re;
      var tam1_cls="";
