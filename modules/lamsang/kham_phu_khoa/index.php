...
<div class="thongtin_luotkham" id="right_col" style="width: 656px!important">
  ...
  <tr>
    <td rowspan="2">
    	<label id="id_trangthai"class="thongtin_thai" lang="luu" type="text" name="id_trangthai"style="color:red;font-size:14px;font-weight:bold"></label> <br>
    ...
    <td align="right" style="width:135px!important; height:34px!important; ">PARA
				    <input lang='luu' type="text" style="width:12px; margin-top:10px  " name="para1" id="para1" class="para" maxlength="1"/>
				    <input lang='luu' type="text" style="width:12px; margin-top:10px " name="para2" id="para2" class="para" maxlength="1" />
				    <input lang='luu' type="text" style="width:12px; margin-top:10px " name="para3" id="para3" class="para" maxlength="1"/>
				    <input lang='luu' type="text" style="width:12px; margin-top:10px " name="para4" id="para4" class="para" maxlength="1"/>
    </td>
  ...
  <tr>
    <td align="center">
    <a href="#" id="laydulieu" class="ui-widget ui-state-default ui-corner-all fm-button-icon-left ui-button ui-button-text-only" style="float: right; visibility: visible;"><span class="ui-button-text">Lấy dữ liệu<span class="ui-icon ui-icon-disk"></span></span></a>
    </a> 
    ...
    <span style="float:right; margin:5px 3px 0 0" id="In_CoChuKy">
 ...
 <div id="panel_main">    
        ...
        <fieldset id="lydo_fieldset" height="20%" width="100%"><table style="height:20%;">
            ...
            <fieldset id="khamlamsang_fieldset" style="max-height:205px" height="40%" width="100%"><table style="height:40%;">
	            ...
	            <tr><td><label style="width:15%;font-weight:bold;">Khám thực thể:</label></td>  
	            ...
	            <tr><td></td><td><textarea lang='luu'id="khamtt" name="khamtt"style="margin-top:-3px; width:100%;height:60px;  font-size:13px" ></textarea> </td>
	            </tr>
            </table></fieldset>   
        </div> 
    </div>          
        <div class="ui-widget-content  thongtin_thai ui-layout-center ">
            <fieldset id="khamcls_fieldset" height="60%" width="100%"><table style="height:60%; ">
	            ...
	            <tr><th><label style="width:15%;font-weight:bold;">Nhuộm soi:</label></th>  
            ...
            <fieldset id="Chuandoandtr_fieldset" height="40%" width="100%"><table style="height:40%;">
	            ...
	            <tr><td style=" width:101px!important;font-weight:bold;"><label>Chẩn đoán:</label></td>  
...
$(document).ready(function() {	
	$("#open_form_hentra").click(function(e){
					  ...
					  var links='resource.php?module='+folder[0]+'&view='+folder[1]+'&id_form='+folder[2]+"&idluotkham="+id_luotkham;				  				  
	...
	$("#xemin").click(function(e){	
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=khamphukhoa&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10&chuky="+chuky,'KhamPhuKhoa');
...
//para 
	...
	$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_phukhoa&idbenhnhan="+<?=$_SESSION["ThongTinBenhNhan"]["ID"]?>, 
		function( data ) {
			data_luotkham=data;
...
$("#add_templatekn,#add_templatepptt,#add_template,#add_templatektt,#add_templatecd,#add_templatedn").removeClass("ui-corner-all");
	...
	$("#laydulieu").click(function(){
		$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dl_canlamsang&ID_LuotKham='+id_luotkham).done(function(data){
			data=jQuery.parseJSON(data);
			if($.trim(data[0]["noidung"])==''){
				$("#stuoi").val("- Bạch cầu : ( xxx )\n-Tế bào biểu mô : ( xxx )- Clue Cell : (  xxx )\n - Nấm : ( xxx ) \n- Trichomonat : ( xxx )");
			}else{
			$("#stuoi").val(data[0]["noidung"]);
			}
			if($("#nsctc").val()==''){
				$("#nsctc").val(data[1]["noidung"]);
			}
			if($("#nhuomsoi").val()==''){
				$("#nhuomsoi").val(data[2]["noidung"]);
			}
			if($("#sieuam").val()==''){
				$("#sieuam").val(data[3]["noidung"]);
			}
			if($("#papsmear").val()==''){
				$("#papsmear").val(data[4]["noidung"]);
			}
...
function load_complete1(){
	...
	if(window.test2=="dathuchien1"){
			       $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luuthuchien&hienmaloi=3&id_benhnhan='+id_benhnhan,datalocalToSend)
			         ...
			         if(window.test2=="hoantat1"){
			         	$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luuhoantat&hienmaloi=3&id_benhnhan='+id_benhnhan,datalocalToSend)
								 .done(function( gridData ) {	
								 	delete window.datalocalToSend; 
								 							tooltip_message("Đã lưu");
			                                            testa();
			       ...
			       $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luudangkham&hienmaloi=3&id_benhnhan='+id_benhnhan,datalocalToSend)
								 .done(function( gridData ) {	
								 	delete window.datalocalToSend;
								 	tooltip_message("Đã lưu");
			                                            testa();	 
...
if(typeof window.datalocalToSend=='undefined'){
	...
	$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=hoantat&id_benhnhan='+id_benhnhan,datalocalToSend)
	.done(function( gridData ) {	
		 //reloadform(); 
		 testa();
		 delete window.datalocalToSend;
...
$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=dathuchien&id_benhnhan='+id_benhnhan,datalocalToSend)
.done(function( gridData ) {	
	//reloadform(); 
	testa();
	delete window.datalocalToSend;
