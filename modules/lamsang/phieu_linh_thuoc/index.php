...
<div id="dialog_sua" title="Sửa" style="display:none">
 <iframe id="i_sua" src="" width="100%" height="100%"></iframe>
    ...
    <div id="n_bottom">
        ...
        <button id="chotphieu">Chốt phiếu</button>
...
$(document).ready(function(e) {
	...
	$("#chotphieu").click(function(e) {
        $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=chotphieu&id_phieu='+window.idphieulinh).done(function(data){
			tooltip_message("Chốt thành công");
			$("#xem").click();
			$("#inphieulinh").button('enable');
			$("#inphieulinh").button('disable');
			$("#chotphieu").button('disable');
		});
    });
	...
	$("#inphieulinh").click(function(e) {
		...
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=phieulinhthuoc_vattuyte&idphieulinh="+window.idphieulinh+"&idphanloai="+window.idphanloai+"&thuocthongthuong="+rowData['ThuocThongThuong']+"&vattuyte="+rowData['VatTuYTe']+"&thuochuongthan="+rowData['ThuocHuongThan']+"&type=report&id_form=10",'PhieuLinhThuoc');
    });
	
	$("#themmoi,#sua,#timkiem,#inphieulinh,#inphieutonghopylenh,#xem,#chotphieu").button();
...
function create_grid() {
        $("#rowed3").jqGrid({
            ...
            colModel: [
				...
				{name:'Dalinh',index:'Dalinh', width:"5%",stype: 'select',searchoptions: { sopt: ['eq', 'ne'],value:':;1:Có;0:Không'}, editable:false,align:'center',hidden:false,edittype:"checkbox",formatter:"checkbox",formoptions:{ rowpos:13, colpos:1},editoptions: {value:"1:0"}},
           ...
           loadonce: true,
            ...
            loadComplete: function(data) {
				...
				if(window.n_tam==0){
					checkbox_search('gs_Dalinh','#rowed3');
				}         
				window.n_tam++;
