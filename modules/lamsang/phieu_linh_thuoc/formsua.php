...
<body>
    ...
    <?php
	 $data= new SQLServer;//tao lop ket noi SQL
        $params = array($_GET['idphieulinh']);//tao param cho store 
        $store_name="{call GD2_GetAllPhieuLinhByID_PhieuLinh(?)}";//tao bien khai bao store
  ...
  <tr>
    ...
    <td rowspan="2" >
    <textarea id="ghichu" style="width:325px; height:45px; float:left;"><?=$thongtin[0]['GhiChu']?></textarea>
...
$(document).ready(function(e) {
	...
	//$("#ngaylinh").datepicker();
	window.id_nguoilinh=<?=$thongtin[0]['Id_NguoiLinh']?>;
	$("#ngaylinh").datepicker({
		changeMonth: true,
		changeYear: true,
	...
	setTimeout(function(){
		create_combobox_new('#nguoilinh',create_nguoilinh,'bw','','data_nhanvien',window.id_nguoilinh);
	},500);
	autocompleted_combobox_callback('#loaidoituong',doituong_callback);


	$("#luu").click(function(e) {
		$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=checkchot&id_phieu=<?=$_GET['idphieulinh']?>').done(function(data){
		if(data=='false'){
			var id_phieulinhthuocchitiet='';
			var iddonthuoc='';
			if($("#nguoilinh").val()!=''){
				//var d=0;
				$('#rowed3 tr td[aria-describedby="rowed3_Chon"] input[type="checkbox"]').each(function() {
						...
						//$(this).prop('checked', true);
						 if($(this).is( ":checked" )) {
							//d=1; 
						  id_phieulinhthuocchitiet+=rowData["id_phieulinhthuocchitiet"]+";;";	
						   iddonthuoc+=rowData["ID_DonThuoc"]+";;";				 
						 }
					//$(this).prop('checked')
				});
				//if(d==1){
					$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_linhthuoc&oper=edit&idkhoa=<?=$_GET['idkhoa']?>&loaidoituong='+$("#loaidoituong").val()+'&nguoilinh='+$("#nguoilinh_hidden").val()+'&ghichu='+$("#ghichu").val()+'&iddonthuoc='+iddonthuoc+'&id_phieulinhthuocchitiet='+id_phieulinhthuocchitiet+'&phanloaithuoc='+window.idphanloai+'&ngaylinh='+$("#ngaylinh").val()+'&idphieulinh=<?=$_GET['idphieulinh']?>').done(function(data){
			...
			tooltip_message("Lỗi, Phiếu lĩnh này đã chốt");
		}
		});// end check
    });
});//end ready
...
function create_grid() {
        $("#rowed3").jqGrid({
            url: 'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsach_phieulinhthuoc_chitiet_sua&idphieulinhthuoc=<?=$_GET['idphieulinh']?>&idphanloai=<?=$thongtin[0]['Id_NhomphanLoaiThuoc']?>',
   			datatype: "json",
            colNames: ['','Mã đơn thuốc','Ngày kê đơn', 'Họ tên bệnh nhân', 'Tuổi', 'Địa chỉ','Số buồng','Số giường','Chọn hủy'],
            colModel: [
			{name: 'id_phieulinhthuocchitiet', index: 'id_phieulinhthuocchitiet', search: true, width: "8%", editable: false, align: 'left', hidden: true},
...
/*                window.selectedVal = "";
           ...
           loadonce: true,
            ...
            onSelectRow: function(id) {
				...
				$("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsach_donthuoc_chitiet&id_donthuoc="+rowData['ID_DonThuoc']+"&idphanloai=<?=$thongtin[0]['Id_NhomphanLoaiThuoc']?>"}).trigger('reloadGrid');
            },
