...
function create_ds_pbChuyenMon(elem,datalocal){ 
    ...
    function create_grid() {
        ...
        $("#rowed3").jqGrid({
            ...
            datatype: "json",
            colNames: ['ID_benhNhan','Mã BN','Họ tên BN','Tuổi','Giới tính','Đối tượng','Phòng CĐ nhập viện','Ngày CĐ','Chẩn đoán nơi giới thiệu','ID_Khoa','Khoa','ID_LuotKham','DiaChiBaoTin','BacSyDieuTri'],
            colModel: [
                {name: 'ID_BenhNhan', index: 'ID_BenhNhan', search: true, width: "6%", editable: false, align: 'left', hidden: true},
            ...
            ondblClickRow: function(rowId) {
				...
				$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_thongtinluotkham_check&id_benhnhan='+rowData['ID_BenhNhan']).done(function(data){
				    data = $.trim(data);
					data = data.split(';');
					if(data[1]=='KetThucKham'){
				 		parent.postMessage('open_idbenhnhan;tao_benh_an_noi_tru;'+rowData['ID_LuotKham']+';'+rowData['ID_BenhNhan']+';;'+rowData['ID_Khoa']+'__'+rowData['Khoa']+';'+rowData['TenBenhNhan']+'&doituong='+rowData['DoiTuong']+'&bacsydieutri='+rowData['BacSyDieuTri']+'&oper=add&diachibaotin='+rowData['DiaChiBaoTin'],'*');
					}else{
            ...
            ondblClickRow: function(rowId) {
				...
				$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_thongtinluotkham_check&id_benhnhan='+rowData['ID_BenhNhan']).done(function(data){
					...
					//if(data[1]=='KetThucKham'){
						window.luotkham=data[0];
						window.tenbn=rowData["HoLotBenhNhan"]+" "+rowData["TenBenhNhan"];
						parent.postMessage('open_idbenhnhan;tao_benh_an_noi_tru;'+rowData['ID_LuotKham']+';'+rowData['ID_BenhNhan']+';;'+rowData['ID_Khoa']+'__'+rowData['Khoa']+';'+rowData['TenBenhNhan']+'&oper=edit&id_ttluotkham='+rowData['ID_LuotKham'],'*');	
					//}
				})
