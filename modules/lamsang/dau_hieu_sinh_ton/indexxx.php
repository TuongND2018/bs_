...
function create_ds_grid(){
	 ...
	 $("#rowed3").jqGrid({
		...
		datatype: "json",
		colNames:['TT','ID_BenhNhan',"Mã BN", "Họ tên", "Tuổi", "Giới", "Phân loại khám","idloaikham","Trạng thái", "Giờ đến", "Ghi chú", "Lịch sử",'ĐT'],
colModel:[
		...
		loadComplete: function(data) {
			...
			for(var i=0;i < tmp1.length;i++){
				...
				xdnhanthan = "<input class='custom_button " + _class + "' style='height:22px;width:50px;' type='button' value='"+rowData["LoaiDoiTuongKham"]+"' onclick=\"moXacDinhNhanThan('"+rowData["ID_BenhNhan"] + "','" + rowData["LoaiDoiTuongKham"] + "');\" />";
				$("#rowed3").jqGrid('setRowData',tmp1[i],{GhiChu:ghichu});
				$("#rowed3").jqGrid('setRowData',tmp1[i],{Lichsu:lichsu});
				$("#rowed3").jqGrid('setRowData',tmp1[i],{LoaiDoiTuongKham:xdnhanthan});
			}
			if(lastsel_rowed3==""){
...
function moXacDinhNhanThan(pId_benhnhan,pdoituong)

{
//alert(pdoituong+'-'+pId_benhnhan);
//parent.postMessage('xacdinhnhanthan;'+pId_benhnhan+';'+pdoituong, "*");
			//parent.postMessage('xacdinhnhanthan;'+pId_benhnhan+';'+pdoituong, "*");

alert(pId_benhnhan);
			parent.postMessage('open_idbenhnhan;xacdinhnhanthan;;'+pId_benhnhan+';;;','*');
}
</script>
