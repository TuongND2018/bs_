...
$(document).ready(function(e) {
	...
	load_select_tenform();
	create_grid();
	$("#rowed3").jqGrid('setGridParam',{url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dshangmuckham&idluotkham='+<?=$_GET['idluotkham']?>,datatype:'json'}).trigger('reloadGrid');
	...
	$("#trakq").click(function(){
		if($('#trakq').is( ":checked" )){
			$.get('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=update_ttluotkham_traketqua&ac=true&id_luotkham='+<?=$_GET['idluotkham']?>).done(function(data) {
				$("#rowed3").jqGrid('setGridParam',{url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dshangmuckham&idluotkham='+<?=$_GET['idluotkham']?>,datatype:'json'}).trigger('reloadGrid');
						
			});
			}else{
				$.get('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=update_ttluotkham_traketqua&ac=false&id_luotkham='+<?=$_GET['idluotkham']?>).done(function(data) {
					$("#rowed3").jqGrid('setGridParam',{url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dshangmuckham&idluotkham='+<?=$_GET['idluotkham']?>,datatype:'json'}).trigger('reloadGrid');
        ...
        $("#rowed3").jqGrid({
			...
			,editoptions: { value:"1:0",dataEvents:  [{ type: 'click', fn: function(e) { 
					...
					if($('#'+rowId+'_DaInKetQua').is( ":checked" )){
						$.get('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=update_inketqua&ac=true&id_kham='+rowId+'&xetnghiem=0&idluotkham=<?=$_GET['idluotkham']?>').done(function(data) {
						});
					}else{
						$.get('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=update_inketqua&ac=false&id_kham='+rowId+'&xetnghiem=0&idluotkham=<?=$_GET['idluotkham']?>').done(function(data) {
						});
						}
			...
			,editoptions: { value:"1:0",dataEvents:  [{ type: 'click', fn: function(e) { 
					...
					if($('#'+rowId+'_TraKetQua').is( ":checked" )){
						$.get('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=update_traketqua&ac=true&id_kham='+rowId).done(function(data) {						
							$("#rowed3").jqGrid('setGridParam',{url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dshangmuckham&idluotkham='+<?=$_GET['idluotkham']?>,datatype:'json'}).trigger('reloadGrid');

						});
					}else{
						$.get('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=update_traketqua&ac=false&id_kham='+rowId).done(function(data) {						
							$("#rowed3").jqGrid('setGridParam',{url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dshangmuckham&idluotkham='+<?=$_GET['idluotkham']?>,datatype:'json'}).trigger('reloadGrid');
...
function load_select_nhanvien(){
	window.nickname = $.ajax({url: "resource.php?module=web_services&function=get_auto_edit_option_byid&action=index&term=ID_PhongBan<>21&status=2&tables=DM_NhanVien&id=ID_NhanVien&name=NickName", async: false, success: function(data, result) {if (!result) alert('Không load được danh mục nhân viên');}}).responseText;	
...
function n_inketqua(idkham,loaikham,tenform_in,idloaikham,xetnghiem){
	...
	if(id_control!=0){
		...
		if(data==1){ 
			...
			$.get('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=update_inketqua&ac=true&id_kham='+idkham+'&xetnghiem=0&idluotkham=<?=$_GET['idluotkham']?>').done(function(data) {
				$("#rowed3").jqGrid('setGridParam',{url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dshangmuckham&idluotkham='+<?=$_GET['idluotkham']?>,datatype:'json'}).trigger('reloadGrid');
