            ...
            <fieldset id="cacbenhdamac_fieldset">
                <legend>Các bệnh đã mắc</legend>
                        ...
                        <table width="100%">
                          ...
                          <tr>
                            <td><label for="bassdown">Bass down</label></td>
                            <td>
    ...
    <div class="dauhieubatthuongthainghen_phai" style=" width:50%;height:80px; float:left;" >
      <table width="100%">
        ...
        <tr>
          ...
          <td><label for="daumat">Đau mắt</label></td>
        </tr>
        <tr>
          <td>
        ...
        <div class="ui-widget-content  thongtin_thai ui-layout-center " id="n_center">
           <div id="sub_center">
           	<fieldset id="sub_center_fieldset" style="padding:5px !important;">
                ...
                <input type="checkbox" onClick=" return checkboxval(this.id);"  name="ktt_daxanhdiemmacnhot" lang='luu' id="ktt_daxanhdiemmacnhot"> <label for="ktt_daxanhdiemmacnhot">Da xanh, diêm mạc nhợt</label>
            </fieldset>
...
dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=khamthai&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10",'PhieuKhamThai');

});

$("#lichkhamthai").click(function(e){
$.cookie("in_status", "print_preview"); 
dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=lichhenkhamthai&header=left&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10",'PhieuKhamThai');
	...
	$.get( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_solankhamthai&idbenhnhan="+id_benhnhan, function( data ) {
	  //$( ".result" ).html( data );
	  window.solandakhamthai=data;
	  //alert(data);
	});
	
	$.get( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_sieuamthai4d&idbenhnhan="+id_benhnhan).done(function(data) {
	 if(data==0){
...
$("#add_khamnoikhoa,#add_khamphukhoa,#add_khamvu,#add_khamsankhoa,#add_khamkhac,#add_ketluan,#add_denghi").click(function(e){
	...
	$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaikham_thai&idbenhnhan="+id_benhnhan, 
		function( data ) {
			data_luotkham=data;
	 	//alertObject(data);
		var tam1_cls="";
	...
	$.each( data_luotkham, function( key, val ) {					 
		for(i=0;i<val.length;i++){
			...
			if(_id_luotkham[navigator_count]==tam1_cls[5]){
				...
				tam_cls+= '<div style="" id="'+val[i]["id"]+'" alt="'+ma_benhnhan+"_"+tam1_cls[5]+"_"+tam1_cls[3]+'">'+tenlk+'</div>';				
					...
					for(var i=0;i<sieuanthai4d.length;i++){
						...
						//console.log(_kham+"=="+idk);
							if(parseInt(_kham)>=parseInt(idk)){
								...
								date.setDate(date.getDate()+280-parseInt(sieuanthai4d[i]["SoNgayThai"]));
