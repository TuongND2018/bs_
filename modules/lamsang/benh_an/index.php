...
<div class="chandoanmau" style="display:none">
 ...
 <div class="top_form ui-widget-content">
    ...
    <!-- <a href="#" id="btn_dieutriphoihop" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="margin-left:0px; display:none  margin-bottom:1px; ">Điều trị phối hợp</a> -->
     ...
     <a href="#" id="btn_benhannoitru" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="margin-left:0px;  margin-bottom:1px; display:none ">BA nội trú</a> 		 
    </div>
 </div> 
    ...
    <div class="ui-widget-content ui-layout-east"> 
     <div class="thongtin_dieutri">
        ...
        <div id="prowed7" style="display:none"></div>       
         ...
         <div>
  		    <input class="custom_button xanh" type="button"  id="chitietquota" value="Chi tiết quota" style="height:22px;width:80px!important;display:none">
            <input class="custom_button xanh" type="button" id="miengiam"  value="Miễn giảm" style="height:22px;width:60px!important;display:none">
            <input class="custom_button xanh" type="button"  value="Ghi chú điều dưỡng" style="height:22px;width:100px!important;display:none">
...
<datalist id="ylenh">
  ...
  <option value="Lấy thuốc đem về">
            ...
            <input id="ngaytaikham" name="ngaytaikham"   type="text" style="width:50px; display:none" >
            <label for="ndtaikham"  style="width:150px;text-align:right; display:none">Nội dung tái khám</label>
            </div>
            <div  style="padding-left:5px;">
            <textarea id="ghichu" rows="3" cols="34"> 
            </textarea>
            <textarea id="noidungtaikham" rows="1" cols="34" style="display:none">            
            </textarea>
            </div>
            <div style="padding-left:5px;">
            ...
            <a href="#" id="btn_phieu" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="margin-left:0px; display:none   ">Phiếu vào viện</a>  
            </div>
    	</div>
    </div>   
...
$(document).ready(function() {
   ...
   $('#rowed3').bind("contextmenu", function(e) {
	...
	window.nickname=  $.ajax({                       
		url: "resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_NhanVien&id=ID_NhanVien&name=NickName",
		async:false                       
	}).responseText; 
	$('#btn_batdau,#btn_chidinhkham,#btn_dieutriphoihop,#btn_hoantat,#btn_luu,#btn_lammoi,#btn_dantoa,#btn_saotoa,#btn_xemtoa,#btn_intoa,#btn_chinhsua,#btn_phieu,#btn_benhannoitru').button();
	...
	$("#btn_xemtoa").click(function(){
		...
		if(($.trim($('#rowed5').jqGrid('getCell', $("#rowed5").jqGrid('getGridParam', 'selrow'), 'ChanDoan'))=='')&&(rowData['ID_trangthai']=='Xong')&&(user_login==rowData['BSChanDoan'])&&(IsDoctor==1)){
			...
			if( ($("#rowed6").getGridParam("reccount")-dong_tam)>0){
			  dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=benhan_intoathuoc&header=top&id_donthuoc="+rowData['ID_DonThuoc']+"&type=report&id_form=10&tenin=TenGoc&check=&xemtruocin=1",'InDonThuoc_Bsy');
			}else{
	...
	$("#btn_intoa").click(function(){
		...
		if(($.trim($('#rowed5').jqGrid('getCell', $("#rowed5").jqGrid('getGridParam', 'selrow'), 'ChanDoan'))=='')&&(rowData['ID_trangthai']=='Xong')&&(user_login==rowData['BSChanDoan'])&&(IsDoctor==1)){
			...
			if( ($("#rowed6").getGridParam("reccount")-dong_tam)>0){
			  dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=benhan_intoathuoc&header=top&id_donthuoc="+rowData['ID_DonThuoc']+"&type=report&id_form=10&tenin=TenGoc&check=&xemtruocin=0",'InDonThuoc_Bsy');
	...
	$("#btn_phieu").click(function(e){	
		$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=check_thanhtoan&idluotkham='+$("#rowed3").getRowData( rowed3_select)['ID_LuotKham']).done(function(data){
			if(data=='false'){
				elem=42432543543;		
				dialog_main("" , 80, 98, elem, "resource.php?module=lamsang&view=benh_an&action=phieuvaovien&header=top&type=report&id_form=10&id_benhnhan="+window.id_benhnhan+"&id_luotkham="+$("#rowed3").getRowData( rowed3_select)['ID_LuotKham']+"&id_kham="+rowed3_select)
...
$('#miengiam').click(function(){
	 ...
	 dialog_main("Miễn giảm: " , 98, 98, elem, 'resource.php?module=hanhchinh&view=mien_giam&id_form=230&id_donthuoc=' + rowData['ID_DonThuoc']+'&id_luotkham=' + rowData['ID_LuotKham']+'&id_benhnhan='+window.id_benhnhan)
	})
	...
	elem=42432543543;
	 rowData = $("#rowed3").getRowData( $("#rowed3").jqGrid('getGridParam', 'selrow'))	
	 dialog_main("Thông tin quota" , 95, 95, elem, 'resource.php?module=hanhchinh&view=chi_tiet_quota&id_form=1&id_quotaNV=' + rowData['id_quotaNV'])
	...
	$("#chandoan").dialog({
		...
		buttons: {
			"Ok": function() {			
				...
				var rowData = $("#rowed3").getRowData(rowed3_select);	
				$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_saochandoan&id_kham="+rowed3_select+"&id_benhnhan="+window.id_benhnhan).done(function(data){
	...
	window.loaikham=  $.ajax({                       
		url: "resource.php?module=web_services&function=get_auto_edit_option_byid&action=index&term=ID_NhomCLS=20 and ID_LoaiKham<>3918&status=2&tables=DM_LoaiKham&id=ID_LoaiKham&name=MaVietTat",
		async:false                       
	}).responseText;		
	...
	$.get( "resource.php?module=web_services&function=create_panel_small&action=index&id_benhnhan="+window.id_benhnhan, function( data ) {
	...
	$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_DMthuoc&id_benhnhan='+window.id_benhnhan).done(function(data) {
		data=data.split('|||');		
...
function create_lichsu_donthuoc(){	
	 ...
	 $("#rowed6").jqGrid({	
		data:mydata,	 
		datatype: "local",		 	
		colNames:['','','','','BH','','Tên thuốc',"Giá bán", "Đ.dùng", "SL", "Tổng", "Dược xuất", "C.dùng", "T.hiện"],
		colModel:[		
			...
			{name:'isbhyt',index:'isbhyt', width:"2%",editoptions: { value:"1:0"}, editable:true,align:'center',hidden:false,edittype:"checkbox",formatter:"checkbox",formatoptions:{disabled: false}},
			...
			,newgrid:true,func_grid:"check_soluong",editoptions:{
				dataInit: function (elem) {
					number_textbox_demical(('#'+elem.id))
				},
		...
		serializeRowData: function (postdata) { 	
		 ...
		 onCellSelect: function (rowid,iCol,cellcontent,e) {			
        	...
        	if((iCol==5)&&(rowData['ID_trangthai']=='DangKham'||rowData['ID_trangthai']=='DangSua'||rowData['ID_trangthai']=='DaThucHien')&&(user_login==rowData['BSChanDoan'])){
			  ...
			  $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_donthuoc&id='+rowData1['iddonthuocct']+'&oper=del').done(function(data){
				  if($.trim(data).split('|')[1]==0){
						 $('#rowed6').jqGrid('delRowData',rowid);
					}else{						
						alert('Toa thuốc đã có xuất tạm không thể xóa');
					}				   
				  })
            }      
			if(iCol==5){
		...
		//$("#"+iRow+"_"+name,"#rowed6").bind('blur',function(){
			...
			if(iCol==6){					
				...
				//alert($('#rowed6 #'+iRow+'_ID_Thuoc :selected').text());
	 ...
	 $("#rowed3").jqGrid({
		...
		serializeCellData: function (postdata) { 
		 ...
		 //}
			...
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_chung&ID_LuotKham='+rowData["ID_LuotKham"]).done(function(data){
				data=data.split('|||');				
				...
				$.each( datacls, function( key, val ) {		 
					for(i=0;i<val.length;i++){
						...
						}else if(tam1_cls[2]==4556||tam1_cls[2]==4383){
...
function create_toamau(elem,datalocal){	
		...
		if($(elem).data('clicked')){
			window.saotaophu=1;
			$('#thuocmau').setGridParam({datatype: 'json',url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_thuocmau&id_donthuoc='+id+'&ngaythuoc='+$('#rowed3').jqGrid('getCell', $("#rowed3").jqGrid('getGridParam', 'selrow'), 'SoNgayThuoc')}).trigger("reloadGrid");	
		}
		},
...
function batdau(){
	...
	$("#btn_batdau").click(function(){
		...
		$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controler&id_kham='+id_rowed3_current+'&ID_LuotKham='+$('#rowed3').jqGrid('getCell', id_rowed3_current, 'ID_LuotKham')).done(function(data){	
...
function create_Dm_thuoc_grid(elem,datalocal){	
		...
		datastr:datalocal,	
		datatype: "jsonstring" ,
		colNames:['Tên thuốc', 'Hoạt chất','Tên biệt dược', 'Tên nhóm thuốc','','','','bhyt','bhytnt','bhytnt','Hãng sản xuất','Nước sản xuất','',''],
		colModel:[			
			{name:'TenBietDuoc',index:'TenBietDuoc'}, 
...
function btn_luu(){
	$('#btn_luu').bind('click',function(e){
		...
		datatosend+=',"ID_Kham":'+JSON.stringify($("#rowed3").jqGrid('getGridParam', 'selrow'));
		if($('#taikham').is(':checked')){
		...
		$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_luu&hienmaloi=1',datatosend).done(function(data){
			$('#btn_lammoi').click();
		})
	})
}
...
function create_khamicd10(){	 
	 $("#khamicd10").jqGrid({
		...
		cmTemplate: {sortable: false},
		cellsubmit: 'remote',
		cellurl : 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_icd10',
		pgbuttons: false, // disable page control like next, back button
        pgtext: null, // disable pager text like 'Page 0 of 10'	 
		serializeRowData: function (postdata) { 
		return	postdata	 
...
function check_soluong(){
	 ...
	 $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_donthuoc_soluong&ds_kho='+ds_kho+'&soluong='+tongcong_thuoc+'&id_thuoc='+$('#'+id_row+'_ID_Thuoc').val()).done(function(data){
		data=data.split(';;');		
		/*if(data[1]==0){		
			window.thieuthuoc=1;
...
function right_menu(){
	...
	$('.chuyenthanhbschinh').click(function(){
		...
		if(id_admin==1){
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_chuyenbschinh&oper=1&id_luotkham='+rowData['ID_LuotKham']+'&id_kham='+rowed3_select+'&id_donthuoc='+rowData['ID_DonThuoc']).done(function(){
				$('#btn_lammoi').click();
				})
		}else{
	...
	$('.chuyenbschinh').click(function(){
		...
		rowData = $("#rowed3").getRowData( rowed3_select);
		console.log(rowData['ID_trangthai']+' '+user_login+' '+rowData['bschinh']+' '+rowData['BSChanDoan']+' '+IsDoctor+' '+rowData['luotkhamphu']);
		if ((rowData['ID_trangthai']=='DangKham'||rowData['ID_trangthai']=='DangSua'||rowData['ID_trangthai']=='DaThucHien')&&((user_login==rowData['bschinh'])||(user_login==rowData['BSChanDoan']))&&(IsDoctor==1)&&(rowData['luotkhamphu']>1)){			
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_chuyenbschinh&oper=1&id_luotkham='+rowData['ID_LuotKham']+'&id_kham='+rowed3_select+'&id_donthuoc='+rowData['ID_DonThuoc']).done(function(){
				$('#btn_lammoi').click();
				})
	...
	$('.saotoathuoc').click(function(){
		rowData = $("#rowed3").getRowData( rowed3_select)
		$('#thuocmau').setGridParam({datatype: 'json',url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_saothuoc&id_donthuoc='+rowData['ID_DonThuoc']}).trigger("reloadGrid");
		})
	...
	$('.saothuoctoaphu').click(function(){
		rowData = $("#rowed3").getRowData( rowed3_select)
			if(id_admin==1){
				window.saotaophu=1;	
				$('#thuocmau').setGridParam({datatype: 'json',url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_saothuoc&id_luotkham='+rowData['ID_LuotKham']}).trigger("reloadGrid");
			}else{
...
function saotaothuoc(){		
			...
			for(i=0;i<=id_thuocmau.length-1;i++){	
				...
				/*	$.ajax({
					  type: "POST",
					  async :false,
					  url: 'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_donthuoc_soluong&ds_kho='+ds_kho+'&soluong='+SoThuocThucXuat+'&id_thuoc='+grid_datathuoc.rows[a]['id'],
...
function laydulieu_khamlamsang(){
	$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_khamlamsang&idbenhnhan='+window.id_benhnhan).done(function(data){	
		  data=data.split('|||');
		 ...
		 $('#btn_chinhsua').click(function(e){
			 ...
			 if(id_khamsua==0){
				 ...
				 $.ajax({
					type: 'POST',
					async : true,
					url: 'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_kiemtra_sua&id_kham='+$("#rowed3").jqGrid('getGridParam', 'selrow')+'&id_donthuoc='+$("#rowed3").jqGrid('getCell', $("#rowed3").jqGrid('getGridParam', 'selrow'), 'ID_DonThuoc'),		
					success: function(output, status, xhr) {		
						output=output.split('||');
...
function kiemtrapass(){
	...
	dataToSend=$.parseJSON(dataToSend);
	$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_pass',dataToSend).done(function(data){
			if($.trim(data)==0){
...
function saotaogannhat(){
	$('#btn_saotoa').click(function(){
		...
		if ((rowData['ID_trangthai']=='DangKham')&&((user_login==rowData['bschinh']))&&(IsDoctor==1)){	
			window.saotaophu=1;	
			$('#thuocmau').setGridParam({datatype: 'json',url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_saothuoc&id_luotkham='+rowData['ID_LuotKham']+'&id_benhnhan='+window.id_benhnhan}).trigger("reloadGrid");
		}
	})
 ...
 $("#rowed6").click(function(e){
	...
	if($.trim(rowData1['LoaiDoiTuongKham'])=="BHYT"){
	 if($(e.target).is(":checkbox")){
		...
		$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controler_isbhyt&id_donthuocct='+rowData['iddonthuocct']+'&trangthai='+trangthai).done(function(){
			var rowData = $("#rowed3").getRowData(rowed3_select);
			$("#rowed6").jqGrid().setGridParam({
				url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_donthuoc&iddonthuoc="+rowData["ID_DonThuoc"]
