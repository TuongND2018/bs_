...
<body>
 <div id="panel_main">            
        <div class="left_col ui-widget-content ui-layout-west" id="LeftPane">
            ...
            <div style="margin-top:5px">
            	<div style="float:left;" >
                    <div class="hinhvuong quathoigian_min" style="display:none"></div><label  style="display:none" class="diengiai">Đã quá thời gian chờ 10 phút</label>
                    <div class="hinhvuong quathoigian_max" style="display:none"></div><label style="display:none" class="diengiai">Đã quá thời gian chờ 15 phút</label>
                </div>
     ...
     "></span></a>-->
                    <a href="#" id="thuchien" class="ui-button ui-widget ui-state-default ui-corner-all fm-button-icon-left" style="margin-left:0px;  margin-bottom:1px; margin-right:5px; ">Đã thực hiện<span class="ui-icon ui-icon-cancel
     "></span></a>
				</div>
            </div>
        </div>         
...
create_combobox_new('#com_khoa',create_khoa,'bw','','data_khoa',<?=$_GET['idkhoa']?>);
...
function create_ds_grid(){	
	 ...
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data&idkhoa='+window.idkhoa,
		datatype: "json",	
		colNames:['TT','ID_BenhNhan',"Mã BN", "Họ tên", "Tuổi", "Giới", "Số buồng","Số giường","Trạng thái"],
colModel:[
			{name:'STT',index:'STT',width:"20%",align:'right',hidden:false,search:false},	
			{name:'ID_BenhNhan',index:'ID_BenhNhan',width:"80%",align:'right',hidden:true},	
			...
			{name:'SoBuong',index:'SoBuong',width:"80%",align:'left',hidden:false,},
			{name:'SoGiuong',index:'SoGiuong',width:"80%", align:'left',hidden:false,},
		...
		onSelectRow: function(id){
			...
			if((lastsel_rowed3!=id)||(kiemtraClickLandau==true)){								 
						...
						// $("#rowed5").jqGrid().setGridParam({url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_sinhton_lichsu&id_luotkham='+id,loadonce: false,datatype: "json"}).trigger("reloadGrid");
						 ...
						 // $("#rowed7").jqGrid().setGridParam({url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_thetrang_lichsu&id_luotkham='+id,loadonce: false,datatype: "json"}).trigger("reloadGrid");	
...
function create_sinhton_grid(){
	 ...
	 $("#rowed4").jqGrid({
		data:mydata,
		datatype: "local",	
		colNames:["id_luotkham","id_benhnhan",'Lần đo','Người đo',"Ngày giờ đo", "Ps", "Pd", "Hr", "Temp","Rr"],
		colModel:[
			...
			{name:'NguoiThucHien',index:'NguoiThucHien',width:"100%",align:'left',hidden:false, editable: true,formatter: "select",editoptions:{value:window.nickname1, defaultValue: <?=$_SESSION['user']['id_user']?>},edittype:"select"},	
			{name:'NgayGioDo',index:'NgayGioDo',width:"80%",align:'center', editable: true,hidden:false},           	 	 
			...
			{name:'NhipTho',index:'NhipTho', width:"40%",align:'right',hidden:false, editable: true,editoptions: {defaultValue: '0'},cellattr: tomaudo,editrules:{minValue:0}},					                            
		],
		grid_save_option: "",
		loadonce: true,
		scroll: false,		 
               ...
               // mtype: 'post',				 			
                addRowParams: {
					...
					oneditfunc: function(rowId) {
					    ...
					    $('#'+rowId+'_NgayGioDo').datetimeEntry({datetimeFormat: 'H:M D/O/y',spinnerImage: ''});
						$('#'+rowId+'_NgayGioDo').val(new Date().toString("H:mm dd"+$.cookie('phancachngay')+"MM"+$.cookie('phancachngay')+"yy"));				   	 					$("#"+rowId+"_NgayGioDo").focus();
					  // $("#"+rowId+"_NgayGioDo").val(new Date().toString("H:mm dd"+$.cookie('phancachngay')+"MM"+$.cookie('phancachngay')+"yy"));					  						
					   ...
					   $("#"+rowId+"_NgayGioDo").keydown(function(e){ 
   							//e.preventDefault();	//mac dinh ngaygiodo
					   });					 	
					   ...
					   number_textbox_demical("#"+rowId+"_Mach");
					    number_textbox_demical("#"+rowId+"_NhipTho");	
					   	
					  // $("#"+rowId+"_NgayGioDo").select();					  						   		 
					},	
...
function create_thetrang_grid(){
	 ...
	 $("#rowed6").jqGrid({
		...
		,colModel:[	
			...
			{name:'ChieuCao',index:'ChieuCao',width:"40%", align:'right',hidden:false, editable: true,cellattr: tomaudo,editrules:{required:true},editoptions: {defaultValue: '0'}}, 
			{name:'CanNang',index:'CanNang', width:"40%",align:'right',hidden:false, editable: true,cellattr: tomaudo,editrules:{required:true},editoptions: {defaultValue: '0'}},
			{name:'VongNguc',index:'VongNguc', width:"40%",align:'right', editable: true,cellattr: tomaudo,editrules:{required:false},editoptions: {defaultValue: '0'}},				                             
		],
		grid_save_option: "",
		loadonce: true,
		scroll: false,		 
		...
		caption: "Đo thể trạng <span></span>",
               ...
               // mtype: 'post',				 			
                addRowParams: {
					...
					oneditfunc: function(rowId) {						
					   ...
					   var tmp1 = $("#rowed6").jqGrid('getDataIDs');	
					   var tmp2 = $("#rowed7").jqGrid('getDataIDs');			    
					   if(tmp1.length>1){						  
						    var rowData2 = $("#rowed6").getRowData(tmp1[tmp1.length-2]);
							$("#"+rowId+"_ChieuCao").val(rowData2["ChieuCao"]);
						   $("#"+rowId+"_CanNang").val(rowData2["CanNang"]);
						   $("#"+rowId+"_VongNguc").val(rowData2["VongNguc"]);						
					   }else{						    
						   // var rowData2 = $("#rowed6").getRowData(tmp1[0]);	
					   }
...
function resize_control(){	 
	right_height=$(window).height()/2-113;	
	$("#rowed3").setGridWidth($(".ui-layout-west").width()-10);
	$("#rowed3").setGridHeight($(window).height()-190);
	$("#rowed4,#rowed6").setGridWidth($(".ui-layout-center").width()-11);
	$("#rowed4,#rowed6").setGridHeight(right_height);  
}
 ...
 function create_khoa(elem, datalocal) {
        ...
        $(elem).jqGrid({
            ...
            onSelectRow: function(id) {
				window.idkhoa=id;
				setTimeout(function(){
					$("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data&idkhoa="+window.idkhoa}).trigger('reloadGrid');
				},500);
