 ...
 <div class="top_form ui-widget-content" >
    ...
    <div class="thongtin_luotkham" id="right_col" style="width: 656px!important">
	    	...
	    	<div style="margin-top: 12px;">
			  <a href="#" id="camket" class="ui-widget ui-state-default ui-corner-all ui-button-icon-left" style="float: right; position: absolute; margin-left: 291px; width: 88px; height: 22px; padding-left:30px; text-decoration:none; padding-top: 5px;">Cam kết<span class="ui-icon ui-icon-circle-arrow-s" style="margin-left: -29px; margin-top: -17px;"></span></a> 
...
$.cookie("in_status", "print_preview"); 
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=FamlilyPlanning&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10",'FamlilyPlanning');
		$(".frame_u78787878975f").css("width","793px");
	}); 
$("#camket").click(function(e){
$.cookie("in_status", "print_preview"); 
dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=camketkhhhgiadinh&header=left&id_benhnhan="+id_benhnhan+"&type=report&id_form=10",'FamlilyPlanning');
...
$('#loikhuyen').val('');
	...
	$( "#ttrinh" ).keydown(function(e){
		$("#ttrinh").data('focus', true);
			if (jwerty.is("enter",e)||jwerty.is("tab",e)) {
	...
	$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_khhgd&idbenhnhan="+id_benhnhan, 
		function( data ) {
			data_luotkham=data;
...
function create_chuandoan(elem, datalocal) {
        ...
        $(elem).jqGrid({
            ...
            loadComplete: function(data) {
                ...
                grid_filter_enter(elem);   
				if(window.nhanvien3_complete==1){
...
function load_complete1(){
	...
	if(window.test2=="dathuchien1"){
			       $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luuthuchien&hienmaloi=3',datalocalToSend)
			         ...
			         if(window.test2=="hoantat1"){
			         	$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luuhoantat&hienmaloi=3',datalocalToSend)
								 .done(function( gridData ) {	
								 	delete window.datalocalToSend; 
								 							tooltip_message("Đã lưu");
			                                            testa();
			       ...
			       $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luudangkham&hienmaloi=3',datalocalToSend)
								 .done(function( gridData ) {	
								 	delete window.datalocalToSend;
								 	tooltip_message("Đã lưu");
			                                            testa();	 
