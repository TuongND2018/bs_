...
$store_name="{call GD2_DM_SelectLoaiKhamByID_NhomCLSAndID_LuotKham_New(?,?)}";//tao bien khai bao store
...
foreach ($tam as $row) {//duyet toan bo mang tra ve
    ...
    $responce->rows[$i]['cell']=array($row["GiaThueNgoaiThucHien"],$row["ThoiGianTrungBinhThucHien"],"",$row["ID_LoaiKham"],$row["TenLoaiKham"],$row["GiaBaoChoBN"],$row["Gia"],$row["GiaBaoHiem"],$row["GiaPhuThuBenhVien"],$row["PhuThuThucHienTaiNha"],$row["HeSoNuocNgoai"],$baohiemchitra,$row["Color"],$row["ID_Nhom_BHYT"],$row["PhanTramCungChiTra"]);
    $i++; 
}
