...
var fromday, today;
	...
	$("#xem").click(function(e) {
		...
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_sinhton_lichsu&id_benhnhan='+id_benhnhan+"&tungay="+$("#from_day_mask").val()+"&denngay="+$("#to_day_mask").val()
		,loadonce: false
		,datatype: "json"}).trigger("reloadGrid");
	$("#rowed7").jqGrid().setGridParam({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_thetrang_lichsu&id_benhnhan='+id_benhnhan+"&tungay="+$("#from_day_mask").val()+"&denngay="+$("#to_day_mask").val()
		,loadonce: false
		,datatype: "json"}).trigger("reloadGrid");	 
...
function create_lichsu_sinhton_grid(){
	 ...
	 $("#rowed5").jqGrid({
		data:mydata,
		datatype: "local",	
		colNames:["Ngày giờ đo", "Ps", "Pd", "Hr", "Temp","ngaygroup"],
		colModel:[
...
function create_lichsuthetrang_grid(){
	 ...
	 $("#rowed7").jqGrid({
		data:mydata,
		datatype: "local",	
		colNames:["Ngày giờ đo", "Chiều cao (cm)", "Cân nặng (kg)", "Vòng ngực (cm)","ngaygroup"]
		,colModel:[			
            {name:'NgayGioDo',index:'NgayGioDo',width:"25%",align:'left'},		   	 	 
