...
<div id="dialog_sua" title="Sửa" style="display:none">
 <iframe id="i_sua" src="" width="100%" height="100%"></iframe>
    ...
    <div id="n_bottom">
        ...
        <button id="timkiem" style="display:none">Tìm kiếm</button>
        <button id="inphieuhoantra">In phiếu hoàn trả</button>
        <button id="inphieutonghopylenh">In phiếu tổng hợp y lệnh</button>
    </div>
</body>
<script type="text/javascript">
var report_code=["PhieuLinhThuoc"];
var report_name=["Phiếu trả thuốc"];
$(document).ready(function(e) {
	openform_shortcutkey();
	window.n_sua=false;
	parent.postMessage('changetitle;<?=$view?>-;Phiếu hoàn trả thuốc - <?=$_GET['tenkhoa']?>;', '*');
	...
	if(tam=='dongdialog'){
		...
		$("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_danhsach_phieuhoantrathuoc&idkhoa=<?=$_GET['idkhoa']?>&tungay="+$("#tungay").val()+"&denngay="+$("#denngay").val()}).trigger('reloadGrid');
	}
	...
	$("#inphieuhoantra").click(function(e) {
        $.cookie("in_status", "print_preview");
		var rowData = jQuery('#rowed3').jqGrid ('getRowData',window.idphieuhoantra);
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=phieutrathuoc_vattuyte&idphieuhoantra="+window.idphieuhoantra+"&idphanloai="+window.idphanloai+"&thuocthongthuong="+rowData['ThuocThongThuong']+"&vattuyte="+rowData['VatTuYTe']+"&thuochuongthan="+rowData['ThuocHuongThan']+"&type=report&id_form=10",'PhieuLinhThuoc');
    });
	
	
	$("#themmoi,#sua,#timkiem,#inphieuhoantra,#inphieutonghopylenh,#xem").button();
...
function create_grid() {
        $("#rowed3").jqGrid({
             url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_danhsach_phieuhoantrathuoc&idkhoa=<?=$_GET['idkhoa']?>&tungay='+$('#tungay').val()+'&denngay='+$('#denngay').val(),
   			datatype: "json",
            colNames: ['Mã phiếu','Người tạo','Ngày tạo', 'Người trả','Ngày trả', 'Mã khoa', 'Tên khoa', 'Mã đối tượng', 'Đã trả','','',''],
            colModel: [
				...
				{name: 'NguoiTra', index: 'NguoiTra', search: true, width: "10%", editable: false, align: 'center', hidden: false},
				{name: 'NgayTra', index: 'NgayTra', search: true, width: "10%", editable: false, align: 'center', hidden: false},
				...
				{name:'DaTra',index:'DaTra', width:"5%", editable:true,stype: 'text',search:false,searchoptions: { sopt: ['eq', 'ne'],value:':;1:Có;0:Không'}, editable:true,align:'center',hidden:false,edittype:"checkbox",formatter:"checkbox",formatoptions:{disabled: true},formoptions:{ rowpos:13, colpos:1}
...
/*                window.selectedVal = "";
           ...
           loadonce: true,
			...
			$("#inphieuhoantra").button('enable');
			window.idphieuhoantra=id;
			$("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsach_phieuhoantrathuoc_chitiet&idphieuhoantrathuoc="+window.idphieuhoantra+"&idphanloai="+window.idphanloai}).trigger('reloadGrid');
            },
				...
				ids = $('#rowed3').jqGrid('getDataIDs');	
					if(window.n_sua==true){
						...
						for(var i=0;i<ids.length;i++){
							//alert(ids[i]+'=='+window.idphieuhoantra)
							if(ids[i]==window.idphieuhoantra){
...
function create_grid2() {
      mydata=[];
        $("#rowed4").jqGrid({
            data: mydata,
            datatype: "local",
            colNames: ['ID đơn thuốc trả lại','Ngày tạo', 'Họ tên bệnh nhân', 'Tuổi', 'Địa chỉ', 'Số buồng', 'Số giường'],
            colModel: [
                {name: 'ID_DonThuocTralai', index: 'ID_DonThuocTralai', search: true, width: "12%", editable: false, align: 'left', hidden: false,classes:'r3_mabenhnhan'},
           ...
           loadonce: true,
            ...
            onSelectRow: function(id) {
				...
				$("#rowed5").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_chitietthuocbyid_donthuoctralai&iddonthuoctralai="+id+'&nhomphanloai='+$("#phanloai_hidden").val()}).trigger('reloadGrid');
...
function them(){
	links='resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=formthemmoi&type=test&id_form=<?=$_GET["id_form"]?>&tenkhoa=<?=$_GET["tenkhoa"]?>&idkhoa=<?=$_GET["idkhoa"]?>&ac=add';
...
function sua(){
	...
	links='resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=formsua&type=test&id_form=<?=$_GET["id_form"]?>&tenkhoa=<?=$_GET["tenkhoa"]?>&idkhoa=<?=$_GET["idkhoa"]?>&idphieuhoantra='+window.idphieuhoantra+'&ac=edit';
...
function create_phanloai(elem, datalocal) {
        ...
        $(elem).jqGrid({
			...
			//setTimeout(function(){
			//$("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsach_phieulinhthuoc_chitiet&idphieulinhthuoc="+window.idphieuhoantra+"&idphanloai="+window.idphanloai}).trigger('reloadGrid');
			//},100);
            },
...
function resize_control(){
	...
	$("#rowed4").setGridHeight($(".n_duoi").height()-60);
