...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_quanhuyen',
		datatype: "json",	
				...
				beforeShowForm: function(formid) {					 
			 	...
			 	afterShowForm : function (formid) {					
					...
					$("#add_new").click(function(e){
					   $.post('resource.php?module=web_services&function=get_link&action=index&folder=danh_muc_tinhthanhpho:').done(function(data) {
...
function load_select(){
	window.province = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_TinhThanhPho&id=ID_ThanhPho&name=TenTinhThanhPho', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban');}}).responseText;	
}

function load_select1(){
	window.province1 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_TinhThanhPho&id=ID_ThanhPho&name=TenTinhThanhPho', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục quốc tịch');}}).responseText;	
