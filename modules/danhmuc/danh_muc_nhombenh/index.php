...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_nhombenh&q=2',
		datatype: "json",	
	...
	$("#rowed3").jqGrid('navGrid',"#prowed3",{add: permission["add"], edit: permission["edit"], del: permission["delete"], search: permission["search"],closeAfterEdit: true,clearAfterAdd :true,closeOnEscape : true,}, //options						 
			   ...
			   afterSubmit : function(response, postdata) { 
				...
				afterShowForm : function (formid) {
					...
					lendong(formid);					
						...
						links="resource.php?module=danhmuc&view=danh_muc_nhombenh&id_form=22"
		...
		{reloadAfterSubmit:true,height:250,width:600,sopt: ["cn"],url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_nhombenh&q=2',navkeys : [ true, 38, 40 ],closeOnEscape : true,
...
function load_phong_ban(status){
	window.ID_NhomBenhCha = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=DM_nhombenh&id=ID_NhomBenh&name=TenNhomBenh', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục nhóm thuốc ');}}).responseText;
