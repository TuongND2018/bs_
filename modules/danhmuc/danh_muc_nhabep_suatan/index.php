...
//=========================
	var fromday, today;
	 ...
	 $("#xuatexcel").click(function(e) {
		...
		window.open("resource.php?module=report&view=danhmuc&action=excel_nhabepsuatan&type=excel&tu_ngay="+ $( '#from_day' ).val()+"&den_ngay="+$( '#to_day' ).val());
...
//========================================
	...
	$("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_nhabepsuasuatan&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()}).trigger('reloadGrid');
        ...
        function load_select() {
           ...
           window.trangThai = 'Đã đặt:Đã đặt;Yêu cầu sửa:Yêu cầu sửa;Yêu cầu hủy:Yêu cầu hủy;Xong:Xong;Hủy bỏ:Hủy bỏ';
		   window.hLe = 'Hợp lệ:Hợp lệ;Không hợp lệ:Không hợp lệ';
        ...
        function create_grid_suasuatan_nhabep() {
            ...
            $("#rowed3").jqGrid({
                ...
                colNames: ['IndexID', 'Ngày', 'Khoa', 'Người chịu tiền', 'Tên thực đơn', 'Số lượng', 'Đơn giá',
                    'Thành tiền', 'Trạng thái','Người đặt','Người sửa','Ngày giờ sửa','Người hủy','Ngày giờ hủy','','Tầng','Hợp lệ'],
                colModel: [
                    ...
                    {name: 'SoLuong', index: 'SoLuong',summaryType: 'sum' ,formatter:'number', formatoptions:{decimalPlaces: 0},search: false, width: "5%",  align: 'right', hidden: false},
				...
				,sorttype:'currency',formatter:'currency'
					...
					{name: 'NguoiSua', index: 'TrangThai', search: false, width: "5%", editable: false, align: 'left', hidden: false},
                  	{name: 'Ngaysua', index: 'Ngaysua', search: false, width: "10%",  editable: false, align: 'left', hidden: false},
					{name: 'NguoiHuy', index: 'NguoiHuy', search: false, width: "5%", editable: false, align: 'left', hidden: false},
					{name: 'NgayHuy', index: 'NgayHuy', search: false, width: "10%",  editable: false, align: 'left', hidden: false},
					{name: 'Buoi', index: 'Buoi', search: false, width: "0%", editable: false, align: 'left', hidden: true},
					{name: 'Tang', index: 'Tang', search: true, width: "5%", editable: false, align: 'left', hidden: false},
					{name: 'HopLe', index: 'HopLe', search: true,stype: 'select', searchoptions: {sopt: ['eq', 'ne'], value: hLeZ}, width: "5%", formatter: "select", editable: false, edittype: "select", editoptions: {value: hLeZ}, align: 'left', hidden: false},
                ...
                groupingView: {groupField: ['TenPhongBan', 'NguoiChiuTien'],
                    ...
                    groupText: [' <b >{0}</b> có  <b style="color: orangered">{SoLuong}</b>  thực đơn', '<b style="color: blue">{0}</b> có <b style="color: orangered"> {SoLuong}</b> thực đơn'],
                    groupCollapse: false,
            ...
            //tô màu
                    ...
                    for (i = 0; i < ids.length; i++) {
						...
						window.slt=parseInt(rowData['SoLuong']);
						if(rowData['TrangThai']=='Hủy bỏ'){
							window.hb=window.hb+window.slt;
							$("#rowed3").setCell(ids[i], 'TrangThai','', {background:'#CCCCCC'}); 
						}else if(rowData['TrangThai']=='Đã đặt'){
							
							window.dc=window.dc+window.slt;
							$("#rowed3").setCell(ids[i], 'TrangThai','', {background: '#d9f970'});
						}else if(rowData['TrangThai']=='Yêu cầu sửa'){
							window.ycs=window.ycs+window.slt;
							...
							window.ych=window.ych+window.slt;
							$("#rowed3").setCell(ids[i], 'TrangThai','', {background: '#FF9900'}); 
							$("#rowed3").setCell(ids[i], "TrangThai", '', "nhapnhay"); 
						}else{
							window.xg=window.xg+window.slt;
                ...
                caption: "Suất ăn từ <label id='fromday_lb' > </label> đến <label id='today_lb' > </label> Có <label id='suatxongl' style='color: red' > </label> suất ăn xong, <label id='dangchol' style='color: blue'> </label> đã đặt, <label id='dahuyl' style='color: blue'> </label> hủy, TỔNG CỘNG <label id='tongsa' style='color: blue'> </label> suất ăn"+"------ <label for='checkbox'style='width:110px' >Thu gọn</label><input type='checkbox' id='checkp'>---------------------- ",
