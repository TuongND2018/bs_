...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_kho_thietbi_vattu&idnew=0',
		datatype: "json",	
		colNames:['Tên kho','STT','Ghi chú','Sử dụng',''],
		colModel:[
			{name:'TenKho',index:'TenKho',search:true, width:"20%", editable:true,align:'left',hidden:false,editrules: {required:true},formoptions:{ rowpos:1, colpos:1}}, 
			{name:'STT',index:'STT', width:"4%", editable:true,align:'left',hidden:false,edittype:"text",formoptions:{ rowpos:2, colpos:1}}, 
			{name:'GhiChu',index:'GhiChu', width:"20%", editable:true,align:'left',hidden:false,editrules: {required:true},edittype:"text",formoptions:{ rowpos:3, colpos:1}},
			{name:'SuDung',index:'SuDung', width:"4%", editable:true,align:'center',hidden:false,edittype:"checkbox",formatter:"checkbox",formoptions:{ rowpos:4, colpos:1},editoptions: {value:"1:0"},stype: 'select',searchoptions: { sopt: ['eq', 'ne'],value:':Tất cả;1:Có;0:Không'}},		
	...
	//jsonReader:{repeatitems:true,subgrid:{repeatitems:false}},	
		...
		caption: "Danh mục Kho Thiết bị - Vật tư"
	});	
		...
		{recreateForm:true,height:'auto',width:'auto',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=edit',closeOnEscape : true,modal: true,recreateForm: true,
			   afterSubmit : function(response, postdata) { 
				  	...
				  	if(temp[0]==1){
						...
						$("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_kho_thietbi_vattu&idnew="+temp[1]}).trigger('reloadGrid');							
					};						
		...
		beforeShowForm : function(formid) {canhgiua_del(formid);},	
				afterSubmit : function(response, postdata) { 				
							if(response.responseText==1){
								...
								$("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_kho_thietbi_vattu&idnew=0"}).trigger('reloadGrid');
															
							};						
							return [success,new_id] ;
				}		
		}, // del options
		{reloadAfterSubmit:true,height:250,width:600,sopt: ["cn"],url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_kho_thietbi_vattu&idnew=0',closeOnEscape : true,
