...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_nghenghiep',
		datatype: "json",	
		colNames:['Id','Tên món ăn','Mô tả','Giá','Sử dụng'],
		colModel:[
			{name:'ID_MonAn',index:'ID_MonAn',search:false, width:"2%", editable:false,align:'right',hidden:true}, 
			{name:'TenMonAn',index:'TenMonAn', width:"25%", editable:true,align:'left',hidden:false,editrules: {required:true},edittype:"text",formoptions:{ rowpos:1, colpos:1}}, 
			{name:'MoTa',index:'MoTa', width:"60%", editable:true,align:'left',hidden:false,edittype:"textarea",editrules: {required:false},formoptions:{ rowpos:2, colpos:1}},	 
			//{name:'Gia',index:'Gia', width:"10%", editable:true,align:'right',hidden:false,edittype:"text",formoptions:{ rowpos:3, colpos:1}},	
			{name:'Giaban',formatter:'number', index:'Giaban',width:"10%",align:'right',editrules: {required:false},hidden:false, editable: true}, 
			{name:'Active',index:'Active', width:"5%", editable:true,align:'center',hidden:false,edittype:"checkbox",formatter:"checkbox",formoptions:{ rowpos:4, colpos:1},editoptions: {value:"1:0", defaultValue: "1"}}
		],
	//
     ...
     bCancel: "Cancel"}, //options						 
		...
		{height:'auto',width:'auto',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=add',closeOnEscape : true,closeAfterAdd:false,modal: true,addedrow: "first",recreateForm:true,savekey: [true,121]
				,afterSubmit : function(response, postdata){
					...
					if(temp[0]==0){
						var success=false;
						tooltip_message("Món ăn này đã có");													
					}	 
					if(temp[0]==1){
						var success=false;
