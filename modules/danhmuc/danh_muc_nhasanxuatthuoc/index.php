...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_nhasanxuatthuoc',
		datatype: "json",	
		colNames:['Id','Tên nhà sản xuất','Ghi chú','Sử dụng'],
		colModel:[
			{name:'ID_NSXThuoc',index:'ID_NSXThuoc',search:false, width:"2%", editable:false,align:'right',hidden:true}, 
			{name:'TenNhaSanXuat',index:'TenNhaSanXuat', width:"15%", editable:true,align:'letf',hidden:false,editrules: {required:true},edittype:"text",formoptions:{ rowpos:1, colpos:1}}, 
	...
	//
		...
		caption: "Danh mục nhà sản xuất thuốc"
	});	
		...
		{recreateForm:true,height:'auto',width:'auto',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=edit',closeOnEscape : true,modal: true,recreateForm: true,
			   afterSubmit : function(response, postdata) { 
				...
				afterShowForm : function (formid) {
					autocompleted_combobox("#ID_NSXThuoc");	
					xuongdong(formid);
					lendong(formid);	
				},
