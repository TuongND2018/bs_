...
if($_GET['donthuocchon']){
	$_luu=$_GET['donthuocchon'];
	$_luu1=explode("_",$_luu);
	$_dem=count($_luu1);
	for($ii=0;$ii<$_dem;$ii++){
		if($_luu1[$ii]>0){
			$check3='';
...
if($_GET['donthuochuy']){
	$_huy=$_GET['donthuochuy'];
	$_huy1=explode("_",$_huy);
	$_dem2=count($_huy1);
	for($ii=0;$ii<$_dem2;$ii++){
		if($_huy1[$ii]>0){
			$params22 = array ($_huy1[$ii],$_SESSION["user"]["id_user"]);
			$store_name22="{call GD2_LoaiKham_DonThuoc_VTTH_Del_New(?,?)}";
			$del=$data->query( $store_name22, $params22);
		}
	}
}
