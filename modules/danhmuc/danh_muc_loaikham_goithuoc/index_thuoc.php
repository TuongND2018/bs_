...
function create_detail_grid(){
	$("#rowed4").jqGrid({
			...
			datatype: "json",	
			colNames:['','','ID','Xóa','Tên thuốc','Liều dùng','Đơn giá','Thành tiền','Cách dùng'],
			colModel:[
			...
			{name:'LieuDungHangNgay',index:'LieuDungHangNgay',search:false, width:"10%",formatter:'integer',formatoptions:{defaultValue: '0'}, editable:true,align:'center',hidden:false,edittype:"text",editoptions: { dataEvents:  [{ type: 'keyup', fn: function(e) { 
				var sl_chon=$('#'+$("#rowed4").jqGrid('getGridParam', 'selrow')+'_LieuDungHangNgay').val();
				var rowData = jQuery('#rowed4').jqGrid ('getRowData',$("#rowed4").jqGrid('getGridParam', 'selrow'));
				if(sl_chon>0){
					sl_chon=sl_chon;	
				}else{
					sl_chon=0;	
				}
				$('#rowed4').jqGrid("setCell", $("#rowed4").jqGrid('getGridParam', 'selrow'), "ThanhTien",sl_chon*rowData['DonGia']);	
				var  Sum_DonGia = $("#rowed4").jqGrid('getCol', 'DonGia', false, 'sum');
				var  Sum_ThanhTien = $("#rowed4").jqGrid('getCol', 'ThanhTien', false, 'sum');
				$("#rowed4").jqGrid('footerData','set', {
					ID_Thuoc: 'Tổng tiền:', 
					DonGia: Sum_DonGia,
					ThanhTien: Sum_ThanhTien,
				});
                 } }]}},
			{name:'DonGia',index:'DonGia',search:false, width:"10%",formatoptions:{defaultValue: '0'},formatter:'integer', editable:false,align:'right',hidden:false,edittype:"text"},
			{name:'ThanhTien',index:'ThanhTien',search:false, width:"10%",formatoptions:{defaultValue: '0'},formatter:'integer', editable:false,align:'right',hidden:false,edittype:"text"},
...
function create_Dm_thuoc_grid(elem,datalocal){	
	...
	onSelectRow: function(id){	
		var id_select = $("#rowed4").jqGrid('getGridParam', 'selrow');
		var rowData = $(elem).getRowData(id);
		var rowData2 = $("#rowed4").getRowData(id_select);
		var sl_chon=$('#'+id_select+'_LieuDungHangNgay').val();
		if(sl_chon>0){
			sl_chon=sl_chon;	
		}else{
			sl_chon=0;	
		}
		$('#rowed4').jqGrid("setCell", id_select, "DonGia",rowData['Giaban']);
		$('#rowed4').jqGrid("setCell", id_select, "ThanhTien",sl_chon*rowData['Giaban']);						
		var  Sum_DonGia = $("#rowed4").jqGrid('getCol', 'DonGia', false, 'sum');
		var  Sum_ThanhTien = $("#rowed4").jqGrid('getCol', 'ThanhTien', false, 'sum');
		$("#rowed4").jqGrid('footerData','set', {
			ID_Thuoc: 'Tổng tiền:', 
...
function resize_control() {
	...
	$("#rowed4").setGridHeight($('.right_col').height()-105);	
}
</script>
