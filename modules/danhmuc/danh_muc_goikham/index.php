...
<div id="dialog-change"  title="Thông báo!"> Dữ liệu đã được thay đổi, bạn có muốn lưu lại không?
</div>
<div id="dialog-refresh"  title="Thông báo!"> Dữ liệu đã được thay đổi, bạn có muốn làm mới không?
</div>
<div id="dialog-form1"  title="Thông báo!">Loại khám này đã tồn tại. Xin vui lòng nhập tên khác!
</div>
<div id="dialog-form" title="Gói khám chi tiết"  >
 <div id="container">
	<table id="aa">
		<tr>
			<td>
				...
				<fieldset  style="height:120px">
					...
					<div class="form_row" style="vertical-align:top;display:inline-block"  >  <label for="tengoikham">Tên gói khám:*</label> 	<input style="width:380px" type="text" name="TenGoiKham" id="TenGoiKham" ></div>
				
				<div class="form_row" style="vertical-align:top;display:block;"  > 
					<div class="form_row" style="vertical-align:top;display:inline-block;text-align:left" >  <label for="sotiendukien">Số tiền dự kiến:</label>	<input style="width:150px" type="text" name="SoTienDuKien" id="SoTienDuKien" disabled></div>
				...
				<div class="form_row" style="vertical-align:top;display:block;"  > 
					<div class="form_row" style="vertical-align:top;display:block;text-align:left" >  <label for="ghichu" style="vertical-align:top">Ghi chú:</label>	<textarea rows="2" cols="23" name="GhiChu" id="GhiChu"></textarea></div>
				</div>
			...
			<td>
				...
				<form>
				<fieldset style="width:105%;height:120px">
		 ...
		 $("#com_thuocloaikham").css("background-color","#F4FA58");
			...
			//----------------
				...
				for(i=0;i<=tmp1.length;i++){
					...
					//alert("ten loai kham da co trong grid"+ rowData["TenLoaiKham"]);
					//alert(rowData["TenLoaiKham"]);
				...
				//var rowData = jQuery('#data_lk').getRowData(rowId);
				var id = $("#data_lk").jqGrid('getGridParam','selrow');
				var rowData = jQuery("#data_lk").getRowData(id);
				
				parameters =
				   {
					initdata : {ID_GoiKhamChiTiet:$("#com_thuocloaikham1").val(),TenLoaiKham:rowData["TenLoaiKham"],GiaBaoChoBN:rowData["GiaBaoChoBN"]},
					position :"last",
					useDefValues : false,
					useFormatter : false,
				...
				$("#SoTienDuKien").val(parseInt($("#SoTienDuKien").val()) + parseInt( rowData["GiaBaoChoBN"]))  ;
                    ...
                    $("#rowed4").jqGrid("footerData", "set", {TenLoaiKham: "Tổng tiền:", GiaBaoChoBN: sum});
			}  
 ...
 $( "#dialog-change" ).dialog({
  ...
  $( "#dialog-refresh" ).dialog({
	...
	jQuery(window).resize(function() {		 
	 ...
	 $("#rowed3").setGridHeight($(window).height()-$("#form_danh_muc_goikham").height()-150);
	 var height = $( "#dialog-form" ).dialog( "option", "height" );
	 var width = $( "#dialog-form" ).dialog( "option", "width" );

	 $("#rowed4").setGridWidth(width-100);
	 $("#rowed4").setGridHeight(height-$("#container").height()-310);
	});
	create_grid();	
	shortcut_key();		
	create_grid1();	
...
function create_grid(){	
	 $("#rowed3").jqGrid({
		...
		colModel:[
			...
			{name:'GhiChu',index:'GhiChu',search:true, width:"20%", editable:false,align:'center',hidden:false},	 
     ...
     bCancel: "Cancel",addfunc: function() { 
		...
		editfunc: function(id) { 
		 ...
		 window.id_edit=id;
				...
				$("#rowed4").jqGrid("setGridParam", {datatype: "json",url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_goikhamchitiet&id='+id}).trigger("reloadGrid");
		...
		{reloadAfterSubmit:true,height:250,width:600,sopt: ["cn"],url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_goikham&q=2',closeOnEscape : true,
  ...
  myDelOptions = {
        ...
        processing:true
			 ...
			 $("#rowed4").jqGrid({
				data:mydata,
				datatype: "local",	
				colNames:['Id','Loại khám','Phí thực hiện'],
				colModel:[
					{name:'ID_GoiKhamChiTiet',index:'ID_GoiKhamChiTiet',search:false, width:"2%", editable:false,align:'right',hidden:true}, 
					{name:'TenLoaiKham',index:'TenLoaiKham', width:"5%", editable:false,align:'letf',hidden:false,edittype:"text"}, 
					{name:'GiaBaoChoBN',index:'GiaBaoChoBN', width:"5%", editable:false,align:'right',edittype:"text",hidden:false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},	  
...
function load_select1(){
	window.nhomcls = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=NhomCLS&id=ID_NhomCLS&name=TenNhom', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban');}}).responseText;	
	...
	/*	  var data2 = new Object();
		  ...
		  var data1['rows'] = [];
		  data2= window.data_nhom2;
		alertObject(data2);
		i=0;
		
		 for (var key in data2['rows']) {	
		 //console.log(key)	 
			if((data2['rows'][key]['cell'][2])==id ){
				
				data1['rows'][i]=data2['rows'][key];
				i++;
			}
		 }*/
		put_Search_grid("#data_lk",2,id,window.data_nhom2,'#com_thuocloaikham','#com_thuocloaikham')
	...
	function create_bs2(elem,datalocal){ 
	  ...
	  //alert(selr);
		 // $("#rowed4").jqGrid('setGridParam',{url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_ds_xephang_dangcho_group&id="+selr}).trigger('reloadGrid');
