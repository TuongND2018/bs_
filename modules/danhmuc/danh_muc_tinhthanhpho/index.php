...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_tinhthanhpho',
		datatype: "json",	
		colNames:['Id','Tên tỉnh thành phố','Quốc gia','Ghi chú','Sử dụng','Độ ưu tiên','Mặc định','Mã tĩnh TP'],
		colModel:[
			{name:'ID_ThanhPho',index:'ID_ThanhPho',search:false, width:"2%", editable:false,align:'right',hidden:true}, 
			{name:'TenTinhThanhPho',index:'TenTinhThanhPho', width:"15%", editable:true,align:'center',hidden:false,editrules: {required:true},edittype:"text",formoptions:{ rowpos:1, colpos:1}}, 
			{name:'ID_QuocTich',index:'ID_QuocTich', width:"15%", editable:true,align:'center',hidden:false,formatter:"select",editrules: {required:true},edittype:"select",stype: 'select', searchoptions:{sopt: ['eq', 'ne'],value:nationz},editoptions: { value: nation},formoptions:{ rowpos:2, colpos:1, elmsuffix:'<a id="add_new" class="fm-button ui-state-default ui-corner-all " style="margin-left:30px!important;vertical-align:top;width:16px;height:13px;"  ><span style="margin-top:-2px;margin-right:-22px!important" class="ui-icon ui-icon-circle-plus"></span></a>'}}, 
			{name:'GhiChu',index:'GhiChu', width:"10%", editable:true,align:'left',hidden:false,edittype:"textarea",formoptions:{ rowpos:3, colpos:1}},	 
			...
			{name:'MacDinh',index:'MacDinh', width:"4%", editable:true,align:'center',hidden:false,edittype:"checkbox",formatter:"checkbox",formoptions:{ rowpos:6, colpos:1},editoptions: {value:"1:0"}},	
			{name:'MaTinhThanhPho',index:'MaTinhThanhPho', width:"4%", editable:true,align:'center',hidden:false,},	
		],
	//
	...
	//jsonReader:{repeatitems:true,subgrid:{repeatitems:false}},	
		...
		caption: "Danh mục tỉnh thành phố"
	});	
		...
		{recreateForm:true,height:'auto',width:'auto',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=edit',closeOnEscape : true,modal: true,recreateForm: true,
			   afterSubmit : function(response, postdata) { 
				...
				afterShowForm : function (formid) {
					...
					$("#tr_MoTa").html(congty);*/						
					autocompleted_combobox("#ID_QuocTich");
					xuongdong(formid);
					lendong(formid);					
					$("#add_new").click(function(e){
					   $.post('resource.php?module=web_services&function=get_link&action=index&folder=danh_muc_quoctich:').done(function(data) {
			 	...
			 	afterShowForm : function (formid) {					
					...
					lendong(formid);			 
					 $("#add_new").click(function(e){
					  ...
					  links='resource.php?module=danhmuc&view=danh_muc_quoctich&id_form=47&id_loai=undefined';
					  elem=1 + Math.floor(Math.random() * 1000000000); 
					  width=90;
					  height=90;
					  dialog_add_dm("Danh mục quốc tịch",width,height,elem,links,load_select1);     
					 }) 
			 	},
				onClose : function(formid){
					$("#editmodrowed3").css("box-shadow","");					
...
function load_select1(){
	window.nation1 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_QuocTich&id=ID_QuocTich&name=TenQuocTich', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục quốc tịch');}}).responseText;	
	...
	if(last1[0]==last[0]){		
		...
		$("#rowed3").setColProp('ID_QuocTich', { editoptions: { value: nation1} });
		$('#ID_QuanHuyen').empty();
		create_select("#ID_QuocTich",nation1);
		if(last1>last){
		$('#ID_QuocTich1').val(last1[1]);
		$('#ID_QuocTich').val(last1[0]);
...
function load_select(){
	window.nation = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_QuocTich&id=ID_QuocTich&name=TenQuocTich', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục quốc tịch');}}).responseText;	
}
