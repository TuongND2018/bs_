...
<script type="text/javascript">
	...
	$("#luu").click(function(){
      ...
      //  $("#luu").html("Đang lưu ...<span class='ui-icon ui-icon-plusthick'></span>");
        $("#luu").button('disable');
	  ...
	  // ids = $('#rowed3').jqGrid('getDataIDs');
	   id = $('#rowed3').jqGrid('getGridParam', 'selrow');
		 $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller',postData).done(function(data){
		$("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaikham&idphongban="+id}).trigger('reloadGrid');
		tooltip_message("Lưu dữ liệu thành công");
        //$("#luu").html("Lưu [F10]<span class='ui-icon ui-icon-plusthick'></span>");
        $("#luu").button('enable');
    ...
    function create_grid() {
        $("#rowed3").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_nhom',
            datatype: "json",
            colNames: ['ID_NhomTemplate', 'Tên nhóm'],
            colModel: [
                {name: 'ID_NhomTemplate', index: 'ID_NhomTemplate', width: "0%", editable: false, align: 'left', hidden: true},
                {name: 'TenNhom', index: 'TenNhom', search: true, width: "80%", editable: false, align: 'left', hidden: false},
            ],
            loadonce: true,
            scroll: false,
            modal: true,
			...
			var rowData = jQuery(this).getRowData(id); 			
			 $("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaikham&idphongban="+id,groupingView: { groupCollapse : true}}).trigger('reloadGrid');
			  window.tennhom = rowData['TenNhom'];
			...
			ids = $('#rowed3').jqGrid('getDataIDs');
			 $("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaikham&idphongban="+ids[0],groupingView: { groupCollapse : true}}).trigger('reloadGrid');
			//$("#"+ids[1]).focus();
			jQuery('#rowed3').jqGrid("setSelection",ids[0], true);	
			jQuery('#'+ids[0]).focus();
			
            },
			caption: "Danh sách nhóm mẫu chuẩn đoán"
        });
    ...
    function create_grid2()
    {
        ...
        $("#rowed4").jqGrid({ 
            data: mydata,
            datatype: "local",
            colNames: ['Nhom_CLS','ID_LoaiKham_NhomTemplate', 'ID nhom', 'Luu', 'events', 'IDNhomTemPlate','ID Nhóm CLS','ID loại khám', 'Tên hạng mục khám', 'Mã viết tắt', 'STT', 'count'],
            colModel: [
                {name: 'Nhom_CLS', index: 'Nhom_CLS', summaryType: 'sum', search: true, width: "5%", editable: false, align: 'center', hidden: true},
				{name: 'ID_LoaiKham_NhomTemplate', index: 'ID_LoaiKham_NhomTemplate', search: true, width: "5%", editable: false, align: 'center', hidden: true},
				{name: 'idnhom', index: 'idnhom', search: true, width: "5%", editable: false, align: 'center', hidden: true},
				...
				{name: 'IDNhomTemPlate', index: 'IDNhomTemPlate', search: true, width: "5%", editable: false, align: 'center', hidden: true},
				
                {name: 'ID_NhomCLS', index: 'ID_NhomCLS', search: false, width: "0%", editable: false, align: 'left', hidden: true,edittype:"select",formatter:'select',editoptions:{value: tennhomcls}},
			...
			groupText : ["<input id='grouping'  type='checkbox' if({1}=={sum}) {title='{1}=={sum}'} class='grouping_{Nhom_CLS}_{1}' onchange='cbChanged(this.className, this);'> <font color='#0000FF'> <b>{0} - {1}  hạng mục  </font><font color='red'> (<span id='dachon'> {sum}</span> hạng mục đã chọn)</font></b>"]
		 	 },
            ...
            loadComplete: function(data) {
               ...
               var groups = $(this).jqGrid("getGridParam", "groupingView").groups
				...
				for(var i=0;i<ids.length;i++){
					...
					if(rowData["ID_LoaiKham_NhomTemplate"]>0 && rowData["idnhom"]>0){
...
function load_select_tennhomcls(){
	window.tennhomcls = $.ajax({url: "resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=NhomCLS&id=ID_NhomCLS&name=TenNhom", async: false, success: function(data, result) {if (!result) alert('Không load được danh mục NhomCLS');}}).responseText;	
