...
function create_grid(){	
		//window.kiemtrasearch = true;
		 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_phongban&q=2',
		datatype: "json",	
	...
	$("#rowed3").jqGrid('navGrid',"#prowed3",{add: permission["add"], edit: permission["edit"], del: permission["del"], search: false,closeAfterEdit: true,clearAfterAdd :true,closeOnEscape : true }, //options						 
		{recreateForm:true,height:'auto',width:'900',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=edit',closeOnEscape : true,modal: true,recreateForm: true,savekey: [true,121],closeAfterEdit: true,
		...
		{height:'auto',width:'900',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=add',closeOnEscape : true,closeAfterAdd:false,modal: true,addedrow: "first",recreateForm:true,savekey: [true,121],closeAfterAdd: true
				,afterSubmit : function(response, postdata){
...
function load_phong_ban(status){
	...
	window.khuvuc = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=DM_KhuVuc &id=ID_KhuVuc&name=TenKhuVuc', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục khu vực');}}).responseText;
	window.congty = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=DM_CongTy &id=ID_CongTy&name=TenCongTy', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục công ty');}}).responseText;
	window.phongcm = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=DM_PhongChuyenMon &id=ID_PhongChuyenMon&name=TenPhongChuyenMon', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban chuyên môn');}}).responseText;
	window.tang = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=GD2_DMTang &id=Id_Tang&name=TenTang', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục tầng');}}).responseText;
	if(status==true){
