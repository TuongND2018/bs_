        ...
        <div class="ui-layout-east ui-widget-content right_col">   
           ...
           <!-- <a  id="luu" class="fm-button ui-state-default ui-corner-all "  > <span class="ui-button-text">Lưu [F10]</span></a>-->
            <a  id="luu" class="ui-button ui-widget ui-state-default ui-corner-all fm-button-icon-left ui-button-text-only " >
                <span class="ui-button-text">Lưu [F10]<span class="ui-icon ui-icon-plusthick"></span></span>
                 </a>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function create_grid() {
        $("#rowed3").jqGrid({
			...
			var rowData = jQuery(this).getRowData(id); 			
			 $("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaikham&idphongban="+id,groupingView: { groupCollapse : false}}).trigger('reloadGrid');
			...
			ids = $('#rowed3').jqGrid('getDataIDs');
			 $("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaikham&idphongban="+ids[0],groupingView: { groupCollapse : false}}).trigger('reloadGrid');
			//$("#"+ids[1]).focus();
    ...
    function create_grid2()
    {
        ...
        $("#rowed4").jqGrid({ 
            data: mydata,
            datatype: "local",
            colNames: ['ID loại khám (Loaikham_phongbanvl)', 'ID Phòng ban', 'Luu', 'events', 'ID_PhongBan','ID Nhóm CLS','ID loại khám', 'Tên hạng mục khám', 'Mã viết tắt', 'STT'],
            colModel: [
