...
jQuery(document).ready(function() { 
	...
	$("#rowed3").jqGrid('navButtonAdd', '#prowed3', {caption: "Excel", buttonicon: 'ui-icon-calculator', id : 'search_rowed3',
            onClickButton: function() {			 
				window.open('resource.php?module=report&view=danhmuc&action=in_danh_muc_chuc_vu&id_form=a&type=excel');
            },
			   ...
			   afterSubmit : function(response, postdata) { 
				...
				afterShowForm : function (formid) {						
					...
					$("#add_new2").click(function(e){
					   $.post('resource.php?module=web_services&function=get_link&action=index&folder=danh_muc_goikham:').done(function(data) {
					  ...
					  $("#add_new").click(function(e){
					   $.post('resource.php?module=web_services&function=get_link&action=index&folder=danh_muc_ctykhachhang:').done(function(data) {
					  elem=1 + Math.floor(Math.random() * 1000000000);
					   width=90;
						  height=90;
					  var folder= data.split(';');
...
function load_select2(){
	window.nation22 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_KhachHangLaCTy&id=ID_KhachHangCTy&name=TenCTy', async: false, success: function(data, result) {if (!result) alert('Không load được');}}).responseText;	
...
function load_select1(){
	window.nation1 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=GoiKham&id=ID_GoiKham&name=TenGoiKham', async: false, success: function(data, result) {if (!result) alert('Không load được');}}).responseText;	
...
function load_select(){
	window.nation = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=GoiKham&id=ID_GoiKham&name=TenGoiKham', async: false, success: function(data, result) {if (!result) alert('Không load được');}}).responseText;	
	window.nation2 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_KhachHangLaCTy&id=ID_KhachHangCTy&name=TenCty', async: false, success: function(data, result) {if (!result) alert('Không load được');}}).responseText;	
	window.hoten = $.ajax({url: "resource.php?module=web_services&function=get_auto_edit_hovaten&action=index", async: false, success: function(data, result) {if (!result) alert('Không load được danh mục nhân viên');}}).responseText; //alert(hoten);	
}
