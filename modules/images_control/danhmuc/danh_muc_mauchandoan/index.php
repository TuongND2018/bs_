...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_template',
		datatype: "json",	
		...
		colModel:[
			{name:'ID_Template',index:'ID_Template',search:false, width:"100%", editable:false,align:'right',hidden:true},
                        ...
                        {name:'ID_ParentTemplate',index:'ID_ParentTemplate', width:"100%", editable:true,align:'center',hidden:false,formatter:"select",editrules: {required:true},edittype:"select",editoptions: { value: nation},formoptions:{ rowpos:6, colpos:1, elmsuffix:'<a id="add_new2" class="fm-button ui-state-default ui-corner-all " style="margin-left:30px!important;vertical-align:top;width:16px;height:13px;"  ><span style="margin-top:-2px;margin-right:-22px!important" class="ui-icon ui-icon-circle-plus"></span></a>'}}, 
			...
			{name:'KetLuan',index:'KetLuan', width:"120%", editable:true,align:'left',hidden:false,edittype:"textarea",formoptions:{ rowpos:2, colpos:2}},
                        ...
                        {name:'GiaTien',index:'GiaTien',editrules: {required:true}, width:"50%", editable:true,align:'left',hidden:false,edittype:"text",formoptions:{ rowpos:4, colpos:2}},
			...
			{name:'STT',index:'STT', width:"50%", editable:true,align:'center',hidden:false,sorttype: 'number', formatter: "number",formoptions:{ rowpos:5, colpos:2}},	
                        ...
                        {name:'ID_NhomTemplate',index:'ID_NhomTemplate', width:"100%", editable:true,align:'center',hidden:false,formatter:"select",editrules: {required:true},edittype:"select",editoptions: { value: nation3},formoptions:{ rowpos:1, colpos:2, elmsuffix:'<a id="add_new3" class="fm-button ui-state-default ui-corner-all " style="margin-left:30px!important;vertical-align:top;width:16px;height:13px;"  ><span style="margin-top:-2px;margin-right:-22px!important" class="ui-icon ui-icon-circle-plus"></span></a>'}},
	...
	$("#rowed3").jqGrid('navGrid',"#prowed3",{add: permission["add"], edit: permission["edit"], del: permission["delete"], search: false,closeAfterEdit: true,clearAfterAdd :true,closeOnEscape : true, bSubmit: "Submit",
     bCancel: "Cancel"}, //options						 
		{recreateForm:true,height:'auto',width:'auto',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=edit&hienmaloi=3',closeOnEscape : true,modal: true,recreateForm: true,
			   afterSubmit : function(response, postdata) { 
				...
				afterShowForm : function (formid) {
					...
					$("#add_new").click(function(e){
					  ...
					  links='resource.php?module=danhmuc&view=danh_muc_nhan_vien&id_form=47&id_loai=undefined';
					  ...
					  links='resource.php?module=danhmuc&view=danh_muc_nhommauchandoan&id_form=47&id_loai=undefined';
					  elem=1 + Math.floor(Math.random() * 1000000000); 
					  width=90;
					  height=90;  
		...
		{height:'auto',width:'auto',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=add&hienmaloi=a',closeOnEscape : true,closeAfterAdd:false,modal: true,addedrow: "first",recreateForm:true,savekey: [true,121]
				,afterSubmit : function(response, postdata){
...
function load_select1(){
	window.xaphuong = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_nhanvien&id=ID_nhanvien&name=Tennhanvien', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục nhân viên');}}).responseText;	
	$("#rowed3").setColProp('ID_nhanvien', { editoptions: { value: xaphuong} });
	$('#ID_nhanvien').empty();
	create_select("#ID_nhanvien",xaphuong);
}
function load_select2(){
	window.xaphuong2 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_template&id=ID_template&name=Tentemplate', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục mẫu chuẩn đoán');}}).responseText;	
	$("#rowed3").setColProp('ID_template', { editoptions: { value: xaphuong2} });
	$('#ID_template').empty();
	create_select("#ID_template",xaphuong2);
}
function load_select3(){
	window.xaphuong3 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_Nhomtemplate&id=ID_Nhomtemplate&name=TenNhom', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục nhóm mẫu chuẩn đoán');}}).responseText;	
...
function load_select(){
	window.nation2 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=DM_nhanvien&id=ID_nhanvien&name=Tennhanvien', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục nhân viên');}}).responseText;	
        window.nation = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=0&tables=DM_template&id=ID_template&name=Tentemplate', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục mẫu chuẩn đoán');}}).responseText;
        window.nation3 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_NhomTemplate&id=ID_NhomTemplate&name=TenNhom', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục nhóm mẫu chuẩn đoán');}}).responseText;
}
