...
function create_viewer(){
	$.getJSON( 'file_manager/php/connector.php?answer=42&tenthumuc='+_folder_name+'&cmd=search&q=<?=$_GET["search_string"]?>&_=1387364774212', 
		...
		$("#images_thumnail img").click(function(e) {
			...
			var _dimension;
			 _dimension=$.ajax({url: 'resource.php?module=web_services&function=get_file_size&action=index&id_form=111&tenthumuc='+_folder_name+'&hash_name='+$(this).attr("alt"), async: false, success: function(data, result) { 			           		 
             }}).responseText;			 
