...
div[id$="ThanhTienTon_summary"] ,div[id$="ThanhTienNhapTrongKy_summary"],div[id$="ThanhTienXuatTrongKy_summary"],div[id$="ThanhTienTonDau_summary"]{ 
...
<ul id="menu" >
 ...
 <div id="panel_main1">
		<div id="grid_phong_ban" style="display:inline-block">
		  ...
		  <span style='margin-left:10px;'><input type='checkbox' id='NCC' />Nhóm theo NCC</span>
          <span style='margin-left:10px;'><input type='checkbox' id='thugon' />Thu gọn</span>
        </div>
...
jQuery(document).ready(function() {
	...
	window.idncc_search=0;
	 window.dataView;
 	 window.grid;
	 	$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+$("#makho1").val()+"&n_order=ORDER BY  id_kho desc,TenBietDuoc,STT_UUTIEN,SoLo").done(function(data){
			data=$.parseJSON(data)
  ...
  window.columns = [
			...
			{name:'SL tồn đầu',id:'SoLuongTonDau',field: "SoLuongTonDau",width: 55,formatter: number,cssClass: "textright"},
			{name:'TT tồn đầu',id:'ThanhTienTonDau',field: "ThanhTienTonDau",width: 100,formatter: number,cssClass: "textright", hasTotal: true,summaryFormatter: costFormatter},				 
            {name:'SL nhập trong kỳ',id:'SoLuongNhapTrongKy',field: "SoLuongNhapTrongKy",width: 55,formatter: number,cssClass: "textright", groupTotalsFormatter: sumTotalsFormatter},
			...
			{name:'SL xuất trong kỳ',id:'SoLuongXuatTrongKy',field: "SoLuongXuatTrongKy",width: 55,formatter: number,cssClass: "textright", groupTotalsFormatter: sumTotalsFormatter},
 ...
 function filter(item) {
	  	...
	  	dataView.setGrouping([					   
					   {
						...
						formatter: function (g) {
							 ...
							 var xuat = sumTotalsFormatter(g.totals, columns[11]).formatMoney(0, ',', '.');
							 var nhap = sumTotalsFormatter(g.totals, columns[9]).formatMoney(0, ',', '.');
							 return "<strong>" + g['rows'][0]['TenThuoc'] + "</strong>  <span style=''> Số lượng tồn: <strong>" + total  + "</strong> Số lượng xuất: <strong>" + xuat  + "</strong> Số lượng nhập: <strong>" + nhap  + "</strong></span>";			
							},	
						aggregators: [new Slick.Data.Aggregators.Sum("SoLuongTon"),
  									  new Slick.Data.Aggregators.Sum("SoLuongXuatTrongKy"),
 									  new Slick.Data.Aggregators.Sum("SoLuongNhapTrongKy")],
						...
						formatter: function (g) {
							if(g.value==1){
								var tennhom='Kho Tổng'
							}else if(g.value==2){
								var tennhom='Kho Khám Bệnh'
							}else if(g.value==3){
								var tennhom='Kho đông y'
							}else if(g.value==4){
								var tennhom='Kho thẩm mỹ'
							}
							
							 var total = sumTotalsFormatter(g.totals, columns[13]).formatMoney(0, ',', '.');
							 var xuat = sumTotalsFormatter(g.totals, columns[11]).formatMoney(0, ',', '.');
							 var nhap = sumTotalsFormatter(g.totals, columns[9]).formatMoney(0, ',', '.');
							 return "<strong>" + tennhom + "</strong>  <span style=''> Số lượng tồn: <strong>" + total  + "</strong> Số lượng xuất: <strong>" + xuat  + "</strong> Số lượng nhập: <strong>" + nhap  + "</strong></span>";			
							},	
						aggregators: [new Slick.Data.Aggregators.Sum("SoLuongTon"),
  									  new Slick.Data.Aggregators.Sum("SoLuongXuatTrongKy"),
 									  new Slick.Data.Aggregators.Sum("SoLuongNhapTrongKy")],
						...
						formatter: function (g) {
							 ...
							 var xuat = sumTotalsFormatter(g.totals, columns[11]).formatMoney(0, ',', '.');
							 var nhap = sumTotalsFormatter(g.totals, columns[9]).formatMoney(0, ',', '.');
							 return "<strong>" + g.value + "</strong>  <span style=''> Số lượng tồn: <strong>" + total  + "</strong> Số lượng xuất: <strong>" + xuat  + "</strong> Số lượng nhập: <strong>" + nhap  + "</strong></span>";			
							},	
						aggregators: [new Slick.Data.Aggregators.Sum("SoLuongTon"),
  									  new Slick.Data.Aggregators.Sum("SoLuongXuatTrongKy"),
 									  new Slick.Data.Aggregators.Sum("SoLuongNhapTrongKy")],
  ...
  $(grid.getHeaderRow()).delegate(":input", "change keyup", function (e) {
    ...
    dataView.beginUpdate();
		//$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho=1&n_order=ORDER BY TenBietDuoc,STT_UUTIEN,SoLo").done(function(data){
		//	data=$.parseJSON(data)
		//	dataView.setItems(data);
    ...
    case '1':
        window.open("resource.php?module=report&view=<?=$modules?>&action=tonghopthuochangngay&type=excel&tungay="+$("#from_day").val()+"&denngay="+$("#to_day").val());
        break;
    case '2':
       window.open("resource.php?module=report&view=<?=$modules?>&action=tonghopthuoctheothang&type=excel&thang="+$('#option2_month option:selected').val()+"&nam="+$('#option2_year option:selected').val());
        break;
    case '3':
        ...
        break;
    case '4':
          window.open("resource.php?module=report&view=<?=$modules?>&action=tonghopthuochangngay_ngoaitru&type=excel&tungay="+$("#from_day").val()+"&denngay="+$("#to_day").val());
        break;
    case '5':
         window.open("resource.php?module=report&view=<?=$modules?>&action=tonghopthuoctheothang_ngoaitru&type=excel&thang="+$('#option2_month option:selected').val()+"&nam="+$('#option2_year option:selected').val());
        break;
	 case '6':
          window.open("resource.php?module=report&view=<?=$modules?>&action=tonghopthuochangngay_tralai&type=excel&tungay="+$("#from_day").val()+"&denngay="+$("#to_day").val());
        break;
	case '7':
          window.open("resource.php?module=report&view=<?=$modules?>&action=tonghopthuoctheothang_tralai&type=excel&thang="+$('#option2_month option:selected').val()+"&nam="+$('#option2_year option:selected').val());
        break;
		}//case
          $( this ).dialog( "close" );
		},
...
$("#to_day").datepicker({
	...
	$("#xem").click(function(){
		$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+$("#makho1").val()+"&n_order=ORDER BY TenBietDuoc,STT_UUTIEN,SoLo&hienmaloi=1").done(function(data){
			data=$.parseJSON(data)
			dataView.setItems(data);
			dataView.setFilter(filter);			
 			dataView.endUpdate();
...
function create_grid(){
	 ...
	 $("#rowed6").jqGrid({
	//	url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop_theonhacungcap&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+window.idkho+"&idncc="+window.idncc+"&n_order= ORDER BY TenNCC,TenBietDuoc",
		url:'',
		datatype: "json",
		...
		onSelectRow: function(id){
			window.idkho=id;
				...
				if($(elem).data('clicked')){
					$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+id+"&n_order=ORDER BY TenBietDuoc,STT_UUTIEN,SoLo&hienmaloi=1").done(function(data){
						data=$.parseJSON(data)
						dataView.setItems(data);				
						dataView.endUpdate();
						if(id==0){
	   ...
	   var rowData = $("#rowed6").getRowData(selectedRowId);
   		$.get('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=update_uutienxuat&hienmaloi=a&idthuoc='+rowData['MaThuoc']+'&solohethong='+rowData['SoLoHeThong']).done(function(data) {
			 ...
			 if(window.n_action==2){
				$("#rowed6").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop_theonhacungcap&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+$("#makho1").val()+"&idncc="+window.idncc+"&n_order= ORDER BY TenBietDuoc,STT_UUTIEN,SoLo"}).trigger('reloadGrid');
			 }
