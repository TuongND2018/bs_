...
$("#to_day").datepicker({
	...
	$("#xem").click(function(){
		...
		$("#com_nhacungcap").val('');
		window.n_action=1;
		 $("#rowed6").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+window.idkho+"&n_order= ORDER BY TenNCC,TenBietDuoc"}).trigger('reloadGrid');
		 //$("#rowed6").jqGrid('groupingRemove');
			 $('#rowed6').jqGrid('groupingGroupBy',['TenNhaCungCap','TenThuoc'], {
				...
				groupSummary : [true],
				groupText : ['<b>{0}</b> ','<b>{0}</b> - Số lượng tồn: <b>{SoLuongTon}</b>']
				},
			});
		jQuery("#rowed6").jqGrid('hideCol', ["TenNhaCungCap"]);
			// $("#rowed6").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+window.idkho+"&n_order= ORDER BY TenNCC,TenBietDuoc"}).trigger('reloadGrid');
			// $("#rowed6").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+window.idkho+"&n_order= ORDER BY TenNCC,TenBietDuoc"}).trigger('reloadGrid');
	});
      ...
      if($("#tenthuoc").is(':checked')==true){
		  if(window.n_action==1){
			...
			$('#rowed6').jqGrid('groupingGroupBy', 'TenThuoc', {
				...
				groupColumnShow :false,
				groupSummary : [true],
				groupText : ['<b>{0}</b> - Số lượng tồn: <b>{SoLuongTon}</b>']
				},
			});
			jQuery("#rowed6").jqGrid('showCol', ["TenNhaCungCap"]);
			$("#rowed6").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+window.idkho+"&n_order= ORDER BY TenBietDuoc"}).trigger('reloadGrid');
		  }else{
			  //$("#rowed6").jqGrid('groupingRemove');
			$('#rowed6').jqGrid('groupingGroupBy', 'TenThuoc', {
				//grouping:true,
			...
			jQuery("#rowed6").jqGrid('showCol', ["TenNhaCungCap"]);
			$("#rowed6").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop_theonhacungcap&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+window.idkho+"&idncc="+window.idncc+"&n_order= ORDER BY TenBietDuoc"}).trigger('reloadGrid');
		  }
      }else{
		  if(window.n_action==1){
			 //  $("#rowed6").jqGrid('groupingRemove');
...
function create_grid(){
	...
	//	url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop_theonhacungcap&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+window.idkho+"&idncc="+window.idncc+"&n_order= ORDER BY TenNCC,TenBietDuoc",
		...
		colModel:[
			...
			{name:'SoLuongTon',index:'SoLuongTon', width:"70",align:'right', editable: false,formatter:'number', formatoptions:{decimalPlaces: 0},  summaryType:'sum'},
	    ...
	    grouping:true,
          groupingView : {
             ...
             groupColumnShow :false,
			groupSummary : [true],
			groupText : ['<b>{0}</b>  - Số lượng tồn: <b>{SoLuongTon}</b>','<b>{0}</b> - Số lượng tồn: <b>{SoLuongTon}</b>']
       },
