...
function create_grid(){
	 ...
	 $("#rowed6").jqGrid({
		url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop_theonhacungcap&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+$("#makho1").val()+"&idncc="+window.idncc+"&n_order= ORDER BY TenNCC,TenBietDuoc",
		//url:'',
		datatype: "json",
      ...
      //  $("#rowed6").jqGrid('footerData', 'set', { 'GiaBan': GiaBan });
		...
		var SoLuongTon = $("#rowed6").jqGrid('getCol', 'ThanhTienTon', false, 'sum');
