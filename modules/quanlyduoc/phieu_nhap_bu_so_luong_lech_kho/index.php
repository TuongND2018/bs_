...
$(document).ready(function() {
	...
	selectbox_search('gs_ID_Thuoc','gs_TenGoc','#rowed3');
	//$('#thang').dateEntry({dateFormat: 'dy -'});
	window.dmthuoc_search=':;1;test';
	$('#thang').datepicker( {
        changeMonth: true,
        changeYear: true,
...
/*	$.get('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dmthuoc_search').done(function(data) {
		window.dmthuoc_search=data;
	})
	*/
...
function create_grid(){	
 var searhicon = '<span class="ui-state-error ui-state-hover  " style="border:0"><span class="ui-icon  ui-icon-closethick" style="float: left; margin-right: .3em;"></span></span>';			
	...
	//	url:"resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_chitietphieuxuat&id_phieuxuatnoibo="+window.id_phieuxuatnoibo,
		...
		colModel:[	
			{name:'xoa',index:'xoa',search:false,width:"1%",align:'center',hidden:false, editable: false},
			...
			{name:'ID_Thuoc',index:'ID_Thuoc',search:true, width:"15%", editable:true,align:'left',edittype:"text",hidden:false,edittype:"select", searchoptions: {sopt: ['eq', 'ne'], value: dmthuoc_search},editoptions:{value:window.thuoc},formatter: "select",editrules:{custom: true, custom_func: function(value, colName) {
			...
			,editrules:{custom: true,required:true, custom_func: function(value, colName) {
						...
						id_row= $(this).closest('tr').attr('id');
			  			$("#rowed3").jqGrid('setCell',id_row,'Post', '0');
			...
			,editrules:{custom: true,required:true, custom_func: function(value, colName) {
				 ...
				 {name:'NgayHetHan',index:'NgayHetHan',search:true, width:"5%", editable:true,align:'center',hidden:false,edittype:"text",formatter: 'date' ,formatoptions: {newformat: "d/m/y", srcformat: 'd/m/y',},editoptions: {	  
			  dataInit: function(element) {
				$(element).datepicker({
							 ...
							 beforeShow: function() {
								...
								//	$("#rowed3").jqGrid('setCell',rowId,'Post', '0');
								}
							 })
			  },
			  ...
			  {name:'SoLoNhaSanXuat',index:'SoLoNhaSanXuat', width:"7%", editable:true,align:'center',edittype:"text",hidden:false,editrules: {required: false},editrules:{custom: true,required:false, custom_func: function(value, colName) {
				 ...
				 {name:'TenGoc',index:'TenGoc',width:"15%",align:'center',hidden:true, editable: false,searchoptions: {searchhidden: true}},
				 {name:'Post',index:'Post',search:false, width:"8%",editrules: {required: false}, editable:false,align:'center',hidden:true,edittype:"text"}, 
		],
		inline_esc:false,
		grid_mode:'',
		grid_move:"",	
	...
	$("#rowed3").jqGrid('inlineNav', '#grid_footer', {add: true, addtext: '', edittext: '', edit: false, save:true,
		 ...
		 $("#rowed3").jqGrid('navButtonAdd', '#grid_footer', {caption: "Lưu [F10]", buttonicon: 'ui-icon-disk',id:'rowed3_luu',
            onClickButton: function() {	
				if(window.luu==0){
					window.luu=1;
					$('#rowed3_luu div.ui-pg-div').html('<span class="ui-icon ui-icon-disk"></span> Đang lưu...');
					...
					*/
					...
					for(var i=0;i<=ids.length-1;i++){
						...
						if(rowData['Post']!=1){
							dataToSend += phancach+'{';
							dataToSend += '"ID_TonKho":' + JSON.stringify(rowData['ID_TonKho'])+',';
							dataToSend += '"ID_Thuoc":' + JSON.stringify(rowData['ID_Thuoc'])+',';
							dataToSend += '"SoLuong":' + JSON.stringify(rowData['SoLuong'])+',';
							dataToSend += '"ThanhTien":' + JSON.stringify(rowData['ThanhTien'])+',';
							dataToSend += '"NgayHetHan":' + JSON.stringify(rowData['NgayHetHan'])+',';
							dataToSend += '"SoLoNhaSanXuat":' + JSON.stringify(rowData['SoLoNhaSanXuat'])+',';
					...
					$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller&oper=add&hienmaloi=a&thang='+thang+'&nam='+nam+'&kho='+kho+'&xoa='+window.ds_thuocxoa,postData).done(function(data){
						...
						$("#chon").click();
						window.luu=0;
						$('#rowed3_luu div.ui-pg-div').html('<span class="ui-icon ui-icon-disk"></span> Lưu [F10]');
