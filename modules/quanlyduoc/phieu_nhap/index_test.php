...
<span><label style="font-weight:bold">Từ ngày</label> <input id="tu_ngay"  value="<?php echo date("d/m/y") ?>"><label style="font-weight:bold">đến ngày</label> <input id="den_ngay" value="<?php echo date("d/m/y") ?>"><button id="timkiem">Tìm kiếm</button>
      ...
      <div id="container">
        <div class="form_row" style="vertical-align:top;width:17%"  >
          ...
          <div>
           ...
           <div>
            <label for="isBHYT" style="width:90px;">BHYT</label>
            <input id="isBHYT" type="checkbox" name="isBHYT"> 
          </div>
        ...
        <div class="form_row" style="vertical-align:top;width:29%"  >
            ...
            <input id="ID_Kho1" type="text" style="width:78px;display:none" lang="luu" name="ID_Kho1" type="text">
          </div>
          <div>
          ...
          <div>
           <span>
            ...
            <label id="ID_NhanVien1" class="hienthi" style="width:208px!important;  text-align:left!important;" id="nhanvien"> </label>   
            </span>
          </div>
          <div>
...
jQuery(document).ready(function() {	
	 ...
	 $("#NgayLapPhieu, #NgayNhapKho, #NgayHoaDon,#den_ngay,#tu_ngay").dateEntry({dateFormat:$.cookie("ngayDateEntry")});
	 $("#NgayLapPhieu,#NgayNhapKho,#NgayHoaDon,#den_ngay,#tu_ngay").datepicker({
           showWeek: true,
							defaultDate: "+1w",
							changeMonth: true,
							changeYear: true,
...
$( "#dialog_excel" ).dialog({
    ...
    case '1':
		window.open('resource.php?module=report&view=<?=$modules?>&action=dauphieunhap_xuat_excel&type=excel&tungay='+$("#tu_ngay").val()+"&denngay="+$("#den_ngay").val()+'&id_loai=<?=$_GET["id_loai"]?>');	
        break;
    case '2':
		window.open('resource.php?module=report&view=<?=$modules?>&action=phieunhap_excel&type=excel&tungay='+$("#tu_ngay").val()+"&denngay="+$("#den_ngay").val());
       break;
		}//case
          $( this ).dialog( "close" );
		},
...
function save_data(action){	
	...
	savedRow = $('#rowed6').jqGrid("getGridParam", "savedRow"); 
	if (savedRow && savedRow.length > 0) {
		//for(var i=0; i<savedRow.length;i++){
	...
	/*if (savedRow && savedRow.length > 0) {			  
		var localGridData_tam = localGridData.splice(0, (localGridData.length-1));	
		var localdataToSend = JSON.stringify(localGridData_tam);						
	}else{	
		var localdataToSend = JSON.stringify(localGridData);				
	}*/
	
	//alertObject(localdataToSend);
...
function create_grid(){
 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_phieu_nhap&oper=phieunhap&id_loai=<?=$_GET["id_loai"]?>',
		datatype: "json",	
		...
		colModel:[
			...
			{name:'ID_Kho',index:'ID_Kho', width:"5%", editable:true,align:'center'}, 
			{name:'ID_NhapXuat',index:'ID_NhapXuat', width:"5%", editable:true,align:'center',edittype:"text"}, 
			{name:'PhanTramVat',index:'PhanTramVat', width:"5%", editable:true,align:'right',edittype:"text",hidden:false,formatter:'number',formatoptions:{decimalSeparator:",", thousandsSeparator: ",", decimalPlaces: 0,  defaultValue: '0'}},
			{name:'TienVAT',index:'TienVAT', width:"5%", editable:true,align:'right',edittype:"text",hidden:false,formatter:'currency',formatoptions:{decimalSeparator:",", thousandsSeparator: ".", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
			{name:'ThanhTien',index:'ThanhTien', width:"10%", editable:true,align:'right',edittype:"text",hidden:false,formatter:'currency',formatoptions:{decimalSeparator:",", thousandsSeparator: ".", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
			...
			{name:'Is_BHYT',index:'Is_BHYT', search: true, stype: 'select', searchoptions: {sopt: ['eq'], value: ":Tất cả;1:BHYT;0:VP"}, formatter: "checkbox", edittype: "checkbox", editoptions: {value: "1:0"}, width:"3%", editable:true,align:'left',hidden:false}, 			 	 	 
		],
		multiselect :false,
		loadonce: true,
		scroll: false,		 
	...
	$("#rowed3").jqGrid('navButtonAdd', '#prowed3', {caption: "<?php get_text("duyet")?>", buttonicon: 'ui-icon-person', id : 'confirm_rowed3',
            onClickButton: function() {
				...
				$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=confirm&hienmaloi=1&id='+window.ids+'&id_loainhapxuat=<?php echo $_GET['id_loai']; ?>')
    			 .done(function( data ) {		
				    window.oper='edit';
	...
	$("#rowed3").jqGrid('navButtonAdd', '#prowed3', {caption: "Duyệt BHYT", buttonicon: 'ui-icon-person', id : 'confirm_bhyt_rowed3',
            onClickButton: function() {
				...
				$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=confirm_bhyt&hienmaloi=1&id='+window.ids+'&id_loainhapxuat=<?php echo $_GET['id_loai']; ?>')
    			 .done(function( data ) {		
				    window.oper='edit';
					window.id_edit=$("#rowed3").jqGrid('getGridParam', 'selrow');				
					$('#rowed3').setGridParam({datatype:"json" }).trigger("reloadGrid");
	...
	$("#rowed3").jqGrid('navButtonAdd', '#prowed3', {caption: "<?php get_text("xem")?>", buttonicon: 'ui-icon-document',id:'printer_preview_2',
            onClickButton: function() {	
				//alert(window.ids);
				window.open('resource.php?module=report&view=<?=$modules?>&action=xuat_excel_BienBanKiemNhap&type=excel&id='+ids); 
            },
            title: "<?php get_text("xem")?>",
            position: "last"
    });	
...
function create_phieunhapchitiet(){	
	 ...
	 $("#rowed6").jqGrid({	
		...
		colModel:[	
			...
			{name:'TenBietDuoc',index:'TenBietDuoc', width:"18%", editable:true,align:'left',edittype:"text",hidden:false,edittype:"select",editoptions:{value:window.thuoc},formatter: "select"			
				,/*editrules:{custom: true, custom_func: function(value, colName) {
                           return check_idthuoc(value,colName);
                        }}*/
			}, 	{name:'TenDonViTinh',index:'TenDonViTinh',search:false, width:"3%", editable:false,align:'center',hidden:false,edittype:"text",editrules:{required: true}}, 			 
			...
			{name:'DonGia',index:'DonGia', width:"8%", editable:true,align:'right',edittype:"text",editrules: { number: true, required: true},hidden:false,formatter:'currency',formatoptions:{decimalSeparator:",", thousandsSeparator: ".", decimalPlaces: 0, prefix: "", defaultValue: '0'},
				editoptions: { dataEvents:  [{ type: 'keyup', fn: function(e) { 
					if(jwerty.is("enter",e)||jwerty.is("tab",e)){
	  ...
	  autoOpen :false,     
 	  ...
 	  open: function( event, ui ) {
		   ...
		   if(oper=='add'){			   
			   ...
			   $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_phieu_nhap&oper=phieunhap_edit&ids='+$('#rowed3').jqGrid('getGridParam', 'selrow')).done(function(data){
...
//	var PhanTramVat=$('#rowed6').jqGrid('getCell',rowId, 'TenBietDuoc')
	if(e!=1){
...
function timkiem(){
	$('#timkiem').click(function(){
		...
		$('#rowed3').setGridParam({url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_phieu_nhap_search&id_loai=<?=$_GET["id_loai"]?>&tungay='+$('#tu_ngay').val()+'&denngay='+$('#den_ngay').val(),datatype:'json'}).trigger('reloadGrid')
	})
}
