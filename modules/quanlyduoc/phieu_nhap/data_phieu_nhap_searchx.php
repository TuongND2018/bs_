...
<?php
	...
	foreach ($tam as $row) {//duyet toan bo mang tra ve
		 ...
		 $responce->rows[$i]['cell']=array($row["ID_NhapKho"],$row["MaPhieu"],$row["nickname"],$row["NgayLapPhieu"],$row["NgayNhapKho"]->format('d-m-Y'),$row["ID_NguoiDuyet"],$row["NgayDuyet"],$row["ID_NCC"],$row["ID_Kho"],$row["ID_NhapXuat"],$row["PhanTramVat"],$row["TienVAT"],$row["ThanhTien"]+$row["TienVAT"],$row["SoHopDong"],$row["SoDonDatHang"],$row["SoHoaDon"],$row["NgayHoaDon"],$row["GhiChu"],$row["ID_NCC1"],$row["ID_Kho1"]);
		 $i++; 
	}  
