		...
		$('#tratoanbo').click(function(){
			...
			if($('#tratoanbo').is( ":checked" )){
				...
				for(var i=0;i<tmp1.length;i++){
					...
					//jQuery('#rowed33').jqGrid("setCell", tmp1[i], "ThanhTienTraLai", parseInt($('#'+tmp1[i]+'_SoLuongTraLai').val())*parseInt($('#'+tmp1[i]+'_DonGiaTraLai').val()), "");
...
function create_grid()
    {
        ...
        $("#rowed33").jqGrid({
            data: mydata,
            datatype: "local",
            colNames: ['Thuốc / vật tư','Số lô','SL thực xuất','Đơn giá bán','Thành tiền bán','Số lượng trả lại','Đơn giá vốn','Thành tiền vốn','Đơn giá trả lại','Thành tiền trả lại','Ngày sản xuất','Ngày hết hạn','ID đơn thuốc','Lưu','SoLoHeThong','SoLoNhaSanXuat'],
            colModel: [
