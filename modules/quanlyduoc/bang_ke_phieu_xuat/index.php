...
<div id="form_search" style="display:none;" title="Tìm kiếm">
        <div class="form_row" style="vertical-align:top;width:100%"  >
          ...
          <div>
            <label for="manhapxuat" style="width:90px;">Mã nhập xuất</label>
            <input id="manhapxuat" name="manhapxuat" style="width:70px;" type="text">
            <input id='manhapxuat_1' class='com_thuocnhomcls1'  style="display:none">
            <label class="hienthi" id="manhapxuat_display" style="width:324px;margin-left:26px"></label>            
          </div>
          <div>
...
function create_grid(){
 $("#rowed3").jqGrid({
		...
		colModel:[
			...
			{name:'ID_NhanVien',index:'ID_NhanVien', width:"5%", editable:true,align:'center',hidden:false,editrules: {required:true},edittype:"select", formatter: "select",stype: 'select',searchoptions: { sopt: ['eq', 'ne'],value:':;'+window.nickname},},
			{name:'NgayLapPhieu',index:'NgayLapPhieu', width:"5%", editable:false,align:'center',editoptions: {dataInit: function(element) {$(element).datepicker({dateFormat: $.cookie("ngayJqueryUi")})}},hidden:false,edittype:"text"},	 
			{name:'NgayXuat',index:'NgayXuat', width:"5%", editable:false,align:'center',editoptions: {dataInit: function(element) {$(element).datepicker({dateFormat: $.cookie("ngayJqueryUi")})}},hidden:false,edittype:"text"},	 
			 
			{name: 'ID_NguoiDuyet',accept:false, index: 'ID_NguoiDuyet', width: "7%",editable:false, align: 'center', formatter: "select", edittype: "select", hidden: false,stype: 'select',searchoptions: { sopt: ['eq', 'ne'],value:':;'+window.nickname}, formoptions: {}},
				{name:'NgayDuyet',index:'NgayDuyet', width:"5%", editable:false,align:'center',editoptions: {dataInit: function(element) {$(element).datepicker({dateFormat: $.cookie("ngayJqueryUi")})}},hidden:false,edittype:"text"},	 		 
			{name:'ID_NCC',index:'ID_NCC', width:"10%", editable:false,align:'center',edittype:"select",formatter: "select",hidden:true, editoptions: {value: window.nhacungcap			 
			   }}, 
			{name:'ID_Kho',index:'ID_Kho', width:"7%", editable:false,align:'center',edittype:"select",hidden:false,stype:'select',searchoptions: { sopt: ['eq', 'ne'],value:':;'+window.kho},formatter: "select", editoptions: {value: window.kho			 
			   }}, 
			{name:'ID_LoaiNhapXuat',index:'ID_LoaiNhapXuat',stype:"select",searchoptions: { sopt: ['eq', 'ne'],value:':;'+window.loainhapxuat}, width:"10%", editable:false,align:'center',edittype:"text",hidden:false,formatter: "select", editoptions: {value: window.loainhapxuat			 
			   }}, 
			...
			{name:'TenKhachHang',index:'TenKhachHang', width:"10%", editable:false,align:'center',edittype:"text",hidden:false},
			{name:'IsXuatChoToaTam',index:'IsXuatChoToaTam', width:"6%", editable:false,align:'center',edittype:"text",hidden:false},
			{name:'IsPercent',index:'IsPercent', width:"5%", editable:false,align:'left',edittype:"text",hidden:false}, 
			{name:'GhiChu',index:'GhiChu', width:"8%", editable:false,align:'left',edittype:"text",hidden:false}, 			 
	...
	//jsonReader:{repeatitems:true,subgrid:{repeatitems:false}},	
		...
		var ThanhTien = $("#rowed3").jqGrid('getCol', 'ThanhTien', false, 'sum');
        	$("#rowed3").jqGrid('footerData', 'set', { 'ThanhTien': ThanhTien });
	...
	$( "#form_search" ).dialog({
     ...
     "MyButton" : {
         ...
         click: function(){
			 ...
			 if($("#manhapxuat").val()!=''){
				manhapxuat=$("#manhapxuat_1").val();
				  string+=phancach+'ID_LoaiNhapXuat='+manhapxuat;
				 }
			  if($("#denso").val()!=''){
				 denso=$("#denso").val();
			...
			//alert(string);
			_string=string;
			jQuery("#rowed3").setGridParam({url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_phieu_xuat&oper=phieuxuat&namhethong='+cur_year+string,datatype:'json'}).trigger("reloadGrid");
	  ...
	  'Hủy':{  text: "Clear",
		 ...
		 click:function(){
			 ...
			 $("#tuso,#denso,#tenkhachhang,#ghichu,#manhapxuat,#manhapxuat_1").val('');
			 $("#manhapxuat_display").text("");
			 $("#tungay") .focus();
...
function create_sub_grid(){
		...
		url:"",
		datatype: "json",	
		colNames:['id_main','id_thuoc','Mã thuốc','Tên biệt dược','Đơn vị tính','Số lô nội bộ','Số lượng','Đơn giá','Tiền VAT','Thành tiền','Đơn giá vốn','Tiền vốn','Chiết khấu'],
		colModel:[
			...
			{name:'DonGia',index:'DonGia', width:"5%", editable:true,align:'right',edittype:"text",editrules: { number: true, required: false},hidden:false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 2, prefix: "", defaultValue: '0'}},
			{name:'TienVAT',index:'TienVAT', width:"5%", editable:true,align:'right',edittype:"text",editrules: { number: true, required: false},hidden:false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
			...
			{name:'DonGiaVon',index:'DonGiaVon', width:"7%", editable:true,align:'right',edittype:"text",hidden:false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 2, prefix: "", defaultValue: '0'}},
	...
	//jsonReader:{repeatitems:true,subgrid:{repeatitems:false}},	
		...
		loadComplete: function(data) {	
			var ThanhTien = $("#rowed4").jqGrid('getCol', 'ThanhTien', false, 'sum');
        	$("#rowed4").jqGrid('footerData', 'set', { 'ThanhTien': ThanhTien });
			var tienvon = $("#rowed4").jqGrid('getCol', 'TienVon', false, 'sum');
			$("#rowed4").jqGrid('footerData', 'set', { 'TienVon': tienvon });
	...
	$("#rowed4").setGridHeight($(window).height()-$("#rowed3").height()-545+"px");
...
function init_dialog(){
	...
	$.dateEntry.setDefaults({spinnerImage: ''});
		...
		create_combobox('#manhapxuat','#manhapxuat_1',".ma_nhapxuat","#ma_nhapxuat",create_Dm_nhapxuat,500,300,'Mã nhập xuất','data_DMnhapxuat',0);
		...
		change_focus("#tenkhachhang","#manhapxuat");
		change_focus("#manhapxuat","#maKHO");
...
function create_Dm_nhapxuat(elem, datalocal) {
        datalocal = jQuery.parseJSON(datalocal);
        $(elem).jqGrid({
            datastr: datalocal,
            datatype: "jsonstring",
            colNames: [ 'Mã nhập xuất','Tên loại nhập xuất'],
            colModel: [
              
                {name: 'Ma_NhapXuat', index: 'Ma_NhapXuat', hidden: false},
				{name: 'Ten_LoaiNhapXuat', index: 'Ten_LoaiNhapXuat', hidden: false},
            ],
            hidegrid: false,
            gridview: true,
            loadonce: true,
            ...
            onSelectRow: function(id) {
				 var rowData = $('#ma_nhapxuat').getRowData(id);
				 $("#manhapxuat_display").text(rowData["Ten_LoaiNhapXuat"]);
            },
