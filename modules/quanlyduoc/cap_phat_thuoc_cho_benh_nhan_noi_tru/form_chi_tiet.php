...
$(document).ready(function(e) {
	...
	<?php
		...
		$store_name="{call GD2_GetTenKhoaByID_PhieuLinhThuoc(?)}";//tao bien khai bao store
		$params = array($_GET['id_phieulinh']);
		$get=$data->query( $store_name,$params);//Goi store
		$excute= new SQLServerResult($get);//Ket noi lop xu ly SQL và truyen gia tri tra ve tu lop ket noi SQL
		$tam= $excute->get_as_array();//Tra ve mang toan bo data lay duoc 
		echo "window.tenphongban='".$tam[0]['TenPhongBan']."';";
	?>
...
$("#xuatthuoc").click(function(e) {
	if(window.n_luu==0){
	   ...
	   $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_tonkho&hienmaloi=1&kho='+$("#khoxuat_hidden").val()+'&id_phieulinh=<?=$_GET['id_phieulinh']?>',postData).done(function(data){
			data=data.split('||')
			if(data[1]==0){
				$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_xuatthuoc&oper=update&hienmaloi=3&kho='+$("#khoxuat_hidden").val()+'&doituong=<?=$_GET['doituong']?>&id_phieulinh=<?=$_GET['id_phieulinh']?>&nguoilinh=<?=$_GET['nguoilinh']?>&ngaythuclinh='+$("#ngaythuclinh").val()+'&tenkhoa='+window.tenphongban,postData)
...
function create_grid(){
        ...
        datatype: "json",
		colNames:['Họ tên bệnh nhân','ID thuốc','Mã thuốc','Tên thuốc - Vật tư y tế','Đơn vị tính','Số lượng','phantramvat','dongia'],
		colModel:[
			{name:'hotenbenhnhan',index:'hotenbenhnhan',search:true, width:"15%", editable:false,align:'left',edittype:"text",hidden:false},
		...
		grouping:true, 
		groupingView : { 
			groupField : ['hotenbenhnhan'],
			groupDataSorted : true ,
			groupCollapse : false,
			groupColumnShow :[false],
			groupText : ['<b>{0} - {1} thuốc </b>']
		},
