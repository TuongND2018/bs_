<?php
	...
	$store_name2="{call GD2_NoiTruGetLinhThuocByID_PhieuLinhThuoc(?,?)}";//tao bien khai bao store
	...
	foreach ($thuoc as $row) {//duyet toan bo mang tra ve
		...
		$responce->rows[$i]['cell']=array($row["MaBenhNhan"].' - '.$row["HoTenBN"],$row["ID_Thuoc"],$row["MaThuoc"],$row["TenGoc"],$row["TenDonViTinh"],$row["SoThuocThucXuat"],$row["PhanTramThue"],$row["DonGia"]);
		$i++; 
	}
