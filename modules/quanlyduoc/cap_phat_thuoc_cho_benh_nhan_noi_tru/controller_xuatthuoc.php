...
if(isset($_POST['id'])){
	...
	$store_name_check="{call GD2_Check_PhieuLinhThuocNoiTru(?)}";
	$params_check = array($_GET["id_phieulinh"]);
	$get_check=$data->query( $store_name_check,$params_check);
	$excute_check= new SQLServerResult($get_check);
	$tam_check= $excute_check->get_as_array();
	if($tam_check[0]['Dem']==0){	
		...
		$params_ds = array($_GET['id_phieulinh'],1);
		$store_name_ds="{call GD2_NoiTruGetLinhThuocByID_PhieuLinhThuoc(?,?)}";
		...
		foreach ($thuoc as $row) {
			if($row['SoThuocThucXuat']!=0){
				echo ','.$row['ID_Thuoc'];
				$params5 = array(
					$id_xuatkho,//		  @ID_XuatKho as int,
					$row['ID_Thuoc'],//   @ID_Thuoc as  int,
					$row['SoThuocThucXuat'],//   @SoLuong as decimal(18,0),
