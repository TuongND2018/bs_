...
function phieunhap(){ 
	...
	if(isset($_GET["tungay"]) && isset($_GET["denngay"])){
		 ...
		 $dengay=convert_date($_GET["denngay"]);
	}else{
		 $tungay=date("Y-m-d");
		 $dengay=date("Y-m-d");		 
	}
	...
	$params = array($tungay,$dengay,$_SESSION["namhethong"],$search);//tao param cho store 
...
function phieunhap_sub(){ 
	...
	//echo $_GET["ids"];
	$params = array($_GET["ids"],$_SESSION["namhethong"]);//tao param cho store 
	//print_r($params) ;
	$get_lich=$data->query( $store_name, $params);//Goi store
	$excute= new SQLServerResult($get_lich);//Ket noi lop xu ly SQL và truyen gia tri tra ve tu lop ket noi SQL
	$tam= $excute->get_as_array();//Tra ve mang toan bo data lay duoc 	 
