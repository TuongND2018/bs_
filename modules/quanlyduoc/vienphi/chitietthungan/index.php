    ...
    <div id="DM_template_container">
        ...
        <fieldset style="display:inline-block;height:188px;width:580px" id="thongtinnoptien" >
   		 <legend>Thông tin nộp tiền:</legend>
                 ...
                 <label style="font-weight:bold;font-size:12px;width:80px;display:inline-block">Lý do giảm giá</label>  <input id="lydogiamgia" type='textbox' style="width:120px" ><input id="lydogiamgia1" type='textbox' style="display:none" > <input style="margin-left:25px" type="checkbox" name="checkbox" id="nhanvien_giamthuoc" >Có tiền thuốc    <br>  
...
window.load_complete4=0;
    $(document).ready(function() {
		...
		$('#btn_lien2').click(function(){
			$.cookie("in_status", "print_preview"); 
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=thungan&header=top&lien=3type=report&id_form=11&kieuin="+$('input[name=sex]:radio:checked').val()+"&idthutrano="+ID_ThuTraNo,'PhieuThanhToan');
			 $('#dialog_print').dialog('close');
		})
	...
	window.dmkho = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_Kho&id=ID_Kho&name=TenKho', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban');}}).responseText;
          ...
          //  userDataOnFooter: true,
           ...
           ondblClickRow: function(rowId) {
            ...
            loadComplete: function(data) {
				...
				$("#rowed5").jqGrid('setGridParam',{url:'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_toathuoc&ID_LuotKham='+tmp1[0],datatype: 'json'}).trigger("reloadGrid");
				}else{
	...
	if(tongcong>0){
		...
		var datatosend = '{"id_giam_nv":"'+id_giam_nv+'","id_giam_thuoc":"'+id_giam_thuoc+'","id_giam_chidinh":"'+id_giam_chidinh+'","id_giam_dtph":"'+id_giam_dtph+'","idbenhnhan":"'+idbenhnhan+'","NgayGio":'+JSON.stringify($('#ngay').val())+',"nguoigd":"'+$('#nguoigd').val()+'","diachi":"'+$('#diachi').val()+'","diengiai":"'+$('#diengiai').val()+'","tienthu":"'+$('#benhnhantra').val().split('.').join('')+'","tongcong":"'+tongcong+'","id_luotkham":"'+id_luotkham+'","giamgia":"'+giamgia+'","dskho":"'+$.cookie("dskho")+'","sum":"'+sum+'","sumvat":"'+sumvat+'","iddonthuoc":"'+window.id_donthuoc+'"'+miengiam_nv;			
			...
			if(data[1]==0){
				...
				$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_chitiet&hienmaloi=1&kho='+$.cookie("dskho").split(',')[0],datatosend).done(function(data1){
					window.oper='edit';
...
function tinhlai(){
	...
	sum1 =  $("#rowed5").jqGrid("getCol", "tongtien", false, "sum");
	 ids = $("#rowed4").jqGrid('getDataIDs');	
		if($('#nhanvien_giamthuoc').is(":checked")){
			nhanvien_giamthuoc=(sum1+sum)*$('#phantram').val()/100;
		}else{
			nhanvien_giamthuoc=0;
		}
	
	 giamgia=giam_dtph+giam_chidinh+giam_thuoc+giamhientai+nhanvien_giamthuoc;
	...
	function load_form(){		
		if(oper=='add'){
			...
			url_rowed3 = 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_vienphichitiet_theoluotkham&ID_LuotKham='+ID_LuotKham;
			url_rowed4 = 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_donthuoc_luotkham&ID_LuotKham='+ID_LuotKham;			
		}else{
			...
			url_rowed3 = 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_vienphichitiet_theoluotkham&ID_ThuTraNo='+ID_ThuTraNo;
			url_rowed4 = 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_donthuoc_luotkham&ID_ThuTraNo='+ID_ThuTraNo;			
		}
	...
	function sua(){
			...
			datatosend+='}';			
			datatosend=jQuery.parseJSON(datatosend);
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_chitiet_sua',datatosend).done(function(){
				window.oper='add';
				load_form()
				})
