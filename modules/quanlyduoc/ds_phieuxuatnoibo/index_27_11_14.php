        ...
        "YES": function() {
		  ...
		  if(window.n_action==1){
			  ...
			  if(window._maphieugop>0){
				  $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_dieuchuyen_gopphieu&hienmaloi=a&maphieugop='+window._maphieugop+'&tukho='+window.tukho+'&denkho='+window.denkho+'&nguoitao='+window.idnguoitao+'&nguoiduyetxuat='+window.idnguoiduyetxuat).done(function(data){	
				 // alert(data);
						data=data.split(';');
						if(data[2]==1){
							$("#tenthuoc_ton").html(data[3]);
	...
	$("#rowed3").jqGrid({
	   ...
	   loadonce: true,
		...
		onSelectRow: function(id) {
			...
			$("#rowed5").jqGrid('setGridParam',{datatype: 'json',url:"resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_danhsach_phieuxuatnoibochitiet&id_phieuxuatnoibo="+id+"&maphieugop="}).trigger('reloadGrid');
		},
...
function create_grid_2(){
	$("#rowed33").jqGrid({
		...
		caption: "Ds phiếu xuất điều chuyển (Chưa duyệt) <div class='grid1 '><div class='hinhvuong chuachot'></div><label class='diengiai'>Chưa chốt</label></div> <span style='margin-left:130px'>Nếu đã duyệt xuất kho mà vẫn không thấy nút <span style='color:red'>Duyệt nhập kho</span> vui lòng nhấn vào nút <span style='color:red'>Xem</span></span>"
	});
