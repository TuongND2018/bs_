		...
		$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=checkchot&id_phieuxuatnoibo='+window.inphieuid).done(function(data){
			if(data=='true'){
				...
				if(window.n_action==1){
					...
					dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=quanlyduoc&action=phieuxuatdieuchuyen_new&idphieu="+window.inphieuid+"&type=report&id_form=10",'PhieuLinhThuoc');
				}
			}else{
      ...
      buttons: {
        "YES": function() {
		  ...
		  if(window.n_action==1){
			   ...
			   $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_dieuchuyen&hienmaloi=a&idxuatnoibo='+window.idxuatnoibo+'&tukho='+window.tukho+'&denkho='+window.denkho+'&nguoitao='+window.idnguoitao).done(function(data){	
					data=data.split(';');
					if(data[2]==1){
						$("#tenthuoc_ton").html(data[3]);
						$( "#dialog-loithuoc" ).dialog('open');
...
function create_grid_2(){
	$("#rowed33").jqGrid({
		...
		colModel: [
			{name: 'SoPhieu', index: 'SoPhieu', search: true, width: "5%", editable: false, align: 'left', hidden: false},
			{name: 'KhoXuat', index: 'KhoXuat', search: true, width: "15%", editable: false, align: 'left', hidden: false},
			{name: 'KhoNhap', index: 'KhoNhap', search: true, width: "15%", editable: false, align: 'left', hidden: false},
			{name: 'NguoiTao', index: 'NguoiTao', search: true, width: "10%", editable: false, align: 'left', hidden: false},
			{name: 'NgayTao', index: 'NgayTao', search: true, width: "10%", editable: false, align: 'left', hidden: false,sorttype:'date'},
			{name: 'NguoiDuyet', index: 'NguoiDuyet', search: true, width: "10%", editable: false, align: 'left', hidden: true},
			{name: 'NgayDuyet', index: 'NgayDuyet', search: true, width: "10%", editable: false, align: 'left', hidden: true,sorttype:'date'},
	   ...
	   loadonce: true,
		...
		onSelectRow: function(id) {
			...
			var ntam=dataRow['ID_TuKho'];
			//var ntam=5;
			console.log(permission['kho_'+ntam]);
			if(permission['kho_'+ntam]!='' && permission['kho_'+ntam]==1){
				$("#rowed33_duyet").removeClass('ui-state-disabled');
			}else{
				$("#rowed33_duyet").addClass('ui-state-disabled');
			}
		...
		//	window.idnguoitao=dataRow2['IDNguoiTao'];
			$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=checkchot&id_phieuxuatnoibo='+rowId).done(function(data){
				if(data=='false'){
	...
	$("#rowed33").jqGrid('navButtonAdd', '#prowed33', {caption: "Duyệt phiếu", buttonicon: 'ui-icon-check',id:'rowed33_duyet',
            onClickButton: function() {
...
function create_grid2_2(){
	$("#rowed44").jqGrid({
		url:'',
		datatype: "json",
		colNames: ['Số phiếu','Kho xuất','Kho nhập','Người tạo','Ngày tạo','Người duyệt','Ngày duyệt'],
		colModel: [
			{name: 'SoPhieu', index: 'SoPhieu', search: true, width: "7%", editable: false, align: 'left', hidden: false},
			{name: 'KhoXuat', index: 'KhoXuat', search: true, width: "15%", editable: false, align: 'left', hidden: false},
			{name: 'KhoNhap', index: 'KhoNhap', search: true, width: "15%", editable: false, align: 'left', hidden: false},
