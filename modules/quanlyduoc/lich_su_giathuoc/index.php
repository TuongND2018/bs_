...
function create_dmthuoc(elem, datalocal) {
  ...
  function load_select() {
             window.Id_LoaiNhapXuat = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option_byid&action=index&term=&status=2&tables=GD2_DM_Nhapxuat&id=ID_NhapXuat&name=Ten_LoaiNhapXuat', async: false, success: function(data, result) {
                    if (!result)
	...
	function create_nhapkho(elem) {
        ...
        $(elem).jqGrid({
      		data:[],
            datatype: "jsonstring",
            colNames: ['Tên gốc','Ngày nhập', 'Mã phiếu','Tên NCC','Số lô NXS','ĐVT','Số lượng','Đơn giá','Giá bán','Thành tiền','Loại nhập','Nhập tại','Nhân viên','Duyệt bởi'],
            colModel: [
