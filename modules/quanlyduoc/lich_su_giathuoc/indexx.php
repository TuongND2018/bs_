 ...
 tab_click=1;
    $(document).ready(function() {
        ...
        $("#to_day").datepicker({
		 ...
		 $('#xem').click(function(){
			 if(tab_click==1){
				  $('#rowed3').jqGrid('setGridParam', { datatype: "json",url: "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_giabanthuoc&ID_Thuoc="+$('#dm_thuoc_hidden').val()+"&tungay="+$('#from_day').val()+"&denngay="+$('#to_day').val() }).trigger("reloadGrid");	
			 }else{
			
			 $('#rowed4').jqGrid('setGridParam', { datatype: "json",url: "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_nhapkho&ID_Thuoc="+$('#dm_thuoc_hidden').val()+"&tungay="+$('#from_day').val()+"&denngay="+$('#to_day').val() }).trigger("reloadGrid");
			 }
			 })
...
function create_dmthuoc(elem, datalocal) {
	...
	function create_giaban(elem) {
        ...
        $(elem).jqGrid({
      		data:[],
            datatype: "jsonstring",
            colNames: ['Tên gốc','Ngày xuất', 'Mã BN','Tên BN','Đơn vị tính','Số lượng','Đơn giá','Thành tiền'],
            colModel: [
			 {name: 'tengoc', index: 'tengoc', hidden: false,width:'20%'},
				...
				{name: 'tenbn', index: 'tenbn', hidden: false,width:'30%'},
				{name: 'dvt', index: 'dvt', hidden: false,width:'10%'},
                {name: 'sl', index: 'sl', hidden: false,width:'10%',align: 'right',formatter:'currency',formatoptions:{decimalSeparator:",", thousandsSeparator: ".", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				{name: 'dongia', index: 'dongia', hidden: false,width:'20%',align: 'right',formatter:'currency',formatoptions:{decimalSeparator:",", thousandsSeparator: ".", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
                {name: 'thanhtien', index: 'thanhtien', hidden: false,width:'10%',align: 'right',formatter:'currency',formatoptions:{decimalSeparator:",", thousandsSeparator: ".", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
            ...
            loadComplete: function(data) {
				...
				$("#rowed3").jqGrid('footerData','set', {sl: sumsl, dongia: sumdongia, thanhtien: sumthanhtien});
            },
        });
        $(elem).jqGrid('filterToolbar', {searchOperators: false, searchOnEnter: false, defaultSearch: "cn"});
        $(elem).jqGrid('bindKeys', {});
	...
	function create_nhapkho(elem) {
        ...
        $(elem).jqGrid({
      		data:[],
            datatype: "jsonstring",
            colNames: ['Tên gốc','Ngày nhập', 'Mã phiếu','Tên NCC','Số lô NXS','ĐVT','Số lượng','Đơn giá','Giá bán','Thành tiền'],
            colModel: [
				{name: 'tengoc', index: 'tengoc', hidden: false,width:'20%'},
                {name: 'nx', index: 'nx', hidden: false,width:'10%'},
                {name: 'mabn', index: 'mabn', hidden: false,width:'10%'},
				...
				{name: 'giaban', index: 'giaban', hidden: false,width:'5%',align: 'right',formatter:'currency',formatoptions:{decimalSeparator:",", thousandsSeparator: ".", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
