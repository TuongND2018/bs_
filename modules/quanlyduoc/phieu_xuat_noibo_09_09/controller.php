...
if(isset($_POST['id'])){
	if(count($_POST['id'])>0 && $id_phieuxuatnoibo==''){
		$params4 = array($_GET["nguoigiaodich"],$_GET["id_phongban"],'',$_SESSION["user"]["id_user"],array($id_phieuxuatnoibo, SQLSRV_PARAM_OUT,SQLSRV_PHPTYPE_INT));
		//print_r ($params4);
		$store_name4="{call Gd2_PhieuXuatNoiBo_Insert (?,?,?,?,?)}";
	...
	if($loi!=1 && $_GET['id_phieuxuatnoibo']!=''){
		$params5 = array($_GET['id_phieuxuatnoibo'],$_GET['id_phongban'],$_GET['nguoigiaodich'],$_SESSION["user"]["id_user"]);
		//print_r ($params4);
		$store_name5="{call Gd2_PhieuXuatNoiBo_Update (?,?,?,?)}";
