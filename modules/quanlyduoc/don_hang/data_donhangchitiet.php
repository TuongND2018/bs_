...
$store_name="{call GD2_Donhangchitiet_get(?,?)}";//tao bien khai bao store
...
foreach ($tam as $row) {//duyet toan bo mang tra ve
    $responce->rows[$i]['id']=$row["ID_DonHangChitiet"];
    $responce->rows[$i]['cell']=array($row["ID_DonHangChitiet"],$row["ID_Donhang"],$row["ID_Thuoc"],$row["SoLuong"]);
    $i++; 
}   
echo json_encode($responce);
}
