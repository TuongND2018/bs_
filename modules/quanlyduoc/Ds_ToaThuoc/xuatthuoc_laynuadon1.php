...
input[id*="nttralai"],input[id*="sltralai"],input[id*="tientl1v"]  {
  text-align:right;
}
...
echo 'var dauthapphan="'.$_SESSION["config_system"]["dauthapphan"].'";';
echo 'var dauhangngan="'.  $_SESSION["config_system"]["dauhangngan"].'";';
...
?>
 ...
 function create_grid() {
        $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_xuathuoc_nuadon1&id_donthuoc=<?=$_GET['ID_DonThuoc']?>',
		datatype: "json",
		colNames:['Id','Thuốc/vật tư','ĐVT','Cách dùng','SL/1ngày','SL BS kê','SL T.Xuất','Đơn giá','N.Thuốc T.Lại','SL trả lại','Tiền T.Lại/1v','T.Tiền trả lại','Tiền BN đã trả','In','','','','','','','','','','','','','','',''],
		colModel:[
			{name:'ID_DonThuocCT',index:'ID_DonThuocCT',search:false,search:false, width:"0%", editable:false,align:'right',hidden:true}, 
			...
			{name:'sttx',index:'sttx',search:true, width:"5%", editable:false,align:'right',hidden:false},
			{name:'dongia',index:'dongia',search:true, width:"5%", editable:false,align:'right',hidden:false,formatter:'currency',formatoptions : {  decimalSeparator:dauthapphan, thousandsSeparator: dauhangngan, decimalPlaces: 0, prefix: "", suffix:""},sorttype:'currency'},	
			...
			{name:'sltralai',index:'sltralai',search:true, width:"5%", editable:true,align:'right',hidden:false,editoptions: { dataEvents:  [{ type: 'keyup', fn: function(e) { 
				...
				var row = $(e.target).closest('tr.jqgrow');
				 ...
				 tientralai=($(this).val().split('.').join(''))*$('#'+tam2+'_tientl1v').val().split('.').join('');
			...
			{name:'tientl1v',index:'tientl1v',search:true, width:"10%", editable:true,align:'right',hidden:false,formatter:'currency',formatoptions : {  decimalSeparator:dauthapphan, thousandsSeparator: dauhangngan, decimalPlaces: 0, prefix: "", suffix:""},sorttype:'currency',editoptions: {sorttype:'currency', dataEvents:  [{ type: 'keyup', fn: function(e) { 
				var row = $(e.target).closest('tr.jqgrow');
				 var tam2 = row.attr('id');
				 
				 var tam=$(this).val().split('.').join('');
				
					tam3=parseInt(tam).formatMoney(0, ',', '.')
					$('#'+tam2+'_tientl1v').val(tam3)
				 tientralai=tam*$('#'+tam2+'_sltralai').val();
				 	$("#rowed3").jqGrid("setCell", tam2, 'tientralai', tientralai);
				 var sum4 = $("#rowed3").jqGrid("getCol", "tientralai", false, "sum");
				  $("#rowed3").jqGrid("footerData", "set", {tientralai:sum4});
				   $('#tongtien').text(formatMoney(sum4))
			...
			{name:'tientralai',index:'tientralai',search:true, width:"10%", editable:false,align:'right',hidden:false,formatter:'currency',formatoptions : {  decimalSeparator:dauthapphan, thousandsSeparator: dauhangngan, decimalPlaces: 0, prefix: "", suffix:""},sorttype:'currency',editoptions: {sorttype:'currency', dataEvents:  [{ type: 'keyup', fn: function(e) { 
				var row = $(e.target).closest('tr.jqgrow');
                var tam2 = row.attr('id');
			...
			{name:'tientruoc',index:'tientruoc',search:true, width:"10%", editable:false,align:'right',hidden:false,formatter:'currency',formatoptions : {  decimalSeparator:dauthapphan, thousandsSeparator: dauhangngan, decimalPlaces: 0, prefix: "", suffix:""},sorttype:'currency'},			
		...
		//alert('aaaaaa');
			...
			for(i=0;i<=ids.length-1;i++){
				var rowData = $('#rowed3').getRowData(ids[i]);
					...
					dataToSend += phancach + '"ID_DonThuocCT":"' + rowData.ID_DonThuocCT+ '"';	
					...
					dataToSend += phancach + '"ID_DonThuoc":"<?=$_GET['ID_DonThuoc']?>"';
			...
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_phieunhap&oper=add&hienmaloi=3',dataToSend)
		})
	}
