...
var arrary_enter1=[];
 ...
 function create_grid() {
        $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=toathuoc_toatam&id_donthuoc=<?=$_GET['ID_DonThuoc']?>',
		datatype: "json",
...
window.nhanvien2_complete=1;
	...
	function luu_click(){
		$('#luu').click(function(e){
			...
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_toatam&oper=update&hienmaloi=3&kho='+dskho.split(',')[0],dataToSend)
	...
	function xuathuoc_click(){
		$('#xuathuoc').click(function(e){
			...
			dataToSend += ']}';		
			dataToSend = jQuery.parseJSON(dataToSend);			
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=update&hienmaloi=3',dataToSend).done(function(data){
