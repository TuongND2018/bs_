...
jQuery(document).ready(function() {
	...
	$("#xembill").click(function(e){	
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=dstoathuoc_thanhtoanhoadon&header=top&id_luotkham=<?=$_GET['ID_LuotKham']?>&type=report&id_form=10&tenin="+$('input[name = "radio"]:checked').val(),'ToaThuocHoaDon');
		$(".frame_u78787878975f").css("width","793px");
	});
	$("#intoahoadon").click(function(e){	
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=dstoathuoc_intoathuoc&header=top&id_luotkham=<?=$_GET['ID_LuotKham']?>&type=report&id_form=10&tenin="+$('input[name = "radio"]:checked').val(),'ToaThuocHoaDon');
		$(".frame_u78787878975f").css("width","793px");
	});
})
...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_hoadon',
		datatype: "json",	
