...
$(document).ready(function() {	
		...
		if($('#chonanh').is( ":checked" )) {
			...
			{
				dialog_pic_select("Chọn ảnh",90,"170px","jdshju8789789",window.search_string,"Pathology",print_action,maviettat,"GENERAL ULTRASOUND","BÁC SỸ SIÊU ÂM","top",3,"Pathology",'<?=$modules?>',"780");
			}else{
				dialog_pic_select("Chọn ảnh",90,"170px","jdshju8789789",window.search_string,"Pathology",print_action,maviettat,"GENERAL ULTRASOUND","BÁC SỸ SIÊU ÂM","top",3,"Pathology_khongchuky",'<?=$modules?>',"780");
				}
							...
							$('.maucd_button').button( {disabled:true});
						       ...
						       alertObject(dataToSend); 
						        if(nhanvien_control==1){
							     $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=dathuchien',dataToSend).fail(function() {
...
function resize_control(){
	...
	$("#loikhuyen").css("height",$(".ui-layout-east").height()-$(".ui-layout-south").height()-$(".ui-layout-north").height()-53+"px");
...
$("#inchidinh").click(function(){
	 $.cookie("in_status", "print_preview"); 
        dialog_report("Xem trước khi in",90,90,"u78787878975b","resource.php?module=report&view=<?=$modules?>&action=inchidinhphathology&header=left&maviettat="+maviettat+"&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10",'PhieuGiamGiaSieuAmPhanTram');
