...
<div  class="data_mauketluan">
 ...
 <div id="panel_main">    
        ...
        <div id="r_top">
        	<table width="100%">
              <tr>
                <td class="ttleft" width="180px">Chọn lý do ngừng đo:</td>
                <td class="ttright">
                <span class="custom-combobox">
...
$(document).ready(function() {
	...
	$("#xemin").click(function(e){		
		...
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=StressECG&header=left&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10&tieude=&chucnang=&chucdanh=&chuky="+chuky,'StressECG');
	});
...
$( "#ketluan" ).keyup(function(e){
	...
	if(id_kham2!="0"){
		$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dientamdogangsuc&idkham="+id_kham2+"&idbenhnhan="+id_benhnhan, 
			function( data ) {
				data_luotkham=data;
	...
	$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_dientamdogangsuc&idbenhnhan="+id_benhnhan, 
		function( data ) {
			data_luotkham=data;
			...
			window.oper='xoalk';
			  ...
			  $("#dathuchien").click(function(){
				  dis_all("an");
					...
					$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=dathuchien',dataToSend)
			  ...
			  $("#hoantat").click(function(){
				...
				dataToSend = jQuery.parseJSON(dataToSend);
				//alertObject(dataToSend); 
				$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=hoantat',dataToSend)
				.done(function( gridData ) {	
									 //alert(gridData);	 
					})
			  ...
			  $('#luu').click(function (){
			      ...
			      // alertObject(dataToSend); 
			       if(window.trangthai_luu=="dathuchien1"){
					   dis_all("an");
			       $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luuthuchien',dataToSend)
					 .done(function( gridData ) {	
						 tooltip_message("Đã lưu");	
						/* if(window.n_xemin==true){
							 	setTimeout(function(){tooltip_message("Report này chưa làm")},2000);
							 } 
			         ...
			         if(window.trangthai_luu=="hoantat1"){
						 dis_all("an");
			         	$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luuhoantat',dataToSend)
						 .done(function( gridData ) {	
							 tooltip_message("Đã lưu"); 
							  /*if(window.n_xemin==true){
								  setTimeout(function(){tooltip_message("Report này chưa làm")},2000); 
			       ...
			       $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luudangkham',dataToSend)
						 .done(function( gridData ) {	
							 tooltip_message("Đã lưu"); 
							 /* if(window.n_xemin==true){
							 	 	setTimeout(function(){tooltip_message("Report này chưa làm")},2000);
...
function resize_control(){
	...
	$("#r_bottom").css("height",$(".right_col").height()-$("#r_top").height()+"px");
