...
$(document).ready(function() {	
	...
	$("#xemin").click(function(e){
			...
			{		
				dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=ABI_new&header=left&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10&tieude=&chucnang=&chucdanh=",'ABI');
				
			}//loai report ABI may mơi
			else
			{
				dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=ABI&header=left&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10&tieude=&chucnang=&chucdanh=",'ABI');
	...
	if(id_kham2!="0"){
		$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_doxovua2&idkham="+id_kham2+"&idbenhnhan="+id_benhnhan, 
			function( data ) {
				data_luotkham=data;
...
//alert(_id_trangthai); 
	...
	$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_doxovua&idbenhnhan="+id_benhnhan,
		function( data ) {
			data_luotkham=data;
	 	//alertObject(data);
		var tam1_cls="";
                 			...
                 			_id_trangthai="DaThucHien";
                       		...
                       		$('.abi_button').button( {disabled:true});
						        ...
						        dataToSend += phancach + '"id_trangthai":"' + _id_trangthai + '"';
								 dataToSend += phancach + '"mota":"' + $( "#mota" ).val() + '"';
								dataToSend += phancach + '"ketluan":"' + $( "#ketluan" ).val() + '"';
