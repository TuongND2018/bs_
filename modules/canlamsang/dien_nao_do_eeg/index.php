 ...
 <div class="top_form ui-widget-content" >
    ...
    <div class="thongtin_luotkham" id="right_col">
    	...
    	<div style="margin-top: 0px; margin-left:5px;" >
	    	<span id="edit_by"><label>Sửa bởi:</label>
            ...
            <span style="margin-left:165px">
                  <label>Kiểu in: </label>
                  <select id="chonanh">
                  <option value="inngang">In ngang</option>
                  <option value="indoc" selected>In dọc</option>
 ...
 <div id="panel_main">    
        ...
        <div class="ui-widget-content ui-layout-east" id="inner"> 
            ...
            <div class="ui-layout-south thongtin_thai">
                <textarea id="loikhuyen"name="loikhuyen" lang='luu' style="font-size:13px; display:none" ></textarea>
            </div>
...
var tenloaikham;
var idluotkham;
var report_code=["EEG","EEG_InNgang"];
var report_name=["Điện não đồ","Điện não đồ (in ngang)"];
$(document).ready(function() {
	...
	$("#xemin").click(function(e){	
		...
		if($( "#chonanh option:selected" ).text()=="In dọc")
		{
			print_action="xemin";			
			$.cookie("in_status", "print_preview"); 
			if($('#In_CoChuKy input:checkbox').is( ":checked" ))
			{		
				dialog_pic_select("Chọn ảnh",90,"170px","jdshju8789789",window.search_string,"EEG",print_action,"PHIẾU ĐIỆN NÃO ĐỒ ĐIỆN TOÁN","ENCEPHALO - ELECTROGRAPHY","BÁC SỸ CHẨN ĐOÁN","left",3,"EEG",'<?=$modules?>',"780");
			}else{
				dialog_pic_select("Chọn ảnh",90,"170px","jdshju8789789",window.search_string,"EEG",print_action,"PHIẾU ĐIỆN NÃO ĐỒ ĐIỆN TOÁN","ENCEPHALO - ELECTROGRAPHY","BÁC SỸ CHẨN ĐOÁN","left",3,"EEG_khongchuky",'<?=$modules?>',"780");
				}
		...
		$(".frame_u78787878975f").css("width","793px");		
			...
			print_action="xemin";
			dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=EEG_InNgang&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+_kham+"&type=report&id_form=10&tieude=PHIẾU ĐIỆN NÃO ĐỒ ĐIỆN TOÁN&chucnang=ENCEPHALO - ELECTROGRAPHY&chucdanh=BÁC SỸ CHẨN ĐOÁN&chucdanh2=KỸ THUẬT VIÊN",'EEG_InNgang');	
		}
	});
	...
	create_combobox('#nhanvien', '#nhanvien1', ".nhan_vien", "#nhan_vien", create_nhanvien, 500, 400, 'Người thực hiện', 'data_nhanvien',0);
	create_combobox('#chuandoan', '#chuandoan1', ".nhan_vien1", "#nhan_vien1", create_bacsi, 500, 400, 'Người thực hiện', 'data_bacsi',0);
	$('#sua').button();
	$('#luu').button();
	$('#xemin').button();
	$('#dong').button();
	...
	resize_control();
	if(id_kham2!="0"){
		$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_diennao2&idkham="+id_kham2+"&idbenhnhan="+id_benhnhan, 
			function( data ) {
				data_luotkham=data;
	...
	$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_diennao&idbenhnhan="+id_benhnhan, 
		function( data ) {
			data_luotkham=data;
...
function resize_control(){
	...
	$("#ketluan").css("height",$(".ui-layout-east").height()-38+"px");
