...
<div id="dialog" title="Thông báo" style="display:none">
 ...
 <div id="panel_main">    
        ...
        <div id="prowed3"><div id="duoi">
        <input type="text" style="margin-left: 130px; margin-top: 1px; width: 88px;background:#4CA109;border:1px solid #327E04; color: #FFFFFF;font-weight: bold; text-align:right;  box-shadow:none !important;" disabled="" readonly value="0" class="disable" id="r_ps">
        <input type="text" style="margin-left: 0px; margin-top: 1px; width: 88px;background:#4CA109;border:1px solid #327E04; color: #FFFFFF;font-weight: bold; text-align:right;  box-shadow:none !important;" disabled="" readonly value="0" class="disable" id="r_pd">
        <input type="text" style="margin-left: 0px; margin-top: 1px; width: 88px;background:#4CA109;border:1px solid #327E04; color: #FFFFFF;font-weight: bold; text-align:right;  box-shadow:none !important;" disabled="" readonly value="0" class="disable" id="r_hr">
        </div></div> 
        <div class="luutt">
...
var report_code=["InHuyetAp"];
var report_name=["Huyết áp 24h"];
...
window.nhanvien_complete=0;
	...
	$("#them").click(function(){
		...
		if(typeof tmp2[tmp2.length-1]!='undefined'){
			...
			selRowId = $("#rowed3").jqGrid ('getGridParam', 'selrow');
			$("#"+selRowId+"_GioDo").focus();
			
			$("#"+selRowId+"_GioDo").keyup(function(e) {
				if (e.keyCode === 13) {
					$("#"+selRowId+"_Ps").focus();
				}
			});
			$("#"+selRowId+"_Ps").keyup(function(e) {
				if (e.keyCode === 13) {
					$("#"+selRowId+"_Pd").focus();
				}
			});
			$("#"+selRowId+"_Pd").keyup(function(e) {
				if (e.keyCode === 13) {
					$("#"+selRowId+"_Hr").focus();
				}
			});
			$("#"+selRowId+"_Hr").keyup(function(e) {
				if (e.keyCode === 13) {
					$("#luu").focus();
				}
			});
			  ...
			  $("#sua").click(function(){
			  	...
			  	//$('#sua').button( {disabled:true});
					...
					for(var i=0;i<tmp1.length;i++ ){
						jQuery("#rowed3").jqGrid('editRow',tmp1[i]);
						$('#'+tmp1[i]+'_GioDo').timeEntry({show24Hours: true});
						if($("#"+tmp1[i]+"_Ps").val()>=140 || $("#"+tmp1[i]+"_Ps").val()<=69){
							$("#"+tmp1[i]+"_Ps").css( "color", "red" );
						}else if($("#"+tmp1[i]+"_Ps").val()>=90 && $("#"+tmp1[i]+"_Ps").val()<=129){
							$("#"+tmp1[i]+"_Ps").css( "color", "#6BC4F3" );
						}else{
							$("#"+tmp1[i]+"_Ps").css( "color", "#FF8C00" );
							}
						$("#"+tmp1[i]+"_Pd").css( "color", "red" );
						if($("#"+tmp1[i]+"_Hr").val()>=101 || $("#"+tmp1[i]+"_Hr").val()<=59){
							$("#"+tmp1[i]+"_Hr").css( "color", "red" );
						}else{
							$("#"+tmp1[i]+"_Hr").css( "color", "#6BC4F3" );
							}
					}
	...
	$.each( data_luotkham, function( key, val ) {
				...
				for(i=0;i<val.length;i++){
                    ...
                    $("#giohentra").html(tam1_cls[11]);
						...
						if(tam1_cls[9]!=null)
						{
								$("#nguoisua").text(tam1_cls[12]);
								$("#ngaygiosua").text(tam1_cls[13]);
						}
						else $("#edit_by").hide();					
					} 
				}
            ...
            loadComplete: function(data) {
				...
				for(var i=0;i<tmp1.length;i++ ){
					if(_id_trangthai=='DangCho' || _id_trangthai=='DangKham'){
						...
						if(rowData['Ps']>=140 || rowData['Ps']<=69){
							$('#rowed3').jqGrid('setCell',tmp1[i],'Ps','',{color:'red'});
						}else if(rowData['Ps']>=90 && rowData['Ps']<=129){
							$('#rowed3').jqGrid('setCell',tmp1[i],'Ps','',{color:'#6BC4F3'});
						}else{
							$('#rowed3').jqGrid('setCell',tmp1[i],'Ps','',{color:'#FF8C00'});
							}
						$('#rowed3').jqGrid('setCell',tmp1[i],'Pd','',{color:'red'});
						
						if(rowData['Hr']>=101 || rowData['Hr']<=59){
							$('#rowed3').jqGrid('setCell',tmp1[i],'Hr','',{color:'red'});
						}else{
							$('#rowed3').jqGrid('setCell',tmp1[i],'Hr','',{color:'#6BC4F3'});
							}
					}
	...
	var nguoihoantat=$("#chuandoan1").val();
	   ...
	   postData=$.parseJSON(postData);
		...
		tooltip_message("Lưu dữ liệu thành công");
			 if(window.n_xemin==true){
				_in();
			 } 
			window.n_xemin=false;
		  })//$.post
	}
...
function draw_chart_dauhieusinhton(){
	...
	$("#right_top").append('<div class="diengiai_sinhton"><div id="ps"></div>Ps<div id="pd"></div>Pd<div id="hr"></div>Hr  </div><div style="height:5px"></div><div class="dauhieusinhton" id="dauhieusinhton" style="width:85%; margin-left:5px;"></div>');	
    // alert($("#inner-top").height());
	//$(".diengiai").css("height",
	$(".dauhieusinhton").css("height",($("#right_top").height()-13)+"px");
	$(".dauhieusinhton").css("width",($("#right_top").width()-16)+"px");	
...
function chart_sinhton  (){
	...
	for(var i=0;i < tmp1.length;i++){ 
		...
		labelx.push({value:(i+1), text:rowData["GioDo"]});		
...
function _in(){
	$.cookie("in_status", "print_preview"); 
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=huyetap24h&header=left&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10&chuky="+chuky,'InHuyetAp');	
	}
function callback(){
