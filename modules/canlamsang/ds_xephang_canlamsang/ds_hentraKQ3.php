...
function create_grid(){	
	 ...
	 $("#rowed3").jqGrid({
		  url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_hentrakq&idluotkham=257722',
		datatype: "json",	
	...
	//
		...
		$.get( "resource.php?module=web_services&function=create_panel&action=index&id_benhnhan="+idbenhnhan, function( data ) {
...
function   save2(){
		$("#save2").bind("click", function(e) { 
				...
				if($('#datepicker1').val()==''&&rowData['NgayGioTraKQ']!='')
					...
					{
						$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_trakq_update&ID_LuotKham='+rowData["ID_LuotKham"]+'&date_NgayGioTraKQ='+$("#datepicker1").val()+'&time_NgayGioTraKQ='+$("#timepicker1").val()).done(function(data){ 
							if ($.trim(data) == '') {
								tooltip_message("<?php get_text1("sua_thanhcong") ?>");
								$('#rowed3').setGridParam({loadonce: false, datatype: "json"}).trigger('reloadGrid');
							}
