...
<div id="main_content">
	...
	<input id='com_thuocnhom' class='com_thuocnhom' >
    </span>
...
jQuery(document).ready(function() {					
	...
	$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_dm_control_tenform').done(function(data) {
		...
		$.ajax({
			type: 'POST',
			async : true,
			url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_xephang&gr=false',
	...
	//setTimeout(function(){resize_control()},3000);
...
function create_grid(data) {
	...
	$("#rowed3").jqGrid({
		...
		{name: 'TenForm', index: 'TenForm', search: false, width: "8%",  align: 'center', hidden: false,edittype:"select",formatter:'select',editoptions:{value: tenform}},
		...
		shrinkToFit: true,
			...
			onSelectRow: function(rowId) { 
				...
				var rowData = jQuery(this).getRowData(rowId); 
				 ...
				 /*var landau_rowed3= $("#landau_rowed3").val();
				 if(landau_rowed3==0){				 
...
function create_grid2()
{
	$("#rowed4").jqGrid({
		...
		{name: 'TenForm', index: 'TenForm', search: false, width: "15%",  align: 'center', hidden: false,edittype:"select",formatter:'select',editoptions:{value: tenform}},
		...
		shrinkToFit: true,
			...
			$("#rowed4").setGridParam({loadonce: true});
                ...
                var caption= 'Bệnh nhân đang khám ('+ jQuery("#rowed4").jqGrid('getGridParam', 'records')+')'+' [ <span class="demgio"> Tự động reload sau ' + reload_luoi_danhsach + ' giây</span> ] <div class="grid1"><div class="hinhvuong quathoigian_max"></div><label class="diengiai">< 15\' nữa đến giờ hẹn KQ hoặc đã quá hẹn</label></div>  <div class="grid1 grid2"><div class="hinhvuong quathoigian_min"></div><label class="diengiai">< 30\' nữa đến giờ hẹn KQ</label></div>';
...
function create_grid4()
{
	$("#rowed6").jqGrid({
		datastr: {},
		datatype: "jsonstring",
		colNames: ['LuotKham','Mã BN','Họ tên','Tuổi','Giới','Hạng mục khám','Giờ h.tất','Ngày giờ tạo','Tên form','Trạng thái','ID bệnh nhân'],
		colModel: [
	...
	$("#rowed7").jqGrid({
		...
		groupingView : { 
			...
			groupText : ['<b>{0} - {1} hạng mục </b>']
			 ...
			 onSelectRow: function(rowId) { 
			// alert()
				//var landau_rowed7= $("#landau_rowed7").val();
				// if(landau_rowed7==0){
				/*	 
					document.getElementById("tieptucxephang").style.display="block";
					document.getElementById("dungxephang").style.display="none";
		...
		timer1 = setInterval(function() {
			if (window.kiemtrasearch == true) {
				...
				if (reload_luoi_danhsach == bodem) {
						...
						type: 'POST',
						async : true,
						url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_xephang',					
						success: function(output, status, xhr) {		
							output=output.split(';;');
							$('#rowed3').jqGrid('setGridParam', { datatype: 'jsonstring', datastr:output[0]  }).trigger('reloadGrid', [{ page: 1}]);
							$('#rowed4').jqGrid('setGridParam', { datatype: 'jsonstring', datastr:output[1]  }).trigger('reloadGrid', [{ page: 1}]);
							$('#rowed6').jqGrid('setGridParam', { datatype: 'jsonstring', datastr:output[2]  }).trigger('reloadGrid', [{ page: 1}]);							loading=false;
						    demclick=0;
							//$('#rowed7').jqGrid('setGridParam', { datatype: 'jsonstring', datastr:output[3]  }).trigger('reloadGrid', [{ page: 1}]);
						},
