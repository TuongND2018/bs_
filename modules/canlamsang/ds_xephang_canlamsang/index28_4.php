...
jQuery(document).ready(function() {					
	...
	$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_dm_control_tenform').done(function(data) {
		...
		create_grid5();
		resize_control();
		window.duongdandata='resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_xephang';
            ...
            loadComplete: function(data) {
				...
				for (i = 0; i < ids.length; i++) {
					...
					thoigianMax=thoigianMax.addMilliseconds(-500);
								...
								var caption= 'Bệnh nhân chưa khám ('+ jQuery("#rowed3").jqGrid('getGridParam', 'records')+')'+' [ <span class="demgio"> Tải lại sau ' + reload_luoi_danhsach + ' giây</span> ] <span id="n_search"></span>';
...
function create_grid2()
{
	$("#rowed4").jqGrid({
		...
		colModel: [
		{name: 'ID_Kham', index: 'ID_Kham', search: false, width: "20%",  align: 'left', hidden: false},
		{name: 'ID_LuotKham', index: 'ID_LuotKham', search: false, width: "20%",  align: 'left', hidden: true},
		{name: 'MaBenhNhan', index: 'MaBenhNhan', search: true, width: "20%",  align: 'left', hidden: false},
		{name: 'TenBenhNhan', index: 'TenBenhNhan', search: true, width: "40%",  align: 'left', hidden: false},
		...
		shrinkToFit: true,
			...
			$("#rowed4").setGridParam({loadonce: true});
                ...
                var caption= 'Bệnh nhân đang khám ('+ jQuery("#rowed4").jqGrid('getGridParam', 'records')+')'+' <div class="grid1"><div class="hinhvuong quathoigian_max"></div><label class="diengiai">< 15\' nữa đến giờ hẹn KQ hoặc đã quá hẹn</label></div>  <div class="grid1 grid2"><div class="hinhvuong quathoigian_min"></div><label class="diengiai">< 30\' nữa đến giờ hẹn KQ</label></div>';
...
function create_grid4()
{
	$("#rowed6").jqGrid({
		datastr: {},
		datatype: "jsonstring",
		colNames: ['ID_Kham','Mã BN','Họ tên','Tuổi','Giới','Hạng mục khám','Giờ h.tất','Ngày giờ tạo','Tên form','Trạng thái','ID bệnh nhân','ID_LuotKham'],
		colModel: [
		{name: 'ID_Kham', index: 'ID_Kham', search: false, width: "2%",  align: 'left', hidden: true},
		{name: 'MaBenhNhan', index: 'MaBenhNhan', search: true, width: "2%",  align: 'left', hidden: false},
		{name: 'TenBenhNhan', index: 'TenBenhNhan', search: true, width: "5%",  align: 'left', hidden: false},
        ...
        sortorder: "asc",
			...
			ondblClickRow: function(rowId, rowIndex, columnIndex) {				
				if(loading==true){
					if(demclick==0){
						var rowData = jQuery(this).getRowData(rowId); 
						mogoikham(rowId,rowData["NgayGioTao"],rowData["TenLoaiKham"],rowData["TenForm"],rowData["ID_TrangThai"],"rowed6");
					}
					demclick++;
				}else{
					var rowData = jQuery(this).getRowData(rowId); 
...
function create_grid5()
{
	...
	$("#rowed7").jqGrid({
		datastr: {},
		datatype: "jsonstring",
		colNames: ['ID khám','Mã BN','Họ tên','Tuổi','Giới','Hạng mục khám','Giờ h.tất','Ngày giờ tạo','Tên form','Trạng thái','ID bệnh nhân','ID Lượt khám'],
		colModel: [
		...
		groupingView : { 
            ...
            ondblClickRow: function(rowId, rowIndex, columnIndex) {
				if(loading==true){
					if(demclick==0){
						var rowData = jQuery(this).getRowData(rowId); 
						mogoikham(rowId,rowData["NgayGioTao"],rowData["TenLoaiKham"],rowData["TenForm"],rowData["ID_TrangThai"],"rowed7");
					}
					demclick++;
				}else{
					var rowData = jQuery(this).getRowData(rowId); 
...
function create_bs(elem,datalocal){ 
	...
	$(elem).jqGrid({
		...
		onSelectRow: function(id){	
			...
			window.duongdandata='resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_xephang_theonhom&id='+selr;
			taidulieu(window.duongdandata);
			/*$.ajax({
				type: 'POST',
				async : true,
				url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_xephang_theonhom&id='+selr,
...
function create_bs2(elem,datalocal){ 
	...
	$(elem).jqGrid({
		...
		onSelectRow: function(id){
			...
			document.getElementById("thuocnhom").checked=false;
			document.getElementById("thuochangmuckham").checked=true;
			window.duongdandata='resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_xephang_theophongban&id='+selr;
			taidulieu(window.duongdandata);
			/*$.ajax({
				type: 'POST',
				async : true,
...
$("#menu2_chitiethangmuckham").bind( "click", function(e) {
	$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=check_quyen_goikham&id_control=793&id_nhanvien=<?=$_SESSION["user"]["id_user"]; ?>').done(function(data)
	{
		if(data==1){
			var selr = jQuery('#rowed4').jqGrid('getGridParam','selrow');
...
function mogoikham(id,ngaygiotao,loaikham,idform,trangthai,luoi){
	//window.tenform
	//alert(id+'__'+ngaygiotao+'__'+loaikham+'__'+idform+'__'+trangthai+'__'+luoi);
...
//	alert(idkham+'__'+tenform+'__'+luoi)
	if(luoi=="rowed3"){
...
function resize_control() {
	...
	$("#rowed3").setGridHeight($('.n_tren').height()-72);
	$("#rowed4").setGridHeight($('.n_duoi').height()-109);
...
function taidulieu2(duongdan){
	...
	$.ajax({
		...
		success: function(output, status, xhr) {		
			...
			$('#rowed3').jqGrid('setGridParam', { datatype: 'jsonstring', datastr:output[0]  }).trigger('reloadGrid', [{ page: 1}]);
			$('#rowed4').jqGrid('setGridParam', { datatype: 'jsonstring', datastr:output[1]  }).trigger('reloadGrid', [{ page: 1}]);
			$('#rowed6').jqGrid('setGridParam', { datatype: 'jsonstring', datastr:output[2]  }).trigger('reloadGrid', [{ page: 1}]);							setTimeout(function(){
				loading=false;
				demclick=0;
				},300)							
			//$('#rowed7').jqGrid('setGridParam', { datatype: 'jsonstring', datastr:output[3]  }).trigger('reloadGrid', [{ page: 1}]);
