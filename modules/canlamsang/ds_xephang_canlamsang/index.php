...
function create_grid() {
	...
	$("#rowed3").jqGrid({
		datastr:{},
		datatype: "jsonstring",
		colNames: ['LuotKham','ID loại khám', 'Mã BN', 'Họ tên', 'Tuổi', 'Giới tính', 'Hạng mục khám', 'Giờ hẹn', 'Giờ chỉ định','Chỉ định','Ghi chú','Gọi khám','NotesStatus','ID Bệnh nhân ','Tên Form','ID loại khám '],
		colModel: [
...
function create_grid2()
{
	$("#rowed4").jqGrid({
		datastr: {},
		datatype: "jsonstring",
		colNames: ['ID khám','ID Lượt khám', 'Mã BN', 'Họ tên', 'Tuổi', 'Giới', 'Hạng mục khám', 'Giờ c.định', 'Giờ hẹn KQ','Chỉ định','T.hiện','Ghi chú','Nhập KQ','Trạng thái','NotesStatus','ID Bệnh nhân','Tên form'],
		colModel: [
...
function create_grid4()
{
	$("#rowed6").jqGrid({
		datastr: {},
		datatype: "jsonstring",
		colNames: ['ID_Kham','Mã BN','Họ tên','Tuổi','Giới','Hạng mục khám','H.tất','Ngày giờ tạo','Tên form','Trạng thái','ID bệnh nhân','ID_LuotKham'],
		colModel: [
...
function create_grid5()
{
	...
	$("#rowed7").jqGrid({
		datastr: {},
		datatype: "jsonstring",
		colNames: ['ID khám','Mã BN','Họ tên','Tuổi','Giới','Hạng mục khám','H.tất','Ngày giờ tạo','Tên form','Trạng thái','ID bệnh nhân','ID Lượt khám'],
		colModel: [
		{name: 'ID_Kham', index: 'ID_Kham', search: false, width: "10%",  align: 'left', hidden: true},
		{name: 'MaBenhNhan', index: 'MaBenhNhan', search: true, width: "10%",  align: 'left', hidden: false},
	...
	if(luoi=="rowed3"){
	 ...
	 var TenLoaiKham = rowData['TenLoaiKham'];
	 	if(TenLoaiKham=='FRAMINGHAM'){
		 	parent.postMessage('canlamsang;Framingham;'+id_benhnhan+';'+TenBenhNhan+';'+idkham, "*");	
		 }else if(TenLoaiKham=='Siêu âm thai 4 chiều'){
		 	parent.postMessage('canlamsang;sieu_am_4d;'+id_benhnhan+';'+TenBenhNhan+';'+idkham, "*");
		 }else{
			 parent.postMessage('canlamsang;'+tenform+';'+id_benhnhan+';'+TenBenhNhan+';'+idkham, "*");	
			 }
