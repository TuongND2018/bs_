...
<?php
	if(isset($_GET["id_benhnhan"])){
		...
		echo "</script>";
		$_n_idbenhnhan=$_GET["id_benhnhan"];
	...
	if($_SESSION["ThongTinBenhNhan"]["ID"]==""){
		...
		echo "</script>";
		$_n_idbenhnhan=$_SESSION["ThongTinBenhNhan"]["ID"];
	}
	}
...
<?php
	...
	$store_name="{call GD2_GetTTBenhNhanByID_BenhNhan(?)}";
	$params = array($_n_idbenhnhan); 
	$get_ttbn=$data->query( $store_name, $params);
	$excute= new SQLServerResult($get_ttbn);
	$tam= $excute->get_as_array();
	echo "var pname='".create_name($tam[0]['HoTenBN'])."';\n"; //$_n_idbenhnhan
	echo "var page='".$tam[0]['Tuoi']."';\n";
	echo "var psex='".$tam[0]['Gioi']."';\n";
	echo "var paddress='".stringUnUtf8($tam[0]['DiaChi'])."';\n";
