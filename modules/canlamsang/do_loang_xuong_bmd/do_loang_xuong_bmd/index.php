...
<body>
 ...
 <div class="top_form ui-widget-content" >
    ...
    <div class="thongtin_luotkham" id="left_col" style="width: 604px;">   
    	<div class="thongtin_luotkham_scroll " style="display:none"></div>
        	<span id="caption" style="font-size:17px;color:#09C;margin-left:5px;margin-top:5px; font-weight:bold">Đo mật độ xương</span>
            <br><br>
 ...
 <table id="rowed111" cellspacing="30px"  style="width:88%;height:65%;margin:auto;">
 	<tr>
...
$(document).ready(function() {	
	...
	$("#xemin").click(function(e){		
		...
		//var tieude='',chucnang='';
		//$.post("resource.php?module=report&view=<?=$modules?>&action=BMD&type=report&id_form=10&id_kham="+id_kham);
		$.cookie("in_status", "print_preview"); 
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=BMD&header=left&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10&tieude=&chucnang=&chucdanh=",'BMD');
	});
	...
	if(id_kham2!="0"){
		$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_doloangxuong2&idkham="+id_kham2+"&idbenhnhan="+id_benhnhan, 
			function( data ) {
				data_luotkham=data;
...
//alert(_id_trangthai); 
	...
	$.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_doloangxuong&idbenhnhan="+id_benhnhan,
		function( data ) {
			data_luotkham=data;
	 	//alertObject(data);
		var tam1_cls="";
		...
		$("#open_form_hentra").click(function(e){
			  ...
			  $("#dathuchien").click(function(){
                 			...
                 			_id_trangthai="DaThucHien";
						      ...
						      $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=dathuchien&hienmaloi=s',dataToSend)
...
function get_min_val(){
	var val_1=	parseFloat($('#bmd_chanphai').val().replace(",","."));
	//alert(val_1);
	
	var val_2=	parseFloat($('#bmd_chantrai').val().replace(",","."));
	
	var val_3=	parseFloat($('#bmd_tayphai').val().replace(",","."));
	
	var val_4=	parseFloat($('#bmd_taytrai').val().replace(",","."));
