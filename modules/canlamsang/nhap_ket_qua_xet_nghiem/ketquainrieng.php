...
$(document).ready(function(){
  ...
  $("#luu_kqir").click(function(){
	  phancach = "";
		...
		$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luu_kqir&hienmaloi=3&id_kham=<?=$_GET["id_kham"]?>&ketqua='+									dataToSend)
	...
	$("#inkqir").click(function(e){	
		...
		dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action="+window.tenclass+"&header=top&id_benhnhan=<?=$_GET["id_benhnhan"]?>&id_kham="+<?=$_GET["id_kham"]?>+"&type=report&id_form=10",'XetNghiem');
