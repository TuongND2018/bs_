			...
			<div class="thongtinchinh" style="width:99%; margin-top:5px;margin-left:4px;">
				<div class=" ui-widget ui-widget-content ui-corner-all ui-resizable panel_form" >
                    ...
                    <div class="ui-dialog-content ui-widget-content" style="width: auto; min-height: 0px; max-height: none; height: 100%; margin-top:20px">
                        ...
                        <a  id="lichbacsy" class="ui-button ui-widget ui-state-default ui-corner-all  fm-button-icon-left "   > Lịch bác sỹ<span class="ui-icon ui-icon-calculator"></span></a>
                        </div>
      ...
      buttons: {
        Cancel: function() {
          ...
          $( this ).dialog( "close" );
			 ...
			 "ok": function() {
					$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_trakq_update&ID_LuotKham='+rowData["ID_LuotKham"]+'&date_NgayGioTraKQ='+$("#datepicker1").val()+'&time_NgayGioTraKQ='+$("#timepicker1").val()+'&xoaID=1').done(function(data)
		{ 
			if ($.trim(data) == '') {
...
function create_grid(){	
	 ...
	 $("#rowed3").jqGrid({
		  url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_hentrakq&idluotkham=<?=$_GET["idluotkham"] ?>',
		datatype: "json",	
	...
	//
		...
		$.get( "resource.php?module=web_services&function=create_panel1&action=index&id_benhnhan="+idbenhnhan, function( data ) {
...
function load_select(){
	...
	window.hoten2 = $.ajax({url: "resource.php?module=web_services&function=get_auto_edit_hovaten&action=index", async: false, success: function(data, result) {if (!result) alert('Không load được danh mục nhân viên');}}).responseText;		
}
...
function   save(){
		$("#save").bind("click", function(e) {  
				...
				{
					$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_hentrakq_update&ID_LuotKham='+rowData["ID_LuotKham"]+'&date_NgayGioHenTraKQ='+$("#datepicker2").val()+'&time_NgayGioHenTraKQ='+$("#timepicker2").val()).done(function(data)
					{
						if ($.trim(data) == '') {
							tooltip_message("<?php get_text1("sua_thanhcong") ?>");
							$('#rowed3').setGridParam({loadonce: false, datatype: "json"}).trigger('reloadGrid');
...
function   save2(){
		$("#save2").bind("click", function(e) { 
				...
				if($('#datepicker1').val()==''&&rowData['NgayGioTraKQ']!='')
					...
					{
						$.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&xoaID=0&action=data_trakq_update&ID_LuotKham='+rowData["ID_LuotKham"]+'&date_NgayGioTraKQ='+$("#datepicker1").val()+'&time_NgayGioTraKQ='+$("#timepicker1").val()).done(function(data){ 
							if ($.trim(data) == '') {
								tooltip_message("<?php get_text1("sua_thanhcong") ?>");
								$('#rowed3').setGridParam({loadonce: false, datatype: "json"}).trigger('reloadGrid');
							}
