...
$params = array('noi_soi',$_GET["idbenhnhan"]);//tao param cho store 
$store_name="{call GD2_KhamGetDataByTenFormAndIDBenhNhan_NoiSoi(?,?)}";//tao bien khai bao store
...
foreach ($tam as $row) {//duyet toan bo mang tra ve
    ...
    if($row["NgayGioHenTraKQ"]!='')
     ...
     if($row["NgayGioThucHien"]!='')
            $row["NgayGioThucHien"]=$row["NgayGioThucHien"]->format("H:i d/m/Y");
    else $row["NgayGioThucHien"]='';
    if($row["NgayGioChanDoan"]!='')
            $row["NgayGioChanDoan"]=$row["NgayGioChanDoan"]->format("H:i d/m/Y");
    else $row["NgayGioChanDoan"]='';
    $responce->rows[$i]['id']=$row["ID_Kham"];
    $responce->rows[$i]['cell']=array(
	$row["MaVietTat"],
	$row["TenLoaiKham"],
	$row["NgayGioTao"]->format("H:i d/m/Y"),
	$row["ID_LoaiKham"],
	$row["BSChiDinh"],
	$row["ID_LuotKham"],
	$row["MoTa"],
