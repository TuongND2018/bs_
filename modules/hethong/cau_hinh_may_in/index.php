...
//}
	var quyenxemfullreport=0;
	 ...
	 $("#rowed3").jqGrid({
		url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=data&quyenxemfullreport='+quyenxemfullreport,
		datatype: "json",	
		...
		colModel:[
			...
			{name:'UserWindowName',index:'UserWindowName', width:"10%", editable:true,align:'left',hidden:false,editrules: {required:true},edittype:"text",formoptions:{ rowpos:4, colpos:1}}, 
	...
	$("#rowed3").jqGrid('navGrid',"#prowed3",{add: permission["add"], edit: permission["edit"], del: permission["delete"], search: false,closeAfterEdit: true,clearAfterAdd :true,closeOnEscape : true, bSubmit: "Submit",bCancel: "Cancel"}, //options						 
		{recreateForm:true,height:'auto',width:'auto',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=edit',closeAfterEdit:true,closeOnEscape : true,modal: true,recreateForm: true,
		...
		{height:'auto',width:'auto',reloadAfterSubmit:false,url:'resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=add',closeOnEscape : true,closeAfterAdd:true,modal: true,addedrow: "first",recreateForm:true,savekey: [true,121]
				,afterSubmit : function(response, postdata){
...
//					window.id_rowed3++; 
	...
	$("#rowed3").jqGrid('navButtonAdd', '#prowed3', {caption: "Tạo cấu hình tự động", buttonicon: 'ui-icon-bookmark',id : 'cauhinh_rowed3',
            onClickButton: function() {
				 ...
				 $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=taocauhinh&domain='+$.cookie("domain")+'&TenMay='+$.cookie("username_window")).done(function(){
...
function load_select(){ 
	...
	window.province = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=ReportsList&id=ReportName&name=Description', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban');}}).responseText;	
	//alert(province);
}

//function load_select1(){
//	window.province1 = $.ajax({url: 'resource.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=ReportsList&id=ReportName&name=Description', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục quốc tịch');}}).responseText;	
