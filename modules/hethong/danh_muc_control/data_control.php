...
function display_node($parent, $level) {
  ...
  foreach ($tam2 as $row) {
	...
	$responce[] = array('ID_Control' => $row["ID_Control"],'ID_Parent' => $row["ID_Parent"], 'Caption' => $row["Caption"], 'TenControl' =>$row["TenControl"],'ID_Form' =>$row["ID_Form"],'IsBarButton' => $row["IsBarButton"] ,'KieuControl' => $row["KieuControl"],'Icon' =>$row["Icon"],'Active' =>$row["Active"],'STT' =>$row["STT"],'PageOpen' =>$row["PageOpen"],'OpenType' =>$row["OpenType"] ,'ChoPhepMoNhieuTab' =>$row["ChoPhepMoNhieuTab"] ,'PhimTat' =>$row["PhimTat"] , 'level' => $level, 'parent' => $valp, 'isLeaf' => $leaf, 'expanded'  => "true", 'loaded'  => "true");
