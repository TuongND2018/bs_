...
function create_grid() {
  ...
  $("#rowed3").jqGrid({
    ...
    datatype: "json",
    colNames: ['','MaLoaiHD','Tạo lúc', 'Mã BN', 'Họ tên','Số HĐ','Địa chỉ hóa đơn','Đối tượng','Phải trả','Miễn giảm','Nội dung','Chi tiết bảng kê','MaLoaiHD','','id thuTN','id_thue','Người hủy','Giờ hủy',''],
    colModel: [
...
function create_grid14() {
  ...
  $("#rowed14").jqGrid({
    ...
    datatype: "json",
    colNames: ['LuotKham', 'Mã BN', 'Họ lót','Tên BN','Địa chỉ','Đối tượng','Giờ đến','Phải trả','Miễn giảm','Tiền thuốc','Tiền khám CB',''],
    colModel: [
...
$('.footrow td[aria-describedby="rowed14_HoLotBenhNhan"]').append('<input type="button" value="Lập bảng kê" onclick=save()> ');
...
function create_grid15(datalocal){
  ...
  $("#rowed15").jqGrid({
    datastr:datalocal,
    datatype: "jsonstring" ,
    colNames: ['LuotKham', 'Mã BN', 'Họ lót','Tên BN','Địa chỉ','Đối tượng','Giờ đến','Phải trả','Miễn giảm','Tiền thuốc','Tiền khámCB','ID_ThuTNo','SOHD'],
    colModel: [
    {name: 'ID_LuotKham', index: 'ID_LuotKham', width: "0%", editable: false, align: 'left', hidden: true},
    {name: 'MaBenhNhan',width: "2%",  classes: "CotSoTien", sortable: true,sorttype: 'number', index: 'MaBenhNhan', search: true, editable: false, align: 'left', hidden: false},
