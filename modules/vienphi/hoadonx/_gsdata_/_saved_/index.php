...
alert(_sumTienThuoc);
     ...
     dialog_main("Đang chọn loại "+ $("#com_thuochmk").val() +  '   ', 90, 90, "56743265743657", "resource.php?module=vienphi>&view=hoadon&action=HDChiTiet&type=test&id_form=11&ID_ThuTNo="+0+"&phanloaiHD="+window.loaiHD);
...
function  addItem()
{
 ...
 for(i=0;i<row.length;i++){
  ...
  //_IDThuTraNo_Array[i]=rowData["ID_ThuTraNo"]

 ////_IDThuTraNo_Array.push(rowData["ID_ThuTraNo"]);
...
function  subItem()
{
 ...
 for(i=0;i<row.length;i++){
  ...
  _sumTienThuoc-=parseInt(rowData["TienThuoc"]);
  _sumTienGiam-=parseInt(rowData["TongMienGiam"]);
 //_IDThuTraNo_Array.splice(rowData["ID_ThuTraNo"]);
...
function create_grid15(datalocal){
  ...
  $("#rowed15").jqGrid({
    datastr:datalocal,
    datatype: "jsonstring" ,
    colNames: ['LuotKham', 'Mã BN', 'Họ lót','Tên BN','Địa chỉ','Đối tượng','Giờ đến','Phải trả','Miễn giảm','Tiền thuốc','Tiền khámCB','SOHD'],
    colModel: [
    {name: 'ID_LuotKham', index: 'ID_LuotKham', width: "0%", editable: false, align: 'left', hidden: true},
    {name: 'MaBenhNhan',width: "2%",  classes: "CotSoTien", sortable: true,sorttype: 'number', index: 'MaBenhNhan', search: true, editable: false, align: 'left', hidden: false},
