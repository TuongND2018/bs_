...
/* if(isset($_GET["loaiHD"])){

        echo "window.loaiHD=".$_GET["loaiHD"];
   ...
   /* @id_thutrano INT,
	...
	@NguoiMuaHang nvarchar(100),
	@MaLoaiHD tinyint*/

//kiểm tra có  id_soHD  thì update 
...
/*	if($_POST['iddonthuoc']!=0){
		if($_POST['laythuoc']=='false')	{
			...
			if(isset($_POST["tien_miengiam"])){
				if($_POST['check_miengiamthuoc']=='true')	{
					$store_name9="{call GD2_donthuoc_tienmiengiam (?,?)}";
					$params9 = array(
						$_POST['miengiamthuoc']+$_POST['bsgiamthuoc'],//   @ID_LuotKham int,
						$_POST['iddonthuoc']
						);
					$get=$data->query( $store_name9, $params9);
					if( !$get ){
