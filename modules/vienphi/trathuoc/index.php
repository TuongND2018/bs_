...
?>
 <?php 
		...
		if($_GET['oper']=='edit'){
			$data= new SQLServer;	
			$store_name="{call GD2_thutrano_gettienvon(?)}";
			...
			echo "window.ID_ThuTraNo=".$id_benhnhan.";";
			echo "</script>";	
...
setTimeout(function(){$("#tienthu").focus()},500); ;
	...
	$('#id_print').keydown(function(e){
			...
			if($('#id_print').val()==4){
			 ...
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=hoan_ung&header=top&lien="+$('#id_print').val()+"&type=report&tenreport=1&id_form=10&idthutrano="+window.ID_ThuTraNo,'InPhieuChi');
			 $('#dialog_print').dialog('close');
			}
  ...
  /*  if(isset($_GET['oper'])){
   ...
   $('#diengiai').val('<?php
		...
		if($_GET['oper']=='edit'){
			...
			echo 'Hoàn tiền theo phiếu trả thuốc '.$_GET['MaPhieu'].', ngày lập: '.$_GET['NgayLapPhieu'].' Tổng tiền trả lại:'.$_GET['TienThuocTraLai'];
		}
   ?>');
   ...
   $('#tienthuoc').val('<?php   
		 if($_GET['oper']=='add'){		
			...
			echo number_format((intval($tam[0]['SoTienTraLai'])), 0, '', '.');
		}
   ?>');
   ...
   $('#tienvon').val('<?php    
		if($_GET['oper']=='add'){		
			...
			echo number_format((round($tam[0]['tienvon'])), 0, '', '.');
		}
   ?>');
   ...
   $('#btn_luu').click(function(){	   
    $('#dialog_sotien').html('Số tiền thu là '+$('#tienthu').val()+'. Bạn có muốn tiếp tục')	;
	   ...
	   $.cookie("in_status", "print_preview"); 
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=hoan_ung&header=top&tenreport=1&lien=2&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
			 $('#dialog_print').dialog('close');
	   
	   })
	      $('#btn_lien2').click(function(){
	   $.cookie("in_status", "print_preview"); 
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=hoan_ung&header=top&tenreport=1&lien=3&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
			 $('#dialog_print').dialog('close');
...
function luu(){
	if(dem==0){
	 dem++;
	    var datatosend = '{"idbenhnhan":"'+id_benhnhan+'","NgayGio":'+JSON.stringify($('#ngaylap').val())+',"nguoigd":"'+$('#nguoigd').val()+'","diachi":"'+$('#diachi').val()+'","diengiai":'+JSON.stringify($('#diengiai').val())+',"tienthu":"'+$('#tienthu').val().split('.').join('')+'","nocu":"'+$('#nocuoi').val()+'","id_nhapkho":"<?=$_GET['ID_NhapKho']?>","tienthuoc":"<?=$_GET['TienThuocTraLai']?>"}';
		datatosend =$.parseJSON(datatosend) ;		
	    $.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&hienmaloi=1',datatosend).done(function(data){
...
function sua(){
			...
			$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_sua&hienmaloi=1',datatosend).done(function(data){
				if($.trim(data)==2){
...
function sua(){
  ...
  function create_grid1() {      
        $("#rowed4").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_chitietcongno&id_benhnhan='+id_benhnhan,
            datatype: "json",
