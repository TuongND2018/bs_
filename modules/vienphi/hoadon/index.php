...
function create_grid() {
  ...
  $("#rowed3").jqGrid({
    url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_dalapHoadon',
    datatype: "json",
    colNames: ['','MaLoaiHD','Ngày Hóa đơn','Ngày khám', 'Mã BN', 'Họ tên','Số HĐ','Địa chỉ hóa đơn','Đối tượng','Phải trả','Miễn giảm','Nội dung','Chi tiết bảng kê','MaLoaiHD','','id thuTN','id_thue','Người hủy','Giờ hủy',''],
    colModel: [
  ...
  ondblClickRow: function(rowId) {
   ...
   {
    dialog_main("Hóa đơn "+ID_ThuTraNo+  '   ', 90, 90, "56743265743657", "resource.php?module=vienphi>&view=hoadon&action=HDChiTiet&type=test&id_form=11&ID_ThuTNo="+ID_ThuTraNo+"&phanloaiHD="+MaLoaiHD+"&ID_HoaDonThueDiary="+ID_HoaDonThueDiary);
   }
   else
   {
    dialog_main("Hóa đơn "+ID_ThuTraNo+  '   ', 90, 90, "56743265743657", "resource.php?module=vienphi>&view=hoadon&action=HDChiTiet&type=test&id_form=11&ID_ThuTNo="+ID_ThuTraNo+"&phanloaiHD="+MaLoaiHD+"&ID_HoaDonThueDiary="+ID_HoaDonThueDiary+"&ID_ThuTraNoMulti="+ID_ThuTraNoMultiP);
   }
...
function create_grid14() {
  ...
  $("#rowed14").jqGrid({
    url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_chualapHoaDon&tu_ngay='+ $( '#from_day' ).val()+'&den_ngay='+$( '#to_day' ).val()+'&id_loaiHD='+window.loaiHD,
    datatype: "json",
    colNames: ['LuotKham', 'Mã BN', 'Họ lót','Tên BN','Tên gói khám SKCT','Địa chỉ','Đối tượng','Thanh toán lúc','Tổng cộng','Miễn giảm','Tiền thuốc','Tiền khám CB','','Ngày khám','NgT1','NgT2','NoT1','NoT2'],
    colModel: [
    {name: 'ID_LuotKham', index: 'ID_LuotKham', width: "0%", editable: false, align: 'left', hidden: true},
    {name: 'MaBenhNhan',width: "3%",  classes: "CotSoTien", sortable: true,sorttype: 'number', index: 'MaBenhNhan', search: true, editable: false, align: 'left', hidden: false},
    {name: 'HoLotBenhNhan',width: "6%",  sortable: true, index: 'TenBenhNhan', search: true, editable: false, align: 'left', hidden: false},
    ...
    ondblClickRow: function(rowId) {
     ...
     dialog_main("Đang chọn loại "+ $("#com_thuochmk").val() +  '   ', 90, 90, "56743265743657", "resource.php?module=vienphi>&view=hoadon&action=HDChiTiet&type=test&id_form=11&ID_ThuTNo="+ID_ThuTraNo+"&phanloaiHD="+window.loaiHD);
  ...
  var rowData = jQuery('#rowed14').getRowData(ids[i]);
    ...
    //$("#rowed14").setCell(ids[i], 'DaCoHDKCBNgoaiTru', '', {background: 'red', color: 'black'});
...
alert('hãy chọn loại bảng kê');
...
else
    {
            ...
            var para="resource.php?module=vienphi>&view=hoadon&action=HDChiTiet&type=test&id_form=11&phanloaiHD="+tam+"&ID_ThuTraNoMulti="+ID_ThuTraNoMulti+"&_sumTienKham="+_sumTienKham+"&_sumTienThuoc="+_sumTienThuoc+"&_sumTienGiam="+_sumTienGiam;
//alert(para)
...
$( "#xem" ).click(function() {
 ...
 if(window.contro=="1"){
  ...
  $("#rowed3").jqGrid('setGridParam',{datatype:'json',url:'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_dalapHoadon&tu_ngay='+ $( '#from_day' ).val()+'&den_ngay='+$( '#to_day' ).val(),datatype:'json'}).trigger('reloadGrid');
}
...
function loadDatatheoComboLoaiHoadon(loaiHD)
{
  ...
  $("#rowed14").jqGrid('setGridParam',{datatype:'json',url:'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_chualapHoaDon&tu_ngay='+ $( '#from_day' ).val()+'&den_ngay='+$( '#to_day' ).val()+'&id_loaiHD='+loaiHD,datatype:'json'}).trigger('reloadGrid');
...
function moghichu(ID_HoaDonThueDiary,MaLoaiHD,ID_ThuTraNo){
 ...
 // dialog_main("Ghi chú của bệnh nhân: "+hoten, 95, 70, elem, 'resource.php?module=hanhchinh&view=ghi_chu&id_form=230&idbenhnhan=' + id_benhnhan);
  $.cookie("in_status", "print_preview");
           dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=vienphi&action=bangkehoadon&header=top&lien=3&type=report&id_form=11&kieuin=1&id_ttno="+ID_ThuTraNo+'&phanloaiHD='+MaLoaiHD+"&ID_HoaDonThueDiary="+ID_HoaDonThueDiary,'HoaDonChinhThuc');
