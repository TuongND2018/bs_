...
window.load_complete4=0;
	...
	$('#btn_bhyt').click(function(){
			$.cookie("in_status", "print_preview");
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=bhyt_nt&header=top&lien=2&type=report&id_form=10&idthutrano="+ID_ThuTraNo,'ChitietBHYT');
        ...
        $('#btn_hdKhamCb').click(function(){
           ...
           dialog_main("Hóa đơn khám chữa bệnh chi tiết "+ID_ThuTraNo+  '   ', 90, 90, "56743265743657", "resource.php?module=vienphi>&view=hoadon&action=HDChiTiet&type=test&id_form=11&ID_ThuTNo="+ID_ThuTraNo+"&phanloaiHD=7");  
        })
          $('#btn_gtgt').click(function(){
        
           dialog_main("Hóa đơn thuốc chi tiết ngoại trú "+ID_ThuTraNo+  '   ', 90, 90, "56743265743657", "resource.php?module=vienphi>&view=hoadon&action=HDChiTiet&type=test&id_form=11&ID_ThuTNo="+ID_ThuTraNo+"&phanloaiHD=6");  
        })
    ...
    resizerClass: 'ui-state-default'
     ...
     function create_grid() {
        ...
        $("#rowed3").jqGrid({
            url: url_rowed3,
            datatype: "json",
            colNames: ['ID_LoaiKham','ID_Kham','Tên loại khám','Phí thực hiện','Giá bảo hiểm','Số lần','Số ngày','Phần trăm','BHYT chi trả','Cùng chi trả','Hỗ trợ','Tổng phải trả','Tên nhóm','','','','','','Phí gốc'],
            colModel: [			
				...
				{name: 'songay', index: 'songay', search: false, width: "10%", editable: false, align: 'left', hidden: false}, 		
				...
				{name: 'Cungchitra', index: 'Cungchitra', search: false, width: "15%", editable: false, align: 'right', hidden: false,formatter:"integer"}, 
				{name: 'hotro', index: 'hotro', search: false, width: "15%", editable: false, align: 'right', hidden: false,formatter:"integer"},
				{name: 'tongphaitra', index: 'tongphaitra', search: false, width: "20%", editable: false, align: 'left', hidden: false,formatter:"integer"}, 
        		...
        		{name: 'ExtField1',index: 'ExtField1',hidden: true},
          		{name: 'id_thanhtoanbill',index: 'id_thanhtoanbill',hidden: true},
				{name: 'ngaybh',index: 'ngaybh',hidden: true},
				{name: 'ngay',index: 'ngay',hidden: true},
				{name: 'gio',index: 'gio',hidden: true},
				{name: 'phigoc', index: 'phigoc', search: false, width: "10%", editable: false, align: 'left', hidden: false,formatter:"integer"}, 
...
function create_grid_toathuoc() {
        ...
        $("#rowed4").jqGrid({
            url: url_rowed4,
            datatype: "json",
            colNames: ['Bác sỹ kê','Ngày kê toa','Tổng tiền','Cùng chi trả','Hỗ trợ','Phải trả'],
            colModel: [
                ...
                {name: 'cungchitr',formatter:"integer", index: 'cungchitr', search: false, width: "10%", editable: false, align: 'right', hidden: false},
           ...
           ondblClickRow: function(rowId) {
            ...
            loadComplete: function(data) {
			  ...
			  sum_tienvon =  $("#rowed4").jqGrid("getCol", "cungchitr", false, "sum");
			  sum_hotro =  $("#rowed4").jqGrid("getCol", "hotro", false, "sum");
		      sum_phaitra =  $("#rowed4").jqGrid("getCol", "phaitra", false, "sum");
			   $("#rowed4").jqGrid("footerData", "set", {cungchitr:sum_tienvon, ThanhTien: sum_ThanhTien,hotro:sum_hotro,phaitra:sum_phaitra});
...
function create_grid_toathuoc_chitiet() {
        ...
        $("#rowed5").jqGrid({
            datatype:'json',
            url: url_rowed5,
            colNames: ['Tên thuốc','ĐVT','Đơn giá','SL kê','SL Bn lấy','Thành tiền','Cùng chi trả'],
            colModel: [
           ...
           ondblClickRow: function(rowId) {
            ...
            loadComplete: function(data) {
			  ...
			  $("#rowed5").jqGrid("footerData", "set", {Cungchitra:sum_giavon, ThanhTien: sum_ThanhTien});
...
function luu(){
	...
	dem=dem+1;
		...
		sum_bhyt_thuoc = parseInt($("#rowed4").jqGrid("getCol","ThanhTien", false, "sum"))-parseInt($("#rowed4").jqGrid("getCol","cungchitr", false, "sum"));
		sum_bhyt_tra   = parseInt(sum_bhyt_kham)+parseInt(sum_bhyt_thuoc);
		
	if(tongcong>=0){
		miengiam_nv='';
		if($('#tiengiam').val()>0){
				miengiam_nv=',"id_miengiam":"'+$('#lydogiamgia1').val()+'","tien_miengiam":"'+parseInt ($('#tiengiam').val())+'"';
		}
			...
			window.datatosend+=',"bhyt":'+JSON.stringify(sum_bhyt_tra);
...
function tinhlai(){
	...
	hotro_thuoc=$("#rowed3").jqGrid("getCol", "hotro", false, "sum");
	hotro_kham=$("#rowed4").jqGrid("getCol", "hotro", false, "sum");

	sum  =  $("#rowed3").jqGrid("getCol", "PhiThucHien", false, "sum");
	sum1 =  $("#rowed4").jqGrid("getCol", "ThanhTien", false, "sum");
	ids  =  $("#rowed4").jqGrid('getDataIDs');

    vienphi =$("#rowed3").jqGrid("getCol", "PhiThucHien", false, "sum")+$("#rowed4").jqGrid("getCol", "cungchitr", false, "sum");
	 giamgia=giam_dtph+giam_chidinh+giam_thuoc+parseInt($('#tiengiam').val().split('.').join(''));
		...
		phaitra= $("#rowed3").jqGrid("getCol", "tongphaitra", false, "sum")+$("#rowed4").jqGrid("getCol", "phaitra", false, "sum")-giamgia+nodauky;
