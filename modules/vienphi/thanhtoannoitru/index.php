				...
				if(oper=='add'){
					...
					for(var i=0;i < ids.length;i++){	
						...
						if(rowData['ExtField1']=='Giuongbenh'||rowData['nhomcha']==22){
							if(rowData['ID_LoaiKham']==6219||rowData['ID_LoaiKham']==6218||rowData['ID_LoaiKham']==6225
							 ||rowData['ID_LoaiKham']==4736||rowData['ID_LoaiKham']==4737||rowData['ID_LoaiKham']==4741
							 ||rowData['ID_LoaiKham']==4742||rowData['ID_LoaiKham']==4745){
								
							}else{
								giamgia_tam=giamgia_tam+(rowData['PhiThucHien']*0.2);
							}
						}
					}
					$('#giamgia').val(parseInt(giamgia_tam).formatMoney(0, ',', '.'))
					$('#tiengiam').val(parseInt(giamgia_tam))
				}
...
function create_grid_toathuoc() {
        ...
        $("#rowed4").jqGrid({
            ...
            colModel: [
                ...
                {name: 'tienvon',index: 'tienvon',formatter:"integer", search: false, width: "35%", editable: false, align: 'right', hidden: false},
