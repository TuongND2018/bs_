...
<table>
    ...
    <div id="DM_template_container">
        ...
        <fieldset style="display:inline-block;height:188px;width:500px" id="thongtinnoptien" >
   		 <legend>Thông tin nộp tiền:</legend>
                 ...
                 <label style="width:90px;display:inline-block">Người nộp</label>  <input type='textbox' style="width:340px" id="nguoigd" value='<?php

				 if($_GET['oper']=='add'){
					 
						 echo $thongtinbenhnhan[0]['HoLotBenhNhan'].' '.$thongtinbenhnhan[0]['TenBenhNhan']; ;}
				 ...
				 ?>'>     <br>
                 <label style="width:90px;display:inline-block">Địa chỉ</label>  <input type='textbox' style="width:340px" id="diachi" value='<?php

				 if($_GET['oper']=='add')
					 {
						 echo $thongtinbenhnhan[0]['DiaChi'];
					 }
				 else{echo $tam[0]['DiaChi']; }?>'>     <br>
                 <label style="width:90px;display:inline-block">Diễn giải</label>  <input type='textbox' style="width:340px" id="diengiai"  value='<?php if($_GET['oper']=='add'){
                ...
                <!-- <label style="font-weight:bold;font-size:12px;width:90px;display:inline-block">Chi phí gốc</label>  <input type='textbox' value='0' disabled id="chiphigoc" style="width:100px;color:#CCC" >-->
                 <label style="font-weight:bold;font-size:12px;width:100px;display:inline-block">Lý do giảm giá</label>  <input id="lydogiamgia" type='textbox' style="width:220px" >
                <input id="lydogiamgia1" type='textbox' style="display:none" > <input style="margin-left:25px" type="checkbox" name="checkbox" id="nhanvien_giamthuoc" > <!--Có tiền thuốc  -->  <br>
       ...
       <fieldset style="display:inline-block;margin-top:7px;height:180px;width:600px" id="mid">
                <div class="form_row">
                    ...
                    <input lang='luu' name="tongcong" style="width:100px;height:25px;display:none" type="text" value="0" id="tongbhyt" disabled >
                    <br>
					...
					if($_GET['oper']=='add'){
							...
							echo  number_format($thongtinbenhnhan[0]['tamung'], 0, '', '.');
					...
					?>" id="tamung" disabled>
                    <label style="font-weight:bold;font-size:12px;width:70px;display:none">Bhyt trả</label>
                    <input lang='luu' name="tongphuthu" style="width:100px;height:25px;display:none" type="text" value="0" id="bhyttra" disabled>
                    <br>
                    ...
                    <input lang='luu' name="giamgia" style="width:100px;height:25px;background-color:#9F9;display:none" type="text" value="0" id="cungchitra" disabled>
                    <br>
...
<button href="#" id="btn_bhyt" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="margin-left:0px;  margin-bottom:1px; ">Bảng kê BHYT</button>
<button href="#" id="btn_bhyt_hotro" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="margin-left:0px;  margin-bottom:1px; ">Bảng kê BHYT có hỗ trợ</button>
<button href="#" id="btn_khonghotro" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="margin-left:0px;  margin-bottom:1px; ">Không hỗ trợ BHYT</button>
  </div>
...
$('#btn_trahet,#btn_sua,#btn_huy,#btn_lien1,#btn_lien2,#btn_trahet,#btn_luu,#btn_gtgt,#hentra,#btn_hdKhamCb,#btn_bhyt,#btn_khonghotro').button();
...
window.load_complete4=0;
	...
	$('#btn_bhyt_hotro').click(function(){
			$.cookie("in_status", "print_preview");
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=bhyt_cohotro&header=top&lien=2&type=report&id_form=10&idthutrano="+ID_ThuTraNo,'ChitietBHYT');
			 $('#dialog_print').dialog('close');
		})
        ...
        $('#btn_hdKhamCb').click(function(){
          ...
          $('#btn_gtgt').click(function(){
           ...
           dialog_main("Hóa đơn thuốc chi tiết ngoại trú "+ID_ThuTraNo+  '   ', 90, 90, "56743265743657", "resource.php?module=vienphi>&view=hoadon&action=HDChiTiet&type=test&id_form=11&ID_ThuTNo="+ID_ThuTraNo+"&phanloaiHD=5");  
        })
...
$('#btn_khonghotro').click(function(){			
			$('#rowed3').jqGrid('setGridParam', { datatype: "json",url:'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_vienphichitiet_theoluotkham&ID_LuotKham='+ID_LuotKham+'&ko_hotro=1' }).trigger("reloadGrid");
			$('#rowed4').jqGrid('setGridParam', { datatype: "json",url:'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_donthuoc_luotkham&ID_LuotKham='+ID_LuotKham+'&ko_hotro=1' }).trigger("reloadGrid");
			$('#rowed5').jqGrid('setGridParam', { datatype: "json",url:'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_toathuoc&ID_LuotKham='+ID_LuotKham+'&ko_hotro=1' }).trigger("reloadGrid");
    ...
    resizerClass: 'ui-state-default'
     ...
     function create_grid() {
        ...
        $("#rowed3").jqGrid({
            url: url_rowed3,
            datatype: "json",
            colNames: ['ID_LoaiKham','ID_Kham','Tên loại khám','Trạng thái','Phí thực hiện','Giá bảo hiểm','Số lần','Phần trăm','BHYT chi trả','Cùng chi trả','Hỗ trợ BV','Tổng phải trả','Tiền vốn','Tên nhóm','',''],
            colModel: [			
				...
				{name: 'GiaBaoHiem',formatter:"integer",sorttype:"number", summaryType: 'sum',index: 'GiaBaoHiem', search: false, width: "10%", editable: false, align: 'right', hidden: false},
				{name: 'solan', index: 'solan', search: false, width: "5%", editable: false, align: 'left', hidden: false}, 			
				{name: 'Phantram', index: 'Phantram', search: false, width: "5%", editable: false, align: 'left', hidden: false}, 
				{name: 'bhyt', index: 'bhyt', search: false, width: "10%", editable: false, align: 'right', hidden: false,formatter:"integer"}, 
				{name: 'Cungchitra', index: 'Cungchitra', search: false, width: "10%", editable: false, align: 'right', hidden: false,formatter:"integer"},
				{name: 'hotro', index: 'hotro', search: false, width: "15%", editable: false, align: 'right', hidden: false,formatter:"integer"},  				
           ...
           ondblClickRow: function(rowId) {
            ...
            loadComplete: function(data) {
				...
				grid.jqGrid('footerData','set', {ID: 'GiaThueNgoaiThucHien:',bhyt:tong_bhyt, GiaThueNgoaiThucHien: sumGiaThueNgoaiThucHien,tongphaitra:tongphaitra_bhyt,hotro:tong_hotro,Cungchitra:tong_cungchitra});
            },
...
function create_grid_toathuoc_chitiet() {
        ...
        $("#rowed5").jqGrid({
            datatype:'json',
            url: url_rowed5,
            colNames: ['ID_DonThuocCT','Tên thuốc','ĐVT','Đơn giá','Đơn giá cùng chi trả','SL kê','SL Bn lấy','Thành tiền','Cùng chi trả','Hỗ trợ','Phải trả','','',''],
            colModel: [

                {name: 'ID_DonThuocCT', index: 'ID_DonThuocCT', width: "0%", editable: false, align: 'left', hidden: true},
                {name: 'TenThuoc', index: 'TenThuoc', search: true, width: "35%", editable: false, align: 'left', hidden: false},
				...
				{name: 'GiaCungchitra',formatter:"integer", index: 'Gia', search: false, width: "8%", editable: false, align: 'right', hidden: false},
				...
				{name: 'Cungchitra',formatter:"currency",formatoptions:{decimalSeparator:",", thousandsSeparator: ".", decimalPlaces: 0, prefix: "", defaultValue: '0'}, index: 'Cungchitra', search: false, width: "10%", editable: false, align: 'right', hidden: false},
				{name: 'hotro',formatter:"integer", index: 'hotro', search: false, width: "10%", editable: false, align: 'right', hidden: false},
...
function luu(){
	...
	if(tongcong>=0){
		...
		//console.log(parseInt($("#rowed5").jqGrid("getCol","ThanhTien", false, "sum"))-parseInt($("#rowed5").jqGrid("getCol","Cungchitra", false, "sum")));
		...
		window.datatosend = '{';
			/*"id_giam_nv":"'+id_giam_nv+'","ID_LuotKham":"'+ID_LuotKham+'","id_giam_thuoc":"'+id_giam_thuoc+'","id_giam_chidinh":"'+id_giam_chidinh+'","id_giam_dtph":"'+id_giam_dtph+'","idbenhnhan":"'+idbenhnhan+'","NgayGio":'+JSON.stringify($('#ngay').val())+',"nguoigd":"'+$('#nguoigd').val()+'","diachi":"'+$('#diachi').val()+'","diengiai":"'+$('#diengiai').val()+'","tienthu":"'+$('#benhnhantra').val().split('.').join('')+'","tongcong":"'+tongcong+'","id_luotkham":"'+id_luotkham+'","tienthu_hoantra":"'+(parseInt ($('#benhnhantra').val().split('.').join(''))-parseInt (phaitra))+'","giamgia":"'+giamgia+'","dskho":"'+dskho+'","sum":"'+sum+'","sumvat":"'+sumvat+'","sum_bhyttra":'+sum_bhyt_tra+',"lydothieu":'+JSON.stringify($.trim(window.lydothieu))+',"iddonthuoc":"'+window.id_donthuoc+'"'+miengiam_nv;*/
...
function tinhlai(){
	...
	if($('#'+ids[0]+'_laythuoc	').is(':checked')){
		...
		phaitra= $("#rowed3").jqGrid("getCol", "tongphaitra", false, "sum")+tienthuoc_tra-giamgia+nodauky;
