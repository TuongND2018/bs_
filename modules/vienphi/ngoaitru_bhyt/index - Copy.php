<?php
	$data= new SQLServer;
	if($_GET['oper']=='add'){
	$store="{call Gd2_thongtinthanhtoan_ngoaitru (?)}";
	$param = array($_GET["ID_LuotKham"] );
    ...
    <div id="DM_template_container">
        ...
        <fieldset style="display:inline-block;height:188px;width:700px" id="thongtinnoptien" >
   		 <legend>Thông tin nộp tiền:</legend>
				 ...
				 if($_GET['oper']=='add'){
					 if($thongtinvienphi[0]['ID_PhanLoaiKham']==25){
						echo $thongtinvienphi[0]["TenCty"];
					 }else{
						 echo $thongtinvienphi[0]["HoLotBenhNhan"].' '.$thongtinvienphi[0]["TenBenhNhan"];}
					 }
				 ...
				 if($_GET['oper']=='add')
					 {
						 if($thongtinvienphi[0]['ID_PhanLoaiKham']==25){
							echo $thongtinvienphi[0]["DiaChicty"];
						 }else{
       ...
       <fieldset style="display:inline-block;margin-top:7px;height:180px;width:400px" id="mid">
                <div class="form_row">
                    ...
                    <input lang='luu' name="tienthua" style="width:100px;height:25px" type="text" value="<?php 					
					if($_GET['oper']=='add'){
						echo '0';						
					}else{
							echo  number_format($tam[0]['tamung'], 0, '', '.');
					 }
					?>" id="tienthua" disabled>
...
<div > 
            <div id="tabs">
              ...
              <input lang='luu' name="benhnhantra" style="width:90px;margin-left:60px" type="text" id="giamdtph" value="<?=$thongtinvienphi[0]['SoTienGiam_dtphoihop']?>" disabled >
              </span>
              <label style="font-weight:bold;font-size:12px;width:100px;display:inline-block">Giảm Chỉ Định</label>
              <input lang='luu' name="benhnhantra" style="width:90px" type="text" value="<?=$thongtinvienphi[0]['SoTienGiam_chidinh']?>" id="giamchidinh" disabled >
              </span>
              </div>
                          </div > 
...
$('#btn_trahet,#btn_sua,#btn_huy,#btn_lien1,#btn_lien2,#btn_trahet,#btn_luu,#btn_gtgt,#hentra').button();
...
var giam_chidinh=<?=$thongtinvienphi[0]['SoTienGiam_chidinh']?>;
var giam_thuoc=<?=$thongtinvienphi[0]['SoTienGiam_thuoc']?>;

var id_giam_dtph='<?=$thongtinvienphi[0]['ID_dtphoihop']?>';
var id_giam_chidinh='<?=$thongtinvienphi[0]['ID_chidinh']?>';
var id_giam_thuoc='<?=$thongtinvienphi[0]['ID_thuoc']?>';

var nhanvien_giamthuoc;


var nodauky=<?php if($_GET['oper']=='add'){
					echo  $thongtinvienphi[0]['nocu'];
