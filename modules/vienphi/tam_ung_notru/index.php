 ...
 <?php 
	if(!isset($_GET['oper'])){
		...
		if($_GET['oper']=='edit'){
			...
			$excute1 = new SQLServerResult($get1);
			$tam1 = $excute1->get_as_array();			
			$store_name1="{call GD2_thongtin_tamungnoitru (?)}";
                   ...
                   <td  align="right" >
                    ...
                    if(!isset($_GET['oper'])){
						...
						if($_GET['oper']=='edit'){
							...
							echo (number_format(($tam2[0]['tongtienkham']+$tam2[0]['tienthuoc']-$tam2[0]['tienthuoctralai']+$tam2[0]['tiengiuong']+$tam2[0]['tongdichvu']), 0, '', '.'));							
						}
					}
					?>"> 
                      </td>
                   ...
                   <label for="tienthu" style="width:60px; "><strong>TIỀN THU:</strong></label>
                    ...
                    if(!isset($_GET['oper'])){
						...
						if($_GET['oper']=='edit'){
							...
							if($_GET['idthutrano']=='tamung'){
								echo (number_format(($tam2[0]['tongtienkham']+$tam2[0]['tienthuoc']-$tam2[0]['tiengiuong']+$tam2[0]['tongdichvu']-$tam2[0]['tamung']), 0, '', '.'));				
							}else{
								echo number_format(($_GET['idthutrano']), 0, '', '.');		
							}
...
$('#id_print').keydown(function(e){
			...
			if($('#id_print').val()==4){
			 ...
			 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=tam_ung&header=top&lien="+$('#id_print').val()+"&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu',100);
			 $('#dialog_print').dialog('close');
			}
...
setTimeout(function(){$("#tienthu").focus()},600); ;
   ...
   $('#btn_lien1').click(function(){
		 ...
		 dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=tam_ung&header=top&lien=2&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
		 $('#dialog_print').dialog('close');
	})
	$('#btn_lien2').click(function(){
	   	$.cookie("in_status", "print_preview"); 
	    dialog_report("Xem trước khi in",90,90,"u78787878975f","resource.php?module=report&view=<?=$modules?>&action=tam_ung&header=top&lien=3&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
		$('#dialog_print').dialog('close');	   
	})   
 ...
 function luu(){
   ...
   function create_grid() {      
        $("#rowed3").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_vienphichitiet_theoluotkham&ID_LuotKham='+idluotkham,
            datatype: "json",
            colNames: ['ID_Kham','Ngày chỉ định','Tên loại khám','Phí thực hiện' ,'Tên nhóm','Phí gốc'],
            colModel: [			
