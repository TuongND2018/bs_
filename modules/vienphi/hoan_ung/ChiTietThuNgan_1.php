   ...
   $("#subToathuoc").layout({
    resizerClass: 'ui-state-default'
     ...
     function create_grid() {
        ...
        $("#rowed3").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_ds_vienphichitiet_theoluotkham&ID_LuotKham=<?= $_GET["ID_LuotKham"] ?>',
            datatype: "json",
        ...
        'ID_Kham','ID_LuotKham','MaBenhNhan','TenBenhNhan','Ngày chỉ định','Tên loại khám','Trạng thái','Phí thực hiện' ,'Tên nhóm','Phí gốc'],
         ...
         var rowData = jQuery(this).getRowData(rowId); 
                 ...
                 dialog_main("Chi tiết viện phí " + ' BN '+TenBenhNhan+' - '+MaBenhNhan, 100, 100, 5674365743657, "resource.php?module=<?=$modules?>&view=<?=$view?>&action=ChiTietThuNgan_1&type=test&id_form=139&ID_LuotKham="+ID_LuotKham);
...
function create_grid_toathuoc() {
        ...
        $("#rowed4").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_donthuoc_luotkham&ID_LuotKham=<?= $_GET["ID_LuotKham"] ?>',
            datatype: "json",
            colNames: ['ID_DonThuoc', 
        'ID_LuotKham','Bác sỹ kê','Ngày kê toa','Tổng tiền','Tiền giảm'],
            colModel: [
...
function create_grid_toathuoc_chitiet() {
        ...
        $("#rowed5").jqGrid({
            url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_toathuoc&ID_LuotKham=<?= $_GET["ID_LuotKham"] ?>',
            datatype: "json",
    ...
    $.getJSON( "resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_chitiet_vienphi_luotkhamcuoi&ID_LuotKham="+<?= $_GET["ID_LuotKham"] ?>, 
        function( data ) {
            data_luotkham=data;
       // alertObject(data);
        var tam1_cls="";
