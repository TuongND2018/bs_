...
/*window.onbeforeunload = function () {
	...
	var plugin_version_window_data=$.cookie("Windowpluginversion");
	var plugin_version_window_plugin="";
	
	$.cookie("plugin_version_window","");	
	window.addEventListener ("plugin_version_window", function (zEvent) {
			$.cookie("plugin_version_window", zEvent.detail);			
			 //alert($.cookie('Windowpluginversion'))
			//alert($.cookie("plugin_version_window"));			 
			 plugin_version_window_plugin=zEvent.detail;
			//alert (plugin_version_window_plugin)
...
$(document).ready(function(){
	...
	//alert(plugin_version_window_plugin)
			 plugin=setTimeout(function(){
					/*if($.trim(plugin_version_window_data)!=$.trim(plugin_version_window_plugin)){
						//alert("Plugin của bạn đã cũ, bấm vào đây để cập nhật")
						  dialog_main("Thông báo", "290px", "80px", "jfghfhgfdhgdfhgdj", "") ;
						  
						  $(".modaljfghfhgfdhgdfhgdj").html("Plugin của bạn đã cũ, <a id='capnhat' href='firefox/plugin_firefox/lastest/family-hospital-printer-management_lastest.xpi?="+(1 + Math.floor(Math.random() * 10000000000000))+"' > bấm vào đây để cập nhật </a>");
						  $("#capnhat").click(function(e) {
							$(".modaljfghfhgfdhgdfhgdj").dialog("close");						
						});
					} */
			},5000);
		...
		var tabTitle = $( "#tab_title" ),
	 	...
	 	this.creatTab = function() {
			...
			tabs.delegate( "span.ui-icon-close", "click", function(e) {
				...
				/*if(e.which) {
					if($(e.target).attr('class').split(' ')[2].split('_')[0]=='benhan'){					
						if($(e.target).attr('class').split(' ')[2].split('_')[1]=='undefined-close'){	
							...
							document.getElementById('benh_an-'+$(e.target).attr('class').split(' ')[2].split('_')[1].split('-')[0]+'-frame').contentWindow.luu_thoatform();			
						}
					}else{*/
						var panelId = $( this ).closest( "li" ).remove().attr( "aria-controls" );
						temp=$( "#" + panelId ).attr("lang");
...
function create_layout(){
	$('#panel_main').layout({
		...
		,	onopen_end:function () {
				...
				$('#scrollbar1').tinyscrollbar_update("relative");
				$("#tabs iframe").css("height",$(".right_col").height()-76+"px");
			}
		,	onclose_end:function () {
				$('#scrollbar1,#scrollbar1 .scrollbar,#scrollbar1 .viewport,#scrollbar1 .track').css("height",$('.left_col').height()+"px");
				$('#scrollbar1').tinyscrollbar_update("relative");
