		...
		return this.each(function(){
			...
			$($t).keydown(function(event){
				...
				if(target) {
					...
					}//sua chua cho nay su kien hoat dong khi row mở					
				}
			});
		});
	},
...
//Helper functions
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		$("#closedialog", "#info_id").click(function(){
			// chinh sua focus giobatdau sau khi loi
			
			//
			//alert($("#info_dialog").data("onClose"));
...
$.jgrid.extend({
//Editing
	editRow : function(rowid,keys,oneditfunc,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc,formname) {
		...
		if( $.type(args[0]) === "object" ) {
			...
			if (formname !== undefined) { o.formname = formname; }
					...
					if(o.keys===true) {
						$(ind).bind("keydown",function(e) {
							...
							if (e.keyCode === 27) {
								...
								window.lastsel=-1 ;//reset bien danh dau dong hien tai
								return false;
							}
							//sua chua
							//grid_mode:"save_inline",
	...
	custom_tab_enter : function(grid) {
			
	},
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc,formname) {
		// Compatible mode old versions
			...
			if(fr >= 0) {
				...
				if($("#"+$.jgrid.jqID(rowid), "#"+$.jgrid.jqID($t.p.id)).hasClass("jqgrid-new-row")){
					setTimeout(function(){
						...
						//alert (afterrestorefunc["formname"]);
						//chinh sua from lich
						//alert (afterrestorefunc["formname"]);
