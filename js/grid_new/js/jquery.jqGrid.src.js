...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			cell_move_enter:false,//enter nhay cell
			inline_esc:true,//khong dong inline khi esc
			enter_cell:true,//luu cell khi enter
			inline_editrule:false,//edit rule inline
			grid_mode:"save_inline",
			grid_move:"enter/tab",
			grid_inline_save:"ctrl+enter/cmd+enter",
			grid_save_option:"enter_to_save",		 
			column:[],
			func_column:[],//them cac bien config
			url: "",
			height: 150,
			page: 1,
			rowNum: 20,
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(ci){
				...
				if(this.search){
					...
					//sua chua bo dau clear search
					if(soptions.clearSearch === undefined) {
						soptions.clearSearch = false;
					}
						...
						if(p.autosearch===true){
							if(p.searchOnEnter) {
								...
								$("input",thd).keydown(function(e){
									...
									switch (key) {
										case 13:
										//sua chua enter van~ search
										
										if ((typeof $($t).data('input_focus') == 'undefined')||$($t).data('input_focus')==''){
											
										}else{
											
											$($($t).data('input_focus')).focus();
											$($($t).data('input_focus')).select();
										}
											//return true;
										case 9 :
										case 16:
...
//Helper functions
	...
	info_dialog : function(caption, content,c_b, modalopt,id_focus) {
		var mopt = {
			width:290,
			height:'auto',
			dataheight: 'auto',
		...
		//sua chua
		$("#info_dialog").keydown(function(e){
			if(e.keyCode==13){
			$("#closedialog").click()
			}
		});
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			...
			case "button" :
				...
				if(eltype !== "button"){
					if(autowidth) {
						if(!options.size) { $(elem).css({width:"82%"}); }
	...
	checkValues : function(val, valref, customobject, nam) {
		...
		if(edtrul) {
			...
			if(edtrul.required === true) {
				//sua chua cm[valref].name lay colmodel name return cv[2]
				if( $.jgrid.isEmpty(val) )  { return [false,nm+": "+$.jgrid.edit.msg.required,cm[valref].name]; }
			}
			// force required
			var rqfield = edtrul.required === false ? false : true;
			if(edtrul.number === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if(isNaN(val)) { return [false,nm+": "+$.jgrid.edit.msg.number,cm[valref].name]; }
				}
			}
			if(edtrul.minValue !== undefined && !isNaN(edtrul.minValue)) {
				if (parseFloat(val) < parseFloat(edtrul.minValue) ) { return [false,nm+": "+$.jgrid.edit.msg.minValue+" "+edtrul.minValue,cm[valref].name];}
			}
			if(edtrul.maxValue !== undefined && !isNaN(edtrul.maxValue)) {
				if (parseFloat(val) > parseFloat(edtrul.maxValue) ) { return [false,nm+": "+$.jgrid.edit.msg.maxValue+" "+edtrul.maxValue,cm[valref].name];}
			}
			var filter;
			if(edtrul.email === true) {
				...
				// taken from $ Validate plugin
					...
					if(!filter.test(val)) {return [false,nm+": "+$.jgrid.edit.msg.email,cm[valref].name];}
				}
			}
			if(edtrul.integer === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if(isNaN(val)) { return [false,nm+": "+$.jgrid.edit.msg.integer,cm[valref].name]; }
					if ((val % 1 !== 0) || (val.indexOf('.') !== -1)) { return [false,nm+": "+$.jgrid.edit.msg.integer,cm[valref].name];}
				}
			}
			if(edtrul.date === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					...
					if(!$.jgrid.checkDate (dft, val)) { return [false,nm+": "+$.jgrid.edit.msg.date+" - "+dft,cm[valref].name]; }
				}
			}
			if(edtrul.time === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if(!$.jgrid.checkTime (val)) { return [false,nm+": "+$.jgrid.edit.msg.date+" - hh:mm (am/pm)",cm[valref].name]; }
				}
			}
			if(edtrul.url === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					filter = /^(((https?)|(ftp)):\/\/([\-\w]+\.)+\w{2,3}(\/[%\-\w]+(\.\w{2,})?)*(([\w\-\.\?\\\/+@&#;`~=%!]*)(\.\w{2,})?)*\/?)/i;
					if(!filter.test(val)) {return [false,nm+": "+$.jgrid.edit.msg.url,cm[valref].name];}
				}
			}
			if(edtrul.custom === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if($.isFunction(edtrul.custom_func)) {
						...
						return $.isArray(ret) ? ret : [false,$.jgrid.edit.msg.customarray,cm[valref].name];
					}
					return [false,$.jgrid.edit.msg.customfcheck,cm[valref].name];
				}
			}
		}
		return [true,"",""];
...
//Editing
			...
			if (editable === "0" && !$(ind).hasClass("not-editable-row")) {
				...
				if(cnt > 0) {
					...
					if(o.keys===true) {
						...
						if($($t).get_config().inline_esc===true){
							if (e.keyCode === 27) {
							...
							//grid_mode:"save_inline",
							//grid_move:"enter",
							//grid_inline_save:"ctrl/cmd+enter",
							//grid_save_option:	"enter_to_save"	
							if($($t).get_config().grid_mode=="save_inline"){						 
								 if (jwerty.is($($t).get_config().grid_inline_save,e)) {
									var ta = e.target;
									...
									return false;								 
								 }else if (jwerty.is($($t).get_config().grid_move,e)){																
									var grid =$($t);
									var colModel = grid.getGridParam('colModel');
									//grid.get_config();
									var tam=e.target.id.replace(rowid+"_","");
									 
									var ii=0;																 
									for(var i=0;i<=colModel.length-1;i++){
										 if(colModel[i].name==tam){
											 ii=i;
										 }
									}
									if(ii<colModel.length-1){								
										for(i=ii;i<=colModel.length-1;i++){																				  
												 if((colModel[i+1].hidden==false)||(colModel[i+1].editable==true)){
													 ii=i+1;
													 break;
												 }								
										}
										
									}else if(ii==colModel.length-1){
										var ta = e.target;
										//alert (ta.tagName);
										//if(ta.tagName === 'TEXTAREA') { return false; }
										if( grid.jqGrid("saveRow", rowid, o ) ) {
											if($t.p._inlinenav) {
												try {
									...
									var colName = colModel[ii].name;
									window.cache_colName = colModel[ii].name;									
									var input = $('#' + rowid + '_' + colName);										
									//column:{"name":"TenBietDuoc","func":"luoiten"},
									//kiemtra=false;
									//alert(colModel[ii-1].name)
									if(colModel[ii-1].newgrid==true){
										window[colModel[ii-1].func_grid]();
										
									}else{	
									   if($($t).get_config().grid_save_option=="enter_to_save"){
										  /*  
											...
											{ 
											 ...
											 //alert($($t).get_config().pager)
											  $($($t).get_config().pager + " .ui-icon-disk").click();
											  
											  $($($t).get_config().pager + " .ui-icon-pencil").click();	
											  $('#' + rowid + '_' + colName).focus();
											  if(colModel[i].edittype!="select"){
											 	 $('#' + rowid + '_' + colName).select();	
											  }
									...
									/*for(var iii=0;iii<($($t).get_config().column).length;iii++){
										 if(colModel[i].name==$($t).get_config().column[iii]){
									 		window[$($t).get_config().func_column[iii]]();
											kiemtra=true;
											break;
										 }
									}
									...
									/*	var obj=jQuery.jgrid.defaults;
									  ...
									  grid.get_config
									 */
									//alert('#' + rowid + '_' + colName)
									//$("#52_ID_KieuPhong").focus();									
									// alert(grid.attr("id"))
									//alert(e.target.id);
									...
									//alert(tam)
									//var currcolModel  = grid.jqGrid('getGridParam','colModel');
									//alert(currcolModel[0].hidden)
		...
		if (editable==="1") {
			...
			if (cv[0] === false){
				try {
					...
					//sua chua
					if($($t).get_config().inline_editrule===true){	
										
						$("#"+rowid+"_"+cv[2]).focus();
						$("#"+rowid+"_"+cv[2]).select();
					}else{
					
						var id_focus_inline =  rowid+"_"+cv[2];
						var left_pos=(Math.round($(window).width() / 2)  -150);
						var top_pos=(Math.round($(window).height() / 2)  -150);
						var tr = $($t).jqGrid('getGridRowById', rowid), positions = $.jgrid.findPos(tr);
						$.jgrid.info_dialog($.jgrid.errors.errcap,cv[1],$.jgrid.edit.bClose,{left:left_pos ,top:top_pos},id_focus_inline);
					}
				} catch (e) {
					alert(cv[1]);
				}
	...
	inlineNav : function (elem, o) {
		...
		return this.each(function(){
			...
			if(o.save) {
				$($t).jqGrid('navButtonAdd', elem,{
					caption : o.savetext || 'Lưu (F10)',
					title : o.savetitle || 'Lưu Dòng',
			...
			if(o.cancel) {
				$($t).jqGrid('navButtonAdd', elem,{
					caption : o.canceltext || 'Hủy (ESC)',
					title : o.canceltitle || 'Hủy Dòng Đang Chọn',
	...
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if ($t.p.savedRow.length>0) {
				...
				window.setTimeout(function () { $("#"+$.jgrid.jqID($t.p.knv)).attr("tabindex","-1").focus();
				//sua chua select lun edit cell
				$("#"+$.jgrid.jqID($t.p.knv)).select();
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$.jgrid.bindEv.call($t, elc, opt);
				window.setTimeout(function () { $(elc).focus();
				$(elc).select();
				},0);
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					//sua chua
					//chuyen cell va luu khi enter
					if($($t).get_config().cell_move_enter===true){
						if (e.keyCode === 13) {
							//luu khi enter cell 
							//if($($t).get_config().enter_cell===true){
								//e.preventDefault(); 
								//e.stopPropagation();
								//$(e.target).unbind();
							...
							//if (iRow+1 <=  $t.rows.length-1) {
								//$($t).jqGrid("saveCell",iRow,iCol);							
								//$($t).jqGrid("GridNav",iRow+1,iCol,'vd');
							/*	scrollGrid(iRow+1,iCol,'vd');	
								setTimeout(function(){
									scrollGrid(iRow-1,iCol,'vu');	
									},200)*/
								//$($t).jqGrid("GridNav")
								$($t).jqGrid("downCell",iRow,iCol);	
								//window.setTimeout(function () {$($t).jqGrid("editCell",$t.p.iRow-1,$t.p.iCol,true);},0);
								if(iRow+1 <=$t.p.reccount ) {
									e.preventDefault(); 
									e.stopPropagation();
									$(e.target).unbind();
								}
						...
						if (e.keyCode === 9)  {
							if(!$t.grid.hDiv.loading ) {
								if (e.shiftKey) {$($t).jqGrid("prevCell",iRow,iCol);
								e.preventDefault(); 
								e.stopPropagation();
								$(e.target).unbind();
								} //Shift TAb
								else {$($t).jqGrid("nextCell",iRow,iCol);
								e.preventDefault(); 
								e.stopPropagation();
								$(e.target).unbind();
					...
					if(cv[0] === true) {
						...
						if ($t.p.cellsubmit === 'remote') {
							if ($t.p.cellurl) {
								...
								try {
								//	alert($("#"+iRow+"_"+nmjq,$t.rows[iRow]).val();)
					...
					$($t).jqGrid("saveCell",iRow,iCol);
						if($($t).get_config().cell_move_enter===true){
							setTimeout(function(){			
								 ...
								 $($($t).get_config().pager + " .ui-icon-plus").click();
