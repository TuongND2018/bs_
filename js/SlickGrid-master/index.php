...
jQuery(document).ready(function() {
	...
	window.idncc_search=0;
	 window.dataView;
 	 window.grid;
	 	$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_baocao_tonghop&from_day="+$("#from_day").val()+"&to_day="+$("#to_day").val()+"&tenkho="+$("#makho1").val()+"&n_order=ORDER BY TenBietDuoc,STT_UUTIEN,SoLo").done(function(data){
			data=$.parseJSON(data)
