...
function edit_pdf_ecg(){
	...
	if(($_POST["loai_ecg"]==2)||($_POST["loai_ecg"]==3)||($_POST["loai_ecg"]==4)){			
		dientimdidong($mpdf,$DocName,$default_url,$thongtinluotkham,$sign_path);	 
	}
	$_FILES["upload"]["name"][1]=$DocName;
	$_FILES["upload"]["type"][1]="application\/vnd.ms-word"; 
	$_FILES["upload"]["tmp_name"][1]=STRING_PATH.$DocName;
...
function dientimdidong($mpdf,$DocName,$default_url,$thongtinluotkham,$sign_path){	
...
/*	$mpdf->SetFont('helvetica', '',6);		 
	...
	$html = '
				...
				<div style="font-family:Arial, Geneva, sans-serif;
				 	...
				 	<span  style="letter-spacing: 0.5px;font-weight:bold;color:#00b3a0">FAMILY</span> <img style="height:35px" src="'.$default_url.'images/logo_mau.png" />  <span style="letter-spacing: 0.5px;text-transform:uppercase">'.$_POST["tenbv"].'</span> - '.$_POST["diachi"].' * T: '.$_POST["dienthoai"].'
				</div>
				...
				<div style="font-family:Arial, Geneva, sans-serif;
				 ...
				 '
				;	
				//echo $default_url."file_manager/php/connector.php?tenthumuc=".$sign_path."&answer=42&cmd=file&target="."f1_".base64_encode($thongtinluotkham[0][chuky_bacsy]);
	$mpdf->WriteHTML($html);	
...
function convert_date($string){
	...
	if(preg_match("/\ /",$string)){
		...
		if(mb_strlen($tam[0])>mb_strlen($tam[1])){			 
			$tam1=explode('/',$tam[0]); 
			return $tam1[2]."-".$tam1[1]."-".$tam1[0]." ".$tam[1];			
		}else{		 
			$tam1=explode('/',$tam[1]); 
			return $tam1[2]."-".$tam1[1]."-".$tam1[0]." ".$tam[0];			
		}				 
	}else{
		$tam1=explode('/',$string); 
		return $tam1[2]."-".$tam1[1]."-".$tam1[0];
	}
}
function taoconfig(){
